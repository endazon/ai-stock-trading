...
- ソリューション: `backend/backend.slnx`（ユニットリポジトリレイアウト。IADR-0046）。共有物は `backend/Shared/AiStockTrading.Shared.*`。**サービスは単一プロジェクト＋VSA へ全面移行中**（[IADR-0259](.ai-context/adr/IADR-0259_single-project-vsa-structure.md)。MSP も同樹形へ移行中＝**逸脱ではなく整合**）—— 新は `Services/<Name>/` 直下の `<Name>.csproj` ＋ `Features/<集約>/<操作>/ Domain/ Infrastructure/ Hosted/ Common/ Tests/<Name>.Tests.csproj`（`Hosted/` は本リポ固有の第4の頂点。3段化は platform ADR-0065 決定2に沿って採る＝移送は#613。IADR-0276）、旧は `Services/<Svc>/{src,tests}` の `<Svc>.{Api,Application,Domain,Infrastructure}`。🔴 **サービス単位で新旧が混在する**（現物を見てから触る。**移送波の完了までは新規コードも現行配置で書く**）。依存方向は自前検査器（IADR-0256）のみで強制する（NsDepCop は導入しない）。**名前空間は先行して基盤へ完全整合済み**（`<Svc>Service.*`。IADR-0259 決定5 / IADR-0261。フォルダの再配置は移送波）。
- 命名規約: サービスのルート名前空間は `<Svc>Service`（基盤と同規則。IADR-0261 で整合済み）、共有物は `AiStockTrading.Shared.*` 据え置き。公開メンバは PascalCase、private フィールドは `_camelCase`。テストメソッド名は日本語可（ただし識別子に全角記号は使えない）。
