...
// NFR, FR-03, FR-10, MSP:ADR-0029, MSP:ADR-0075, IADR-0284 決定 5（段 2）, IADR-0328, IADR-0331, IADR-0427 決定 5, #997 (#753):
// リスク管理の読み取り（`aistocktrading.riskmanagement.v1.RiskControlsRead`）を呼ぶ**本サービスの**輸送。
// 構成 `RiskManagement:Grpc`（宛先）を宣言したときだけ DI に載り、載っていれば保有ポジション（損切り検知の入力）のポートが
// `Grpc*` 実装を選ぶ（**既定は REST**。宣言しなければ本ファイルは何も登録しない＝振る舞いは 1 つも変わらない）。
//
// 🔴 **チャネルは本型が所有する（`GrpcChannel` を DI へ裸で登録しない）。** 取引判断・費用統制では段 1 の全体前提条件が
// `GrpcChannel` を singleton で引いており、裸で登録すると後勝ちで別の宛先へ飛ぶ（取引判断の同名の型と同じ形に揃える。
// 呼び出し元ごとの複製は `*.Client` を廃した裁定どおり。IADR-0264 決定 1）。
//
...
//     損切り優先の巡回を長時間ブロックしない（REST と同じ理由）。
