...
// 母集団の実測（develop 3d9b91c2）: roots=14 handlers=0 fields=13。下限はその半分に置く。
public class CompositionWiringGuardTests(ITestOutputHelper output)
{
    ...
    {
        using var factory = new InformationCollectionWorkerWebApplicationFactory();

        var report = factory.InspectComposition(typeof(CompositionWiringGuardTests).Assembly);

        output.WriteLine(report.Describe("InformationCollectionService"));
        report.AssertPopulation(minRoots: 7, minFields: 6, minHandlers: 0);
