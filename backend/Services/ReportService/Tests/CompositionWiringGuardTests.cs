...
// 母集団の実測（develop 3d9b91c2）: roots=34 handlers=0 fields=47。下限はその半分に置く。
public class CompositionWiringGuardTests(ITestOutputHelper output)
{
    ...
    {
        using var factory = new ReportWorkerWebApplicationFactory();

        var report = factory.InspectComposition(typeof(CompositionWiringGuardTests).Assembly);

        output.WriteLine(report.Describe("ReportService"));
        report.AssertPopulation(minRoots: 17, minFields: 23, minHandlers: 0);
