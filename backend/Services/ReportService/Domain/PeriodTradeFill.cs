...
// PositionEffect は集計で**使う**（#892, IADR-0381）: 在庫の増減は Side の符号で判定するが、手仕舞い（Close）が
// 期間で切った在庫で賄えない分は期間より前に建てた建玉の決済として PeriodInventory が見分ける（幻のショートを開かない）。
// 台帳の LedgerFill との整合・監査・将来の両建て（別ロット）会計のためにも保持する（IADR-0018/0025 と同方針）。
