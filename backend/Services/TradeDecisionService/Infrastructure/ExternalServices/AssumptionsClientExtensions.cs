...
//
// 🔴 本サービスは AssumptionsChanged を購読していない（移送前も購読していない。IADR-0264 決定 4）。
// 版の追随は TTL（既定 5 分）だけで行う。購読を足すならハンドラを本サービスへ置き、Wolverine の
// 発見範囲へ入れること——それは移送ではなく**振る舞いの変更**であり、別 issue で判断する。
//
