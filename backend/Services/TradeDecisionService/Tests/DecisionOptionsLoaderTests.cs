...
// FR-04, IADR-0039, IADR-0212, IADR-0272, #571: Decision:* 構成の読み取りと安全側フォールバックの検証。
// VoteCount の既定＝現行挙動（1 票）を config 経由で壊さないことを保証する。
// EnableScreening は #571（基盤 trade-decision-screening 登録が前提）により既定 true へ反転した
// （IADR-0272。DecisionOrchestrationOptions.Default 自体は不変であり、ローダーの構成既定だけが変わる）。
public class DecisionOptionsLoaderTests
{
    ...
    // IADR-0272: 新既定（true）を構成で明示的に打ち消せること（fail-safe な上書き経路の否定形テスト）。
    [Fact]
    ...
    {
        // bool.TryParse が失敗する値（非 true/false）は新既定（true）のまま倒れる（IADR-0272）。
