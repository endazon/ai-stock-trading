...
// 母集団の実測（develop 3d9b91c2）: roots=1 handlers=0 fields=1。下限はその半分に置く。
public class CompositionWiringGuardTests(ITestOutputHelper output)
{
    ...
    {
        using var factory = new OpendAuthWebApplicationFactory();

        var report = factory.InspectComposition(typeof(CompositionWiringGuardTests).Assembly);

        output.WriteLine(report.Describe("OpendAuthGateway"));
        report.AssertPopulation(minRoots: 1, minFields: 1, minHandlers: 0);
