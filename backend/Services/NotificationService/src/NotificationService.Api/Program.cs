...
// 購読するのは NotificationHandlers.cs のハンドラ群（取引実行・拒否・損切り／前提条件変更 FR-17／報告書の
// 確定・提示 FR-06/07/09・IADR-0116／費用しきい値 NFR／撤退基準到達 FR-20／日報未確定 #210／建玉乖離
// #292・IADR-0118／強制買戻し・維持率縮小／為替の情報源 #381・IADR-0196・IADR-0198）。
// **Wolverine ではハンドラの発見漏れ＝静かな未通知**になる（明示登録が無いため「登録し忘れ」は
// ハンドラのアセンブリを渡し忘れる形で起こる）。**宣言したハンドラが 1 つ残らず発見されること**は
// `NotificationConsumerCoverageTests` が固定する。
//
// 🔴 **ここに種類数を書かない。** 以前は「10 種」と書いていたが、実際には 16 種まで増えており
