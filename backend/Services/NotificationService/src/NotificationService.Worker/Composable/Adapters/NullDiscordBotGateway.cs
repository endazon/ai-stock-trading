...
// FR-14, IADR-0063 決定1: Bot の安全既定＝Gateway に接続しない no-op（IADR-0020 の LoggingNotificationSender と同型）。
...
internal sealed class NullDiscordBotGateway(ILogger<NullDiscordBotGateway> logger) : IDiscordBotGateway
{
    ...
    {
        logger.LogInformation("Discord Bot は無効です（Gateway に接続しません・安全既定・IADR-0063）。");
