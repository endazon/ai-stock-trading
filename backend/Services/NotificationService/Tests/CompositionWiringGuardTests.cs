...
// 母集団の実測（develop 3d9b91c2）: roots=16 handlers=32 fields=74。下限はその半分に置く。
public class CompositionWiringGuardTests(ITestOutputHelper output)
{
    ...
    public void 本番の組み立てに配線の抜けが無い()
    {
        ...
        output.WriteLine(report.Describe("NotificationService"));
        report.AssertPopulation(minRoots: 8, minFields: 37, minHandlers: 16);
