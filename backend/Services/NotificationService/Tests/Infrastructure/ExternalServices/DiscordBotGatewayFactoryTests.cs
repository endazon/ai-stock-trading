...
public class DiscordBotGatewayFactoryTests
{
    ...
    private static IDiscordBotGateway Create(DiscordBotOptions options)
    {
        ...
        var policyHandler = new PolicyRevisionCommandHandler(
            new FakePolicyRevisionController(), new FakeWatchlistController(), options, NullLogger<PolicyRevisionCommandHandler>.Instance);
        // #1025, IADR-0433: `/policy` の確認ボタン（確定＋入れ替え案の適用）も同じ Gateway に載る。
        var policyApprovalHandler = new PolicyApprovalCommandHandler(
            reportHandler, new FakePolicyRevisionController(), new FakeWatchlistController(), options,
            NullLogger<PolicyApprovalCommandHandler>.Instance);
        return DiscordBotGatewayFactory.Create(
            options, handler, pauseHandler, stageGateHandler, gfvHandler, reportHandler, driftHandler, policyHandler, policyApprovalHandler,
