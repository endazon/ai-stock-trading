...
//
// 🔴 本テストは IADR-0260（共有カーネルの新設）で新設した。同 ADR で TradingAssumptions ・
// TradingAssumptionsDefaults ・CostCalculator を AiStockTrading.Shared.Kernel へ移送した結果、
// 設定サービスの Domain に残るのは VersionedAssumptions だけになった。**移送した型のテストは
// Shared.Kernel.Tests へそのまま移しており、削除していない。** 残った型のテストが 1 件も無い状態は
// 「テストを消した」と区別が付かないため、残存分の判定をここで明示的に押さえる。
