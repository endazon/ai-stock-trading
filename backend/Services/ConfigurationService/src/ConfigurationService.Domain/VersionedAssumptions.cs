...
// AI 判断）は Version つきで共通参照する（IADR-0021/0064）。消費側が設定サービスの Application 層に依存せず参照できるよう
// Domain に置く（IADR-0064 決定 3）。
public sealed record VersionedAssumptions(TradingAssumptions Assumptions, int Version)
{
    /// <summary>
    /// 未解決を表す番兵バージョン（IADR-0064 決定 5）。設定サービスから一度も取得できず既定値へ倒れたことを示す。
