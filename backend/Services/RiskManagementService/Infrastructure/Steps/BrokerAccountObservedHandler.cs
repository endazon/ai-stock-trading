    ...
    ILogger<BrokerAccountObservedHandler> logger)
{
    public void Handle(BrokerAccountObserved message)
    {
        ...
        else
        {
            ...
            //
            // 🔴 **抑制は置かない。読み出し側（EfCapitalBaselineStore.GetCurrent の平常経路）が無言なのと
            // 非対称なのは意図である。** #897 の教訓は「毎回鳴らすな」ではなく
            // **「正常な見え方で鳴らすな」**である。
            //   - 読み出しは**審査のたび**に起き、その大半は**正常**（供給できている）。鳴らせば本物が埋もれる。
            //   - ここは巡回ごと（既定 5 分）だが、**鳴るのは評価額を取れていない異常のときだけ**である。
            //     正常時は 1 件も出ない。異常が続けば 1 日最大 288 件出るが、**それは「288 回分の取引日が
            //     古い分母で回った」という事実そのもの**であり、間引くと継続していることが読めなくなる。
            // 件数で読むなら指標（`ast.risk.capital_baseline_reads{outcome}`）を見る。ログは事象ごとに残す。
            logger.LogWarning(
