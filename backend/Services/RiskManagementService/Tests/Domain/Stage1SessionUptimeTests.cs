...
public class Stage1SessionUptimeTests
{
    ...
    //
    // 🔴 **本テストは「限界の可視化」ではなく「裁定を守る装置」である**（#407 / IADR-0187）。
    // 利用者裁定 2026-08-07 が「祝日は判別しない。除外しない」「外部カレンダーを用いない」と定めており、
    // **祝日表を足すことは裁定違反**である。本テストはそれを機械的に止める。**消してはならない。**
    //
    ...
    //
    // **いずれも平日であり、実装はこれらを特別扱いしない。これは欠陥ではなく裁定どおりの挙動である**
    // （利用者裁定 2026-08-07・#407 / IADR-0187。「祝日は判別しない。除外しない」）。
    // **本テストは「いつか塞ぐ穴」の記録ではなく、裁定どおりに振る舞っていることの固定である。**
    [Theory]
    ...
    public void 祝日と半日取引日を実装は判別しない__裁定どおり(int year, int month, int day)
    {
        ...
        // 休場日については**過大計上**であり、**裁定はこれを承知のうえで「除外しない」と定めた**
        //（年 2〜3 日／60 営業日。**「昇格が早まる」側であり fail-safe ではない**が、
        // 60 営業日が 57〜58 営業日相当になる程度で、昇格には取引件数と利用者承認も要る）。
        // 半日取引日は逆に**過少計上（安全側）**へ倒れる（2 仮説の両方で 50% を求めるため）。
