...
public class InformationDegradationScreeningTests
{
        ...
        InMemoryInformationDegradationStore? degradationStore = null)
    {
        ...
        var service = new OrderScreeningService(
            ...
            TestShortSellContexts.Unavailable(new FakeClock(Now, new DateOnly(2026, 7, 9))));
        return (service, degradation);
    }

    [Fact]
    public async Task 縮退中の新規建ては発注審査で拒否される()
    {
        var (service, degradation) = Create();
        degradation.MarkDegraded("news");

        var outcome = await service.ScreenAsync(Decision(Intent(PositionEffect.Open)));
    ...
    [Fact]
    public async Task 縮退中でも決済は承認される_否定形()
    {
        // ADR-0020 決定2/決定3・ADR-0009: 限定縮退は手仕舞い・損切りを止めない。
        var (service, degradation) = Create();
        degradation.MarkDegraded("news");

        var outcome = await service.ScreenAsync(Decision(Intent(PositionEffect.Close)));

        outcome.IsApproved.Should().BeTrue();
    }

    [Fact]
    public async Task 回復後の新規建ては承認される_対の肯定形()
    {
        var (service, degradation) = Create();
        degradation.MarkDegraded("news");
    ...
    [Fact]
    public async Task 縮退が無ければ新規建ては承認される_対の肯定形()
    {
        var (service, _) = Create();

        (await service.ScreenAsync(Decision(Intent(PositionEffect.Open)))).IsApproved.Should().BeTrue();
    }
    ...
    [Fact]
    public async Task 再起動直後で現況が不明なら新規建ては拒否される_否定形()
    {
    ...
    [Fact]
    public async Task 現況が不明でも決済は承認される_否定形()
    {
        var (service, _) = Create(new InMemoryInformationDegradationStore(new StubTimeProvider(Now)));

        (await service.ScreenAsync(Decision(Intent(PositionEffect.Close)))).IsApproved.Should().BeTrue();
    }

    // 🔴 **受け入れ基準①。** 遷移イベントを 1 件も受け取らなくても、収集サービスの現況観測だけで
    // 停止が**復元される**（再起動を跨いで統制が戻る）。
    [Fact]
    public async Task 縮退継続中の現況観測だけで新規建ての停止が復元される()
    {
        var store = new InMemoryInformationDegradationStore(new StubTimeProvider(Now));
    ...
    [Fact]
    public async Task 現況観測が健全なら新規建ては承認される_対の肯定形()
    {
        var store = new InMemoryInformationDegradationStore(new StubTimeProvider(Now));
        var (service, _) = Create(store);
