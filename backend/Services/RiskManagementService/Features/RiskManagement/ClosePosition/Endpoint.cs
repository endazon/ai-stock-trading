...
// ---- 建玉の手仕舞い（FR-10, FR-11, UC-06, #292, #847, IADR-0117, IADR-0357）: 利用者のみ（OwnerOnly）。理由必須 ----
...
internal static class ClosePositionEndpoint
{
    public static void MapClosePosition(this IEndpointRouteBuilder owner) =>
            ...
            async (PositionCloseRequest req, PositionCloseService svc, IMessageBus bus, HttpContext http) =>
        {
            ...
            var outcome = svc.Request(
                new PositionCloseCommand(
                    req.Symbol, market, req.Quantity, req.LimitPrice, req.Reason, req.MarketOrder),
            ...
            if (!outcome.Accepted)
            {
                ...
                {
                    PositionCloseRejection.PositionNotFound => Results.NotFound(new { error }),
                    // #847: 成行と指値の同時指定は**要求の作り方**の誤りであり、状態の問題ではない（400）。
                    PositionCloseRejection.ConflictingPriceMode => Results.BadRequest(new { error }),
            ...
            return Results.Accepted(value: new
            {
                ...
                price = intent.Price,
                // #847: 成行なら price は**参照価格**である（送信する注文価格ではない）。応答で区別できるようにする。
                marketOrder = intent.MarketOrder,
                mode = intent.Mode,
            });
        });
    ...
    private static string DescribeRejection(PositionCloseRejection rejection) => rejection switch
    {
        ...
        PositionCloseRejection.PriceUnavailable =>
            "決済価格を決められません（現在値を取得できず limitPrice も指定されていません）。"
            + "limitPrice を指定するか、marketOrder を省略して成行で手仕舞ってください。",
        // #847, IADR-0357: どちらを捨てても利用者の意図と食い違う。
        PositionCloseRejection.ConflictingPriceMode =>
            "marketOrder=true と limitPrice は同時に指定できません（成行に指値はありません）。"
            + "成行なら limitPrice を省き、指値なら marketOrder を省いてください。",
        _ => "決済要求を受理できません。",
    };
}
...
// Quantity 省略＝処理中を除いた残り全量。
// 🔴 FR-10, UC-06, #847, IADR-0357: **LimitPrice 省略＝成行**（既定が変わった。旧: 現在値の指値）。
// MarketOrder は明示の指定で、false なら旧既定（現在値の指値）を選べる。名前が `Market`（市場）ではなく
// `MarketOrder` なのは、本要求が既に `Market`（日本 / 米国）を持っているためである（JSON も `marketOrder`）。
