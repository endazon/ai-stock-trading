...
public static class RiskEvaluator
{
        ...
        BuyInBanSupply? buyInBan = null)
    {
        ...
        // FR-01, FR-02, FR-10, ADR-0020 決定2/決定3, #337, #564, IADR-0249, IADR-0267: 情報収集の縮退（限定縮退）中は
        // 新規建てを止める。欠測した文脈・単一ソース由来の急シグナルでの新規建てを、LLM の自制（KB の
        // 欠測文言）ではなく決定的コードで止める（ADR-0003 の最終防衛線と同じ位置）。
        // 🔴 **この 1 ビットは「縮退している」だけでなく「縮退しているか分からない」でも立つ**（#564）。
        // 収集の現況観測が届いていない／失効した状態は**不明**であり、**不明は止める側へ倒す**
        // （供給の不在で統制が黙って外れないようにする。口座種別の未確認〔上〕と同じ向き）。
