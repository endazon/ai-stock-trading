...
// ［2026-09-25 追記 / #967］本文脈を組む供給元は ShortSellContextSupplier（IADR-0425）。借株可否は発注執行の照会、
// エクスポージャは台帳と当日の未約定の新規建てから組み、どちらかが分からなければ null を返す（偽の文脈を組まない）。
// 権利確定日（コーポレートアクション）と借株料（単位未確定）の供給元は無く、いずれも null で渡る。
