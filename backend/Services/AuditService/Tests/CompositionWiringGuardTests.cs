...
// 母集団の実測（develop 3d9b91c2）: roots=4 handlers=52 fields=106。下限はその半分に置く。
public class CompositionWiringGuardTests(ITestOutputHelper output)
{
    ...
    {
        using var factory = new AuditWorkerWebApplicationFactory();

        var report = factory.InspectComposition(typeof(CompositionWiringGuardTests).Assembly);

        output.WriteLine(report.Describe("AuditService"));
        report.AssertPopulation(minRoots: 2, minFields: 53, minHandlers: 26);
