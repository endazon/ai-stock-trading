...
//
// #132 / IADR-0059: 本番切替に向けて接続パラメータを外部化した（既定値は現行挙動と同一）。
internal sealed record MoomooBrokerOptions(string OpenDHost, ushort OpenDPort, string? RsaPrivateKeyPath = null)
{
    ...
    // #132, IADR-0059 決定 5: config で実弾（TrdEnv_Real）を要求されたら「黙って SIMULATE で流す」のではなく停止する。
...
// #132, IADR-0059 決定 5: moomoo 発注経路の起動時 preflight。本番切替で踏みやすい構成ミスを、
