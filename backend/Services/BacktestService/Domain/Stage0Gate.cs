...
//
// 先頭 7 つが判定器（Stage0GateEvaluator）の 7 条件である。末尾 7 つは**駆動側（Hosted の定時評価）の事前条件**であり、
// 判定器は決して出さない（#688, IADR-0310 決定3・#632, IADR-0318・#749, IADR-0387）。同じ enum に同居させるのは、
// 未達理由の文字列表現を Stage0GateResult.FormatFailedChecks() の単一情報源に保つためである
// （2 系統に分けると区切り文字がドリフトする）。
//
// 🔴 **判定器の 7 条件は #749 でも増えていない。** ADR-0036 決定1 の遮断（申告なし・全件除外）は
// **事前条件として置く** —— 合否の条件を足すのではなく、**合否を組める前提が無いことを報告する**。
public enum Stage0GateCheck
{
    ...
    /// <summary>
    /// FR-04, FR-15, ADR-0036 決定1, #749, IADR-0387: 記録が **as-of 入力の再構成可否を申告していない**
    /// （未申告・3 種を覆わない部分申告）。何を判定母集団から外すべきかが読めないため判定を走らせない。
    /// <para>
    /// 🔴 **「申告が無い＝痩せていない」ではない。** 申告の無い記録を通すと、痩せた入力での結果が
    /// そのまま Stage 0 の合格根拠になり得る（ADR-0036 決定1 がまさに禁じたこと）。
    /// </para>
    /// </summary>
    InputCompletenessNotDeclared,

    /// <summary>
    /// FR-04, FR-15, ADR-0036 決定1, #749, IADR-0387: 再構成不可に依存する判断を外した結果、
    /// **判定母集団が 1 件も残らなかった**。
    /// <para>
    /// 🔴 同決定「**外した結果 Stage 0 の対象が実質的に成立しなくなった場合は、合格としない。範囲を狭めて
    /// 通すのではなく、通らないことを報告する**」の実装である。無発注の走行は成績が動かないため、
    /// 判定器へ通すと「損失が無い」ように見え得る —— 判定の手前で断つ。
    /// </para>
    /// </summary>
