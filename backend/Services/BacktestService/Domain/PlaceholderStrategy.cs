...
// 記録・再生方式で評価すると決めた。記録器・記録再生戦略はまだ無く、**評価できる本番戦略が存在しない**。
// 同 ADR の「統制と現在の実現手段」表は、その暫定手段として #688 を名指しで
// 「プレースホルダ戦略での動作確認に留める」と定めている。本型はその「プレースホルダ戦略」である。
//
...
//
// 本番戦略（記録・再生）が載ったら、駆動側の差し替え先は本型だけである（publish 経路は変えなくてよい）。
