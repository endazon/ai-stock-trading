...
// 母集団の実測（develop 3d9b91c2）: roots=4 handlers=0 fields=5。下限はその半分に置く。
public class CompositionWiringGuardTests(ITestOutputHelper output)
{
    ...
    public void 本番の組み立てに配線の抜けが無い()
    {
        ...
        output.WriteLine(report.Describe("BacktestService"));
        report.AssertPopulation(minRoots: 2, minFields: 2, minHandlers: 0);
