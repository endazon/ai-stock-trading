...
//   1. **評価対象がプレースホルダのことがある**（構成 `Backtest:Stage0:Strategy` の既定。ADR-0033 は #688 を
//      名指しで「プレースホルダ戦略での動作確認に留める」と定めた）。
...
//
// 実際の合否判定（Stage0GateService）は #632 / IADR-0318 で駆動へ配線済みである（`Strategy=recorded-replay`
// かつ記録が構成と整合するとき）。本型の役割は**評価できない場合の不合格**に残る —— どの入口からも
// 合格は作れず、合格を出せるのは Stage0GateService の 7 条件だけである。
public static class Stage0DriverVerdict
{
    ...
    /// （ADR-0033 決定3 は汚染対策をカットオフ後データに限る。値は ADR-0037 決定2 が計画へ登録済みで、実装は構成から受け取る。
