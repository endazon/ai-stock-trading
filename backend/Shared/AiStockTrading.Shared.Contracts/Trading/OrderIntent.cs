...
// （IADR-0107 決定1。moomoo アダプタは本値をそのまま注文価格に用いるため、円換算値を入れてはならない）。
...
// FR-10, FR-17, #257, IADR-0107: FxRateToBase は「ローカル通貨 1 単位あたりの基準通貨（円）額」。取引判断が
    ...
    decimal FxRateToBase = 1m)
{
    ...
    /// <summary>基準通貨（円）建ての概算約定金額。リスク統制の金額判定はこちらを用いる（IADR-0107）。</summary>
