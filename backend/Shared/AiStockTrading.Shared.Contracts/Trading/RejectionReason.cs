...
public enum RejectionReason
{
    ...
    //
    // **3 種とも計画が新設を明示している**（ADR-0021 決定4-5・**2026-08-07 改訂**）。
    ...
    //
    // 畳まない理由は ADR-0016 決定10 の規律（原因も解除条件も異なるものを畳むと監査ログ〔FR-11〕が実態と
    // 食い違う）であり、**改訂後の計画も同じ理由で 3 種を分けている**——解除条件は「T+1 の決済」
    // 「違反記録の失効（90 日以上先）」「照会の成功（数分以内）」であり、いずれも互いに異なる。
    //
    // クラス分類は **A / B / A**（`CashAccountSettlementHold` = A、`BrokerAccountTypeUnverified` = **B**、
    // `GoodFaithViolationLimitReached` = A）であり、計画表と一致している。序数は 25 / 26 / 27 で不変である。
    ...
    /// <summary>
    /// FR-10, #869, ADR-0041 決定2, IADR-0354: **統制上限の基準資金（equity）を照会できていない。**
    /// <para>
    /// 計画は「判定に用いる equity は前営業日終値時点の USD 評価額」と定め、その**供給元をブローカーの
    /// 口座照会に確定した**（ADR-0041 決定2）。照会できないとき（観測が 1 件も無い・鮮度が切れた・
    /// 応答に評価額が無い）は**新規建てを拒否する**（fail-closed）。
    /// </para>
    /// <para>
    /// **新しい規律ではない**——ADR-0016 決定3「借株料を事前照会できない場合は空売り自体を行わない」・
    /// ADR-0028「未供給時の fail-closed」と同じ形である。分母が分からないまま比率の上限（1 注文 25% 等）を
    /// 判定すると、統制が掛かっているように見えて実際には何も制限していない状態になる。
    /// </para>
    /// <para>
    /// **手仕舞い（Close）・損切りは止めない**（FR-10 の不変条件・ADR-0009）。
    /// 照会できないことを理由に建玉を抱え続けさせるのは統制ではなく事故である。
    /// </para>
    /// <para>クラス分類は**クラス B**である（取引を止めている状態そのものの記録。
    /// <see cref="BrokerAccountTypeUnverified"/> と同じ区分）。</para>
    /// <para>**序数は 29（末尾追加）で不変**（IADR-0134 決定2）。</para>
    /// </summary>
    CapitalBaselineUnavailable,
}
