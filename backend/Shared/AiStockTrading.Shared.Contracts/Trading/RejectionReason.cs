...
public enum RejectionReason
{
    ...
    /// <summary>
    /// FR-10, #935, IADR-0394: <b>その取引日のうちに損切りで手仕舞った銘柄へ、同じ方向の新規建てをしようとした。</b>
    /// <para>
    /// 「損切り」は S1（ソフトウェア逆指値の成行決済の発動）と S0（ブローカー側逆指値の約定）だけを数える。
    /// owner の手仕舞い・判断由来の決済・維持率割れの自動縮小・保護喪失の成行手仕舞いは数えない（IADR-0394 決定1）。
    /// 区切りは<b>市場の現地取引日</b>（米国株は米国東部の暦日。<c>TradingDay.Of</c>）。
    /// </para>
    /// <para>
    /// 🔴 <b><see cref="SameDayReentry"/>（差金決済防止）とは別の理由である。</b> 後者は制度・決済の制約
    /// （日本株現物・米国の現金口座）で「当日に売買したすべての銘柄」を止め、信用口座の米国株には掛からない。
    /// 本理由は口座種別に依存せず、「損切りした」事実だけを入力にする。
    /// </para>
    /// <para><b>手仕舞い（Close）は止めない。</b>クラス分類は<b>クラス A</b>（統制が設計どおり作動した記録）。</para>
    /// <para>**序数は 30（末尾追加）で不変**（IADR-0134 決定2）。</para>
    /// </summary>
    StoppedOutSameDay,

    /// <summary>
    /// FR-10, #935, IADR-0394: <b>その銘柄を当日に損切りで手仕舞ったかどうかを確かめられない</b>ため、
    /// 同じ方向の新規建てを止めた。
    /// <para>
    /// 当日の決済の承認行に<b>由来が記録されていない</b>（由来の列を足す前に記録された行）とき、その決済が
    /// 損切りかどうかは分からない。🔴 <b>「分からない」を「損切りではない」として扱わない</b>（#865 / #877 と同じ向き）。
    /// </para>
    /// <para><b>手仕舞い（Close）は止めない。</b>クラス分類は<b>クラス B</b>（取引を止めている状態そのものの記録。
    /// <see cref="CapitalBaselineUnavailable"/> と同じ区分）。</para>
