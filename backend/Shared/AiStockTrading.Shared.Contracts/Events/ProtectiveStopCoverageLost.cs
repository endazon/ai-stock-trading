...
public enum ProtectiveStopRemediation
{
    ...
    /// <summary>
    /// 🔴 FR-10, #1013, IADR-0428（2026-09-26 追記）: <b>ブローカーの建玉が 0 に見えるが、エントリー注文の状態を確認できない</b>
    /// （まだ約定していないのか、建って消えたのか分からない）ため、常駐ガードが<b>保護逆指値を取り消さず、保護記録も閉じずに据え置いている</b>。
    /// 取消も成行も再発注もしていない。
    /// <para>
    /// 据え置く理由: 未約定のエントリーを「建玉が消えた」と読んで逆指値を取り消すと、約定後の建玉が無保護で残る。逆に建玉が本当に消えていて
    /// 逆指値が生きていれば、発火で反対方向の建玉を生む——<b>どちらかを人が証券会社の画面で確かめる</b>（エントリー注文・建玉・逆指値）。
    /// </para>
    /// <para>
    /// <c>CloseDecisionId</c> / <c>CloseIntent</c> は運ばない（何も送っていない・台帳は押さえない）。<c>Cause</c> は常に
    /// <see cref="ProtectiveStopLossCause.LapsedInFlight"/>（発行元が常駐ガードであることだけを表す）。
    /// 状態が分かるまで約 1 時間ごと（と再起動のたび）に再発行される（人の確認を要する・Warning）。
    /// </para>
    /// 🔴 列挙の**末尾へ足している**（既存値の序数を動かさない）。
    /// </summary>
    EntryStateUnknown,
