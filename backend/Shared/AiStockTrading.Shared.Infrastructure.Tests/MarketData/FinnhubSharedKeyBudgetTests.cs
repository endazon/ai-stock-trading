...
// FR-01, IADR-0272: 実クラスタでの実測（60 回/60 秒固定ウィンドウ）と、ローカル実行環境（values-local.yaml）で
...
// 責務でしか防げない。IADR-0272 決定3 が指摘したとおり、市況の消費サービスは実装上 4 つ
...
public class FinnhubSharedKeyBudgetTests
{
    ...
    // IADR-0272 実測: Finnhub Free `/quote` は 60 回/60 秒の固定ウィンドウ（ローリングではない）。
