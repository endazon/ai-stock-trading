...
// 「常に 0 行更新 → 常に競合例外 → 終端が一度も記録されない」という、
// **在庫が永久に解放されない**（#848 の実害の恒久版）最悪の壊れ方になる。
//
...
public sealed class ApprovedOrderTerminalConcurrencyE2ETests : IAsyncLifetime
{
    ...
    // 対（肯定形・最重要の裏返し）: **勝者はちゃんと書けている。**
    // 述語が壊れて「常に 0 行更新」になると、上の否定形だけなら緑のまま通ってしまう
    // ——そのとき起きるのは「終端が一度も記録されず在庫が永久に解放されない」という最悪の壊れ方である。
    [Fact]
