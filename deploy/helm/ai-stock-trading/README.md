...
| `DISCORD_OWNERAUTH_CLIENTID` | `discord-owner-auth-client-id` | notification・report・risk-management・market-monitor | dev 既定 `ai-stock-trading-owner`（Bot 制御コマンドの OwnerAuth・#226 / IADR-0098）。**report も同じ鍵を `Reports__DelegatedActor__TrustedClientIds` として読む**（#774 / IADR-0240 決定11: Bot が確定要求で運ぶ「代理される利用者」を信じてよいクライアント。空=誰も信じない）。risk-management は `RiskControls__DelegatedActor__TrustedClientIds`（#868）、market-monitor は `Monitor__DelegatedActor__TrustedClientIds`（#1025 / IADR-0433: `/policy` の入れ替え案の適用の変更者）として同じ鍵を読む |
