...
- 報告書サービスの応答の方針・理由・説明は、メンション構文と Discord のマスクリンク `[表示](URL)` を崩して返す（幅ゼロ空白の挿入。
  方針は切り詰めない）。素の URL は URL のまま見えるため崩さない。保存する方針は検証済みの原文のまま。
...
## 受け入れ基準 → テスト（T-10-1300〜T-10-1349 を予約。使用は T-10-1300〜1345）

| # | 基準 | テスト |
| --- | --- | --- |
| 1 | 出力スキーマの検証（正常・各違反で案全体を捨てる・フェンス付き JSON を読む） | `PolicyRevisionProposalParserTests`（T-10-1300〜1305） |
...
| 4 | 改訂の表（既存の改訂・新規・確定済み・土台なし・効かない・LLM 失敗で保存しない・本文の追記・提示・**営業日の自動生成前は作らない**・**LLM を待つ間の更新で保存しない**・**保存後の提示の失敗は保存済み未提示**） | `ReportPolicyRevisionServiceTests`（T-10-1311〜1318・1336〜1340） |
...
| 6 | Discord ハンドラ（認可・解析・指示の検証・確認ボタンの版・失敗の表示・**書式外の period**・**ボタンが出るとき方針の全文が届いている**） | `PolicyRevisionCommandHandlerTests`（T-10-1322〜1328・1344・1345） |
| 7 | 越境の契約（送り手の本物の型で要求・応答を直列化）と失敗・不明の区別 | `HttpPolicyRevisionControllerTests`（通知・extern alias `ReportWorker`。T-10-1329〜1333） |
