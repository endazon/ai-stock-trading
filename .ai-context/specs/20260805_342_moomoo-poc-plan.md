...
>
> **ADR-0019 決定 1 は 7 項目**である。項目 7（米国株の日足 OHLC 履歴を API から取得できるか、およびその品質）は **2026-08-04 の追補で正式に追加**されており（ADR-0023（計画リポ） 決定 3 が「ADR-0019 決定 1 に項目 7 を追加した」と明記している）、補助項目ではない。
>
...
> ⚠️ **`JPN Stocks: No permission`**。計画（03_moomoo-integration（計画リポ））は日本株の市況取得・現物発注が 2026-06 から可能と記載しているが、**現在のアカウントには日本株の市況権限が無い**。発注権限とは別建てである可能性があるため断定はしないが、日本株の市況を要する機能（FR-02 の価格変動検知等）は現状動かない。**別途の確認が要る。**
...
> 実測で裏付けられた。ADR-0016（計画リポ） 決定 14 の
...
**ADR-0023（計画リポ）（Stooq は取得不能・回避実装を禁止）が残した「では何を使うのか」に対して、moomoo が答えになる。** FR-15 のバックテストに流せる日足が実在し、20 年分に届く。**#382 の解決策として環流する。**
...
**`ImShortRatio=50` / `MmShortRatio=30` は ADR-0016（計画リポ） 決定 14 が記載する実弾の規制値（ショート 初期 50% / 維持 30%）と完全に一致する。** 出所が計画書に無かった値の裏取りになる。あわせて、SIMULATE 側で実測した初期証拠金 60%（訂正 1）も決定 14 の記述と一致しており、**決定 14 の「SIMULATE だけが保守側の外れ値になる」という指摘が実測で裏付けられた**。
...
> **ADR-0016（計画リポ） 決定 3 の「借株料が年率 20% を超える銘柄への空売りは拒否する」は、この情報源では恒久的に発火しない統制になる。**
...
そもそも `TrdGetMarginRatio` が SIMULATE で使えない以上、**決定 7（維持率 40%）の検証は Stage 1 では成立しない**。これは ADR-0016（計画リポ） 決定 14 が
