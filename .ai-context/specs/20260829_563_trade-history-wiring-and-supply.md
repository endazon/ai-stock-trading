...
`backend/Services/ReportService/Domain/` の描画コードから、未供給を表す表示文字列を全数抽出した（22 件）。
形は **2 系統**しかない。

| 系統 | 形 | 使用箇所 |
| --- | --- | --- |
| **節レベル**（節の本文 1 行） | `- **<対象>を照会できませんでした（供給元がありません）**: 「<0/なし の言い換え>」とは区別しています。` | 強制買戻し / 借株コスト / OpenD 稼働率 / 三者比較 / LLM 利用実績（5 件）。ほかに「（要確認）」版が 2 件（維持率割れ・為替情報源） |
| **セルレベル**（表の 1 セル） | `**供給されていません**（<0 の言い換え> ではありません）` | 日報 §1 サマリ表の 4 行（為替差損益 / OpenD 稼働率 / スキップ回数 / 分割・切り詰め） |
