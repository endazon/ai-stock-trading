...
### 着手時に見つけた既存の穴（本 PR の範囲外・記録のみ）

- **`TradeHistoryRenderer` は実装されているが `ReportRenderer` から呼ばれていない**（実測: 本番コードからの
  参照 0 件・テストからのみ）。したがって日報テンプレート §2「取引履歴（全明細）＋取引詳細」と
  §「見送り判断」は**報告書本文に一度も現れていない**。合成に要る `TradeHistoryView`（銘柄選定の理由・
  売買判断の理由・参照した情報・想定シナリオ）の供給が存在しないため、本 PR では合成しない。
  **供給が無いまま合成すると、空の表と空のブロックが並ぶだけである**（節を出す価値がある未供給表記とは
  性質が違う——あちらは「照会できていない統制の事実」を伝えるが、こちらは判断根拠そのものが無い）。
- 日報テンプレート §3「ポジション一覧」（方向・借株料累計・損切りライン）も同じ理由で未実装である。
  **借株料は §4 の「空売りの記録」として本 PR で出す**が、建玉単位の一覧は建玉の供給が要る。
- いずれも #338 のスコープ（拡張された記載事項の反映）ではなく、**別 issue で起票すべき残件**である。
