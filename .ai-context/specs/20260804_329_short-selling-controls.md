...
- 計画書リンク: 02_requirements FR-10（計画リポ） ／
...
[テスト仕様書 FR-10](../../docs/tests/FR-10_risk-controls-tests.md)。**否定形は「拒否されること」ではなく
...
- 差異: **あり**（2 件・いずれも計画へ環流済み）
  1. **`StopOrderRequired`（拒否理由 8 種目）**: FR-10 本文と ADR-0016 決定2(b) は逆指値の同時発注を
     ...
     計画への環流: feedback/20260804_adr0016-stop-order-rejection-reason.md（環流記録）
...
3. **空売り比率 50% の分母**: 計画の文言（「空売り建玉の合計は、建玉総額の 50% を超えない」）を
   ...
   → **第 3 段階で計画へ環流した**（feedback/20260804_adr0016-short-ratio-denominator.md（環流記録）。
   等価形 `空売り建玉 ≦ ロング建玉総額`・5 つの含意・3 つの選択肢と代償）。等価形は
   プロパティテスト T-10-156 で機械的に固定した。裁定待ちであり実装は現行（文字どおり）のまま。
