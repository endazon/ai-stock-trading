...
［2026-09-24 追記 / PR #929 監査］ 監査（条件付き GO）の是正で T-10-724〜T-10-728 を足した（T-10-729 は未使用）。

| ID | 対象 |
| --- | --- |
| T-10-724 | 実 DI 構成の `IMarketSchedule` が `MarketHoursSchedule` で、2026-09-24T14:00Z（米国）を開場・2026-11-26（感謝祭）を閉場と読む（監査 F1） |
| T-10-725 | `Monitor:Holidays` は ISO の `yyyy-MM-dd` だけを受け、`"10/09/2026"` を月先で読まない |
| T-10-726 | 保有を知らない閉場でも「閉場と判定しています・次の開場」を閉場期間ごとに市場ごと 1 回出す（監査 F3） |
| T-10-727 | 閉場中に起動した最初の巡回で 1 回出し、開場を挟んだ次の閉場でまた 1 回出す（開場の巡回が印を解く配線） |
| T-10-728 | `TradeCycle:Holidays` も ISO の `yyyy-MM-dd` だけを受ける（取引判断の実 DI 構成） |
...
- 規則計算の誤り（臨時休場・規則改定）は「開場と読んで終値で回す」側ではなく「閉場と読んで保護が止まる」側へも倒れ得る。
  **閉場と読んだ最初の巡回は必ずログに出る**ため、誤りは無音にならない。
  ［2026-09-24 追記 / PR #929 監査］ **この「必ずログに出る」は誤りだった。** 報告は自分が観測した保有だけで、閉場のたびに
  その市場の観測状態を捨てるため、誤って閉場と読んだ日も閉場中の再起動も 1 行も出なかった。是正と残余リスク
  （時計のずれ・臨時休場日の日付表記）は IADR-0380「結果・残余リスク」の同日の追記に書いた。
- 再起動が閉場中に起きると、その閉場期間の保護の空白は出せない（保有を照会していないため何も知らない）。
  ［2026-09-24 追記 / PR #929 監査］ 保有の報告は出せないままだが、「閉場と判定しています・次の開場」の Information は
  起動後の最初の巡回で出るようにした（T-10-726 / T-10-727）。
