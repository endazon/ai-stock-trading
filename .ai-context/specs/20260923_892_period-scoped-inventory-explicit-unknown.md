...
- **散文の LLM プロンプト**: 実現損益・源泉徴収税額・評価損益・決済件数・勝ち件数を `算出不能（N件）` に置き換え、
  言及しないよう指示する（本文・要約が「算出不能」と描くのに散文だけが部分値を権威として受け取らない）。
- **週報 §5 の源泉徴収税額**: §1 と同じセル（`算出不能`）にする（実現損益〔部分値〕から算出されるため）。
...
| `Features/Reports/ReportNarrativePromptBuilder.cs` | 散文プロンプトへ部分値を渡さず、言及しないよう指示する |
...
**テスト ID 帯**: `T-16-001`〜 を本作業で新設する（`T-16-016`〜`023` は監査の指摘で足した）。走査（`grep -rnoE "\bT-16-[0-9]+\b"`）の結果、
...
| T-16-016 | 散文プロンプトが部分値を渡さず `算出不能（N件）` と言及しない指示を載せる（対: 0 件なら従来どおり） |
| T-16-017 | 週報 §5 の源泉徴収税額が `**算出不能**` になる（対: 0 件なら税額を出す） |
| T-16-018 | 週報 §5 の費用率が `**算出不能**` になり、部分値の分母から計算した比率を出さない |
| T-16-019 | 自動生成が `OpeningInventory` を保存された報告書の未供給入力へ**実際に記録する**（T-16-013 は語彙だけ） |
| T-16-020 | Discord 要約が実現損益・決済件数・勝ち件数を数字として出さない（費用は出す） |
| T-16-021 | **一部だけ賄えた決済**（`Realizing` かつ `Unvalued`）が部分値でハイライト・寄与最大に並ばない |
| T-16-022 | 三者比較の算入できなかった件数が到達済みの列だけを合算し、月報 §5 に出る |
| T-16-023 | 週報 §2・§3（決済の有無の両経路）と月報 §2 が算入しなかった件数を注記する（対: 0 件なら出さない） |
