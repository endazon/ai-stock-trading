...
- **その日の通常取引時間（分母）**: 半日取引日カレンダーの判定源は**計画側の裁定待ち**
  （[docs/blocked-tasks.md](../../docs/blocked-tasks.md) B-4・環流記録 `feedback/20260804_fr20-stage1-session-calendar.md`）。
...
- 対応: [docs/blocked-tasks.md](../../docs/blocked-tasks.md) の B-4 と「実装済みだが実際には発動しない機能」の表を更新し、
...
- [ ] **未達（意図的に残す）**: issue #385 の「市場休場が除外される」は**週末のみ**満たし、**祝日は満たさない**。
      ...
      環流記録 20260805_fr20-stage1-market-holiday-exclusion（環流記録）・
      [docs/blocked-tasks.md](../../docs/blocked-tasks.md) B-4
