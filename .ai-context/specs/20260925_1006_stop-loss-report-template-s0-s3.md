...
| h | 食い違いの理由（空売り・未知） | `空売りの新規建ては S0 で扱う` / `未知の手法の値のため S0 と同じ扱いにした` | 空売りの新規建て・未知の値 | 一致（語を含む。変えない） |
...
5. 解決結果はあるがすべて見送りだった月は `当月の損切りの実行機構: S0〜S3 のいずれも適用されませんでした（解決結果はすべて見送り）` と書く（空の内訳を出さない）。
