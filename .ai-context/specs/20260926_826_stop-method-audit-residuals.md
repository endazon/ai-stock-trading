...
- `docs/functional/FR-10_risk-controls.md`:
  ...
  - S1 の表の「同じ銘柄に S0 と S1 の建玉が併存」の直後に「同じ銘柄に S0 と S2 の建玉が併存」の行を足す（差し引けない・起きること・今は起きない理由・
    決まるまでの暫定の運用〔同じ銘柄に一方の手法の建玉が残っている間は S0 と S2 の間で切り替えて新規建てを出さない〕）。
...
| ID | 内容 | 置き場所 |
| --- | --- | --- |
| T-10-1575 | 既知の制約: 同じ銘柄・方向に S2 の建玉があると、S0 の行から見た建玉残に S2 の数量が入る（S1 の行なら差し引く＝対照）。`ProtectiveStopNetting.RemainingPositionFor` の水準で固定する（ガードはこの値が 0 より大きい間、S0 の逆指値を取り消さない） | `OrderExecutionService.Tests`（新しいファイル） |
...
## 監査の指摘への対応（2026-09-26・フレッシュな文脈の監査）

- FR-10 の文が S3 を落としていた（`ProtectiveStopNetting` は S0 以外の**すべての**手法の有効な記録を差し引く。S3 の行は数量全体を主張する）→ 文を直した。
- 監査の読み（未実行）: 同じ銘柄に S0 と S3 の建玉が併存し一方を手で決済すると、両方の行が相手の主張を差し引いて建玉残 0 と読み、同じ巡回で
  どちらの逆指値も取り消し得る。既存の挙動で、今は到達しない（SIMULATE では S0 が拒否され、実弾は S0 だけ）。**問い Q2 に併せて PR 本文へ載せた**（決めない）。
- 暫定の運用を両方向（S0↔S2）にし、「決まるまでの暫定」と明記した。到達性の書き方（「手法を途中で切り替えた場合にだけ」）を改めた。
