...
| 49 | **帰属不明の建玉をガードが検知し、同じ状態では鳴らさずに 1 回だけ知らせる**（🔴 **走るのは有効な保護記録が 1 件以上ある巡回だけ**。受理後に 0 約定で取り消された決済の残りは、他に有効な記録が無ければ走らない＝12 巡目 BLK-12-2 で是正した射程） | `帰属不明の建玉をガードが検知して同じ状態では一度だけ知らせる`（T-10-486・用量反応）／`受理後に取り消された決済の残りは行が完了していても検知される`（T-10-487・`[Theory]` で取消／未約定の用量反応） |
...
## ［2026-09-19 追記（7） / #820 の 11 巡目監査］武装の判定の順序と、検知の射程の開示

11 巡目の監査は**無作為 3,000 試行の掃きで超過売り 0 件・最大超過 0 株**を実測した。
ブロッキング 2 件のうち **1 件は追記(6) の是正 2 が作った競合**である。

### 是正（IADR-0344 追記(10)）

1. 🔴 **`ConfirmEntryFills` を `GetPositionsAsync` の「前」へ移す**（BLK-11-1）。
   追記(6) の是正 2 は確定を**照会の後**に置いたため、RPC の待ちのあいだに `OrderFillPollingService` が
   先行エントリーを終端化すると **`claimed` だけが新しく `net` は古い**——帰属不明が過少に読まれて武装した
   （監査の `PROBE4`: `Forgone=なし` / `Entries=1`）。確定を先に置けば**安全側（見送り）へ倒れる**。
   **固定は T-10-506。**
2. **`docs/functional/FR-10_risk-controls.md` の検知の行から「受理後に 0 約定で取り消された決済の残り」を落とし、
   「検知が走らない配置」を独立して開示する**（BLK-11-2）。**その配置ではまさに検知が走らない。**
3. **NB-2 の誤った根拠を訂正し、追随を #880 として起票した**（`BrokerPositionSnapshotService` への相乗り案つき）。
4. **NB-1・NB-3 を「残る制約」へ記録した**（本 PR では直さない）。

### 🔴 追記(6) の是正 2 が新たな欠陥を作ったことについて

追記(6) の是正 2 は「終端済みエントリーの確定待ちの窓を消す」ことが目的で、**2 文の順序までは見ていなかった**。
`ProtectiveStopGuard` と**逆の順序**になっており、しかも**発注側だけが危険な向き**だった。

**同型の再発を防ぐ読み方**: 「確定（ローカルな突き合わせ）」と「照会（RPC）」を並べるときは、
**RPC の待ちのあいだに前者の入力が変わり得る**ことを前提に、**取り違えが安全側へ倒れる順序**を選ぶ。
S1 では「帰属不明を過大に読む」が安全側である。

### 母集合（是正の走査・2026-09-19。**新たに書いた文の側の語**で引く＝規則 10）

10 巡目の是正 5 は「文書の事実誤認の訂正」だったのに、**その是正で追加した文へ同じ種類の誤りが入った**。
是正前の語で引いても捕まらないため、**今回書いた文の語**で引き直した。

| 検索語（今回書いた文の側） | 生の件数 | 扱い |
| --- | --- | --- |
| `常駐ガードが毎巡回検知` | 1 行 / 1 ファイル | `docs/functional/FR-10_risk-controls.md` 849（**是正**。「毎巡回」を外し、走る条件を明記） |
| `帰属不明の検知」がこの状態を知らせる` | 0 行（リポジトリ内） | **PR 本文にのみ存在**（リポジトリ外のため走査に掛からない。**PR 本文を直した**——走査だけでは捕まらない面がある） |
| `対象ゼロでも建玉を照会する` | 1 行 / 1 ファイル | IADR-0344 追記(9) の残る制約（**インラインで訂正**） |
| `ガードが毎巡回` / `毎巡回これを持っている` | 5 行 / 5 ファイル | 🔴 **［2026-09-19 追記（8） / 12 巡目監査 NB-12-2］この分類の理由は事実と違っていた。** `ProtectiveStopNetting.cs` 470・`ProtectiveStopGuard.cs` 131・テスト 888 の 3 行は確かに「**材料を毎巡回持っている**」（動機の記述）だが、**本仕様書 621 と IADR-0344 689 は文字どおり「ガードが毎巡回『帰属不明の建玉』を検知して」と書いている**。**凍結記録なので本文は書き換えず残す判断は変えない**が、理由は「誤りではない」ではなく「**誤りだが凍結記録であり、訂正は追記側で行う**」である（訂正は IADR-0344 追記(11) と本追記）。ガードの注記には走らない条件を足した |
| `巡回の対象に 1 件も残らない` | 2 行 / 2 ファイル | `docs/` 849（**是正**）・IADR-0344 の残る制約（**正しい文脈**。ここでは「だから検知が走らない」と書いてある） |

**除外とその理由**: `CHANGELOG.md`（生成物）／`.ai-context/` の他の凍結記録（当時の記述。本仕様書と IADR-0344 だけを
日付つき追記で訂正する）／フロントエンド・`deploy/`（挙動を変えていない）。

### 受け入れ基準の追加

| # | 受け入れ基準 | テスト |
| --- | --- | --- |
| 56 | **建玉照会の最中に先行エントリーの約定が確定しても、武装の判定は安全側（見送り）へ倒れる** | `SoftwareStopBlockingRegressionTests.建玉照会の最中に約定が確定しても武装の判定は安全側へ倒れる`（T-10-506） |

### 赤→緑の実測

`ConfirmEntryFills` と `GetPositionsAsync` の 2 文を是正前の順序（照会 → 確定）へ戻して T-10-506 を走らせた
（出力は PR 本文と報告に貼った）。**最初に書いたテストは是正前でも緑だった**——インメモリの発注記録ストアが
同一 DecisionId を上書きせず（`FindByDecisionId` は最初の 1 件を返す）、競合そのものを再現できていなかったためである。
**終端の記録が照会の最中に現れる形へ直し、競合が起きたことを先に assert してから**赤を確認した。

### テスト ID の採番

**T-10-483〜493 は据え置き**（本 PR が最初にマージされる想定）。**空いている T-10-506 を採った**（507 も空きのまま残す）。

🔴 **［2026-09-19 追記（8） / 12 巡目監査 NB-12-3］帯の数字を是正した。** 実測は **#874 が 508〜515**、
**#873 が 494〜505 ＋ 516〜517**（506 / 507 との衝突を避けるため動かされた）。**結論（506・507 が空き）は変わらない。**

## ［2026-09-19 追記（8） / #820 の 12 巡目監査］窓の両方向を同時に閉じ、同型の誤記を別ファイルまで追随させる

12 巡目の監査は**中核が無傷**であることを実測した（無作為順列 1,200 試行で `violations=0` /
`worstNetAfterSystemClose=0`。武装 1,355 回・実決済を伴うガード巡回 304 回まで届いた**非空虚な掃き**）。
**ブロッキング 2 件のうち 1 件（BLK-12-1）は追記(7) の是正 1 が新設した鏡像の競合**である。

### 是正（IADR-0344 追記(11)）

1. 🔴 **`claimed = min(照会の前, 照会の後)`**（BLK-12-1）。追記(7) の「確定 → 照会に固定する」は
   **「窓のあいだ `claimed` は増える方向にしか動かない」を暗黙に仮定**していた。
   **減る方向にも動く**（決済・外部要因の確定・`PendingExternalReduction` の計上）ため、
   逆向きの窓で帰属不明が過少に読まれて武装した。**固定は T-10-507**（実決済経路を窓に踏ませる形）。
2. **`docs/data/audit-events.md` の同型の誤記を是正**（BLK-12-2）。11 巡目に `docs/functional/` を直したとき、
   **同じ文言を持つ別ファイルを引き直していなかった**。
3. **NB-12-1〜NB-12-6 をすべて対応**（下表）。

### 🔴 反復パターンの自己診断（監査の総括を受けて）

> 11 巡連続で実在の欠陥が出ている理由は、**「是正の向きだけを見て、逆向きを見ていない」**。

本追記ではこれを**手順**に落とした。**窓（時間差）を扱う是正では、是正の形を決める前に
「その量が増える側」と「減る側」の両方のプローブを書き、3 通りの形で実測する。**

| 形 | T-10-506（増える側） | T-10-507（減る側） |
| --- | --- | --- |
| 照会の**後**だけで数える（追記(6) の形） | **赤** | 緑 |
| 照会の**前**だけで数える（追記(7) の形） | 緑 | **赤** |
| **`min(前, 後)`（本追記）** | **緑** | **緑** |

**片側だけの形では必ず逆側が空く**ことが表の対角から読める。この表を出せるまで是正の形を確定しない。

🔴 **［2026-09-19 追記（9） / 13 巡目監査 NB-13-5］この手続きは本仕様書（#820 固有の凍結記録）の中にしかない。**
同型の事故（是正が自分の鏡像を新設する）は **10 巡目 → 11 巡目、11 巡目 → 12 巡目の 2 回**起きており、
規約の「同型 2 回で検査器／規約」の条件を満たす。**恒久の置き場所への昇格は #894 で裁定を仰いでいる**
（併せて、**規則 9・10 が本リポジトリの `traceability.repo.md` に存在しないのに 51 ファイルが引いている**
齟齬も同 issue で扱う。読み込み予算は実測 87% で余地が薄く、置き場所の選択に影響する）。

### 母集合（是正の走査・2026-09-19。**同型の文言を別ファイルまで**引き直す＝規則 9・10）

11 巡目は `docs/functional/` だけを直して `docs/data/` を落とした。**今回はファイルを絞らずに引いた。**

🔴 **［2026-09-19 追記（9） / 13 巡目監査 NB-13-2］下の表の件数は実測と合っていなかった。生の出力へ差し替えた。**
基準は**是正前のコミット `abaa7060`**、コマンドは `git grep -c -I "<語>" abaa7060 -- .`（合計は `awk` で加算）。

| 検索語 | 生の件数（`abaa7060`） | 内訳と扱い |
| --- | --- | --- |
| `0 約定で取り消された決済の残り` | **14 行 / 10 ファイル** | IADR-0344 3・`.ai-context/adr/README.md` 1・本仕様書 2・`ProtectiveStopGuard.cs` 1・`ProtectiveStopNetting.cs` 2・migration 1・`SoftwareStopBlockingRegressionTests.cs` 1・`SoftwareStopExecuted.cs` 1・`docs/data/audit-events.md` 1・`docs/functional/FR-10_risk-controls.md` 1。**是正したのは** `docs/data/audit-events.md`（BLK-12-2）・本仕様書の受け入れ基準 49・`SoftwareStopExecuted.cs`・migration・`ProtectiveStopNetting.cs` の XML doc、**13 巡目で** `SoftwareStopBlockingRegressionTests.cs` の T-10-487 ヘッダ（NB-13-1）。**正しい文脈のまま残したのは** IADR-0344 の追記（走らないと明記）・`.ai-context/adr/README.md` の索引・`ProtectiveStopGuard.cs`・`docs/functional/`（11 巡目で是正済み） |
| `毎巡回` | **31 行 / 12 ファイル** | 「毎巡回**検知する**」と書いているのは本仕様書 621 と IADR-0344 689 の 2 箇所のみ（**凍結記録のため本文は残し、追記側で訂正**＝NB-12-2）。他は「毎巡回鳴らさない」「毎巡回持っている」「毎巡回の引き直し」等の別の意味 |
| `取り違えは必ず` | **5 行 / 5 ファイル** | IADR-0344 追記(10) 決定 1（**偽の全称命題。撤回・訂正**）・`.ai-context/adr/README.md` の索引行（**撤回の注記を挿入**）・`OrderExecutionAppService.cs`（**書き直し**）・`SoftwareStopBlockingRegressionTests.cs` の T-10-506 ヘッダ（**書き直し**）・IADR-0151（**無関係**。別の主題） |
| `同じ順序` | **4 行 / 4 ファイル** | `OrderExecutionAppService.cs`（**書き直し**）・`SoftwareStopBlockingRegressionTests.cs`（**書き直し**）＝NB-12-5／`20260904_634` 仕様書・`GoodFaithViolationCountingService.cs`（**無関係**） |

🔴 **複数行にまたがる文をどう数えたか**（13 巡目監査 NB-13-2 の後段）:
撤回した命題「取り違えは**必ず**…**安全側（見送り）へ倒れる**」は、IADR-0344 の中で**3 行に分かれて**いる。
そのため **`必ず` と `安全側` を 1 行の中で結ぶ検索（`-E "必ず.{0,40}安全側"`）では構造的に拾えない**
——実測すると無関係な 1 件（`20260730_292` 仕様書）しか出ない。
**行をまたぐ命題は、行の中に収まる一意な断片を検索語にする**のが正しい引き方であり、
ここでは **`取り違えは必ず`**（5 行 / 5 ファイル）がその断片である。
12 巡目に書いた「`必ず` ＋ `安全側` → 2 行 / 2 ファイル」は**どの単行の走査でも再現できない数字**であった
（当時は自分が直前に書いた文を記憶で挙げており、**規則 9「記憶で挙げない」を破っていた**）。

**除外とその理由**: `CHANGELOG.md`（生成物）／`.ai-context/` の**他の**凍結記録（当時の記述。本仕様書と IADR-0344 だけを
日付つき追記で訂正する）／フロントエンド・`deploy/`（挙動を変えていない）。
...
| # | 対応 |
| --- | --- |
| NB-12-1 | `docs/tests/FR-10_risk-controls-tests.md` へ **T-10-506・T-10-507 の 2 行を登録**した（10 巡目の慣行に合わせる） |
| NB-12-2 | 走査表の分類理由を「誤りではない」→「**誤りだが凍結記録であり、訂正は追記側で行う**」へ直した |
| NB-12-3 | 帯の数字を実測へ是正（**#874 が 508〜515**・**#873 が 494〜505 ＋ 516〜517**）。結論は不変 |
| NB-12-4 | **`ConfirmEntryFills` が照会失敗の巡回でも走る**ようになった挙動変化を IADR-0344 追記(11) の「残る制約」へ記録 |
| NB-12-5 | `ProtectiveStopGuard` が検知の直前に保護記録を引き直している点を、両者の注記へ明記 |
| NB-12-6 | T-10-506 の assert コメントを「フックが発火し窓を置けたこと」までに弱めた（交錯そのものは示していない） |
