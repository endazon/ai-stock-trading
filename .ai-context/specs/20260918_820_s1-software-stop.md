...
| `ガードが毎巡回` / `毎巡回これを持っている` | 5 行 / 5 ファイル | `ProtectiveStopNetting.cs` 470・`ProtectiveStopGuard.cs` 131・テスト 888・本仕様書 621・IADR-0344 692。いずれも「**材料を毎巡回持っている**」（＝動機の記述）であり「毎巡回検知する」とは言っていない。**誤りではない**が、ガードの注記には走らない条件を 1 行足した |
...
**T-10-483〜493 は据え置き**（本 PR が最初にマージされる想定）。#873 が 494〜505、#874 が 508〜513 を使うため、
**空いている T-10-506 を採った**（507 も空きのまま残す）。
