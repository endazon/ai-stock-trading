...
| 検索語 | 生の件数 | 扱い |
| --- | --- | --- |
| `0 約定で取り消された決済の残り` | 9 行 / 5 ファイル | `docs/data/audit-events.md`（**是正**＝BLK-12-2）・本仕様書 672 の受け入れ基準 49（**是正**）・`docs/functional/FR-10_risk-controls.md`（11 巡目で是正済み・正しい）・IADR-0344 追記(9)(10)(11)（**正しい文脈**＝走らないと明記）・`ProtectiveStopNetting.cs` / `ProtectiveStopGuard.cs` の注記（正しい） |
...
| `必ず` ＋ `安全側` | 2 行 / 2 ファイル | IADR-0344 追記(10) 決定 1（**偽の全称命題。撤回・訂正**）・`OrderExecutionAppService.cs` の同じ注記（**書き直し**） |
| `同じ順序` | 3 行 / 3 ファイル | IADR-0344 追記(10)（**訂正**）・`OrderExecutionAppService.cs`（**書き直し**）・`ProtectiveStopGuard.cs`（**差を明記**）＝NB-12-5 |
