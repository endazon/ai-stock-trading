...
- 観測時刻は **現在の 2 日前**とする。ストアは「当日（米国東部の暦日）より前の取引日で最新の行」だけを
  判定に使い（当日の観測は日中の評価損益を含むため見ない）、鮮度上限は既定 4 日である。2 日前（48 時間前）は
  どの時刻でも米国東部の前暦日に属し、鮮度内に入る。1 日前（24 時間前）は、夏時間が終わる 25 時間の日の
  最後の 1 時間（米国東部 23:00〜23:59）に当日と同じ暦日へ落ちるため採らない（PR #903 の監査）。
- 評価額は従前の既定資金と同額（`TradingDefaults.InitialCapital` ＝ $3,000）とし、試験のコメントが
  前提にしている上限計算（1 注文 25% ＝ $750 ほか）を保つ。
- 同じ観測は口座種別のストアにも記録されるが、2 日前の観測は 30 分の有効期間を過ぎているため
