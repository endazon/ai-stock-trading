...
［2026-09-24 追記 / PR #931 監査］数量を持つ判断を外すと、差分で積み上がる再生では残した判断の経路が歪む
（入口を外せば残した出口が裸の空売りを建てる）ことが監査で実測された。数量を持つ判断を 1 件でも外したら
`ExcludedDecisionAltersReplayPath` で判定を組まない。見送り（数量 0）の除外は遮断しない（IADR-0387 決定3 追記）。
テスト ID を 2 つ足す（既存最大は T-15-112。重複が無いことを走査で確認した）。T-15-108 の評価側テストは外す記録を
見送りへ改め、T-15-111 は見送りの全件除外を対で置く。

| ID | 観点 |
| --- | --- |
| T-15-113 | 🔴 **否定形**: 数量を持つ判断を外すと残した判断の経路が歪むので、名前つきの理由で判定を組まない（入口／出口の双方） |
| T-15-114 | 🔴 **陰性対照**: 外したのが見送りだけなら経路は変わらず判定器へ到達する（再生の注文列が見送りを持たない記録集合と一致） |
