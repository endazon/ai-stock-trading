...
| 項目 | 供給の実測 | 根拠 |
| --- | --- | --- |
| **維持率** | **無い。** `IMaintenanceMarginSnapshotSource` の既定実装は `UnavailableMaintenanceMarginSnapshotSource`（常に `null`） | [IADR-0133](../adr/IADR-0133_maintenance-margin-auto-reduce.md) 決定5。PoC 項目 3 は「**実弾口座でのみ**照会でき SIMULATE では照会 API 自体が失敗」（[blocked-tasks](../../docs/blocked-tasks.md) A-2） |
