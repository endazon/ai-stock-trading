...
- 規則 11（窓）: 窓＝「免除の記録」と「終端の約定記録」の到着の間。増える側（打ち消しが要るのに残らない）＝終端が先に届く順序、
  ...
  ③ を T-10-897 / T-10-900 / T-10-901 で実測した。変異注入（2026-09-25・`WaiverSettlement` の 11 件）: 免除の側の判定を外す（＝①）→ T-10-900 が赤、
  終端の約定記録の側の判定を外す（＝②）→ T-10-901 が赤。

  | 形 | 増える側（免除 → 取消の順） | 増える側（取消 → 免除の順） | 減る側（全量約定・二重追記） |
  | --- | --- | --- | --- |
  | ① 終端側だけ | 残る | 🔴 残らない（T-10-900 赤） | 作らない・1 件 |
  | ② 免除側だけ | 🔴 残らない（T-10-901 赤） | 残る | 作らない・1 件 |
  | ③ 両側＋決定的 Id（採用） | 残る（T-10-901） | 残る（T-10-900） | 作らない（T-10-899）・1 件（T-10-900） |
