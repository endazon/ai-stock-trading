...
- [ ] `claude-sonnet-5` の 2026-09-01 以降の正式単価を一次情報（Anthropic 公式ドキュメント）で確認した
- [ ] 確認した単価と現行投入値が一致することを確認し、数値は変更していない
- [ ] `values-local.yaml` / `README.md` / `operations.md` / `IADR-0122` の期限つき警告を、確認済みの記述へ更新した
- [ ] `IADR-0122` は本文を書き換えず、日付つき追記ブロックで追補した
- [ ] コード変更を伴わないため `.NET` のビルド・テストへの影響はない（確認のみ実施）
