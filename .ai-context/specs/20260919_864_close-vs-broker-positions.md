...
| N1 | `Evaluate` が同一 (銘柄, 市場) を**符号付きでネット**するため、両建てでは正当な決済を止める（実測: ロング +300・ショート −100 でショートの買い戻しが「建玉なし」・ロングの売り決済が 200 株へ縮む） | **方向ごとに数える**形へ改め、回帰テスト T-10-516 を追加。報告する乖離の数量だけはネット（定期突合と同じ物差し） |
...
| N6 | 序数 4・5 が機械に固定されていない | `StopLossMethodContractTests` に序数と総数（6）の assertion を追加（T-10-517） |
| N7 | `DriftOf` の `BrokerOnly` 分岐が到達不能 | 分岐を削り、**到達しない理由**をコメントに残した |
| 採番 | 併走の #830（S1）が **T-10-483〜493 と 506** を先に使い、#874 が 508〜515 を使う | 本 PR を **T-10-494〜505** へ振り直し（追加分は **T-10-516・T-10-517**。**衝突していたのは 506 だけ**で、507 は未割当だったが**対を隣り合わせに保つため一緒に動かした**） |

## 差分監査の指摘の反映（#873・MERGEABLE の非ブロッキング 5 件）

| # | 指摘 | 反映 |
| --- | --- | --- |
| NB1 | N1 の是正で `closable` と `net` が独立したため、**両建てでネット 0・`Reduce` のとき乖離が `LedgerOnly` と誤分類**される（100 株を送った直後に「ブローカーには無い」と報告する） | 勧められた①を採用。`DriftOf` へ `ClosableQuantity` を渡し、**`closable > 0` なら数量不一致へ倒す**。回帰テスト **T-10-518**（変異〔`closable` の条件を外す〕で 2 件が赤になることを実測） |
