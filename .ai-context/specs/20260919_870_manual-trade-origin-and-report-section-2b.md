...
- 🔴 **［2026-09-19 追記 / 監査 2 巡目］期間で切った在庫の畳み込みそのものの欠陥は本 PR では直さない（#892）。**
  監査 BLK-1 の是正が塞いだのは「**取り込み**が在庫 0 に当たって新しく建てる」側であり、
  「**システム自身の決済約定**が在庫 0 に当たって新しく建てる」側は残る（取り込みが 1 件も無くても起きる。
  実測: 前期建玉 100 株を当期に 250 で決済し現在値 300 のとき評価損益 −5,000）。
  **是正前の実装でも同値**であることを確認しており、本 PR の退行ではない。
