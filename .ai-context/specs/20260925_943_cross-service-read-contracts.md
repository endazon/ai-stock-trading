...
| 8 ★ | 市場監視 `HttpPositionStore` | リスク `/open-positions` | `HeldPosition`（市場監視の Domain record） | `OpenPositionView` | **無し**（既存テストは**受け手自身の型**を直列化） | **`StopLossPrice`→0＝ロングは `price ≦ 0` まで発火しない（損切り保護が黙って外れる）**／`Side`→0＝ショートをロングとして判定（下落で誤発火）／`Symbol`→null（［2026-09-25 追記 / PR #959 監査］**実際はもっと悪い**: 実運用の市況源 Finnhub では `Uri.EscapeDataString(null)` が `ArgumentNullException` を投げ、`FinnhubMarketDataSource` が捕まえないため**巡回全体が失敗し、全建玉の損切り検知・値動き検知・生存要約が止まる**（エラーログのみ）。また `StopLossPrice`→0 では**含み益のショートが毎巡回ライン 0 で誤発火**する）。**fail open（損切りライン）** | T-10-803。実行時の堅牢化は追随 |
