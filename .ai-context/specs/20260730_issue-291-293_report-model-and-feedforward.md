...
- 計画根拠: 04_workflows/03_reporting-cycle（計画リポ）
...
- 本作業の実装判断は [IADR-0120](../adr/IADR-0120_report-kind-purpose-and-parent-policy-feedforward.md)（当初 `IADR-0117` で起票したが、並行 PR #294 が同番号を先に確保し
...
- **日報→取引の結線は実装済み**である。`ReportService.GetConfirmedDailyPolicy`
  ...
  タイムアウト・例外はすべて `null`＝取引しない安全側（FR-07・[IADR-0028](../adr/IADR-0028_daily-policy-sync-api.md)）。**本作業の対象外**。
- 確定は OwnerOnly のまま（ADR-0003・[IADR-0115](../adr/IADR-0115_report-auto-generation-scheduler.md) 決定1）。本作業は生成時の文脈供給のみを扱う。
...
- **日報→取引の結線**。実装済み（[IADR-0028](../adr/IADR-0028_daily-policy-sync-api.md)）。
- **確定（Confirm）の経路**。OwnerOnly のまま。自動確定は導入しない（ADR-0003・[IADR-0115](../adr/IADR-0115_report-auto-generation-scheduler.md) 決定1）。
- **`ReportPolicyDraft.CarryOver` が生成する方針文**。自動生成では「新しい方針を機械に提案させない」
  という [IADR-0115](../adr/IADR-0115_report-auto-generation-scheduler.md) 決定4 の判断は維持する。上位方針は**散文（Narrative）の文脈**としてのみ与え、
...
`PolicySummary` は「確定すると取引に効く」フィールドであり、[IADR-0115](../adr/IADR-0115_report-auto-generation-scheduler.md) 決定4 は自動生成で
