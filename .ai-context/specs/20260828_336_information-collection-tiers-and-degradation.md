...
- [ ] **区分 × 欠測の判定テーブルテスト**（必須欠測 → サイクル中止 or 縮退の分岐・ニュース系の「いずれか 1 つ以上」判定）
- [ ] **縮退中でも手仕舞い・損切り経路が生きていることのテスト**（否定形: 新規建てのみが止まる）
- [ ] **一般 Web 発動条件 4 件の境界テスト**と、**発動記録が月次の期間集計へ届くこと**のテスト
- [ ] **コネクタ単位のフェイク／録画ベーステスト**（外部 API を CI で叩かない）
- [ ] 検証用途区分がライブ判断の入力に入らないこと
- [ ] 一時降格（ADR-0005 決定 5）で欠測時の扱いが「記録のみ」へ切り替わること
