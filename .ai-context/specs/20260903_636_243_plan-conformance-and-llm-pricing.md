...
## ［2026-09-03 追記］本番 `values.yaml` への単価投入は撤回した

初版は本番 `values.yaml` へ `LlmPricing__PerModel__*` を投入したが、**Helm ゲートで赤くなり前提の誤りが判明した**。

```
##[error]既定描画に LlmPricing__ が現れた（経路B 限定の単価が本番へ漏れた）。
```

`.github/workflows/helm.yml` のこのゲートは **IADR-0114 決定 6「LLM 単価は変動する外部価格。本番
`values.yaml` には置かず経路B 限定で与える」**を機械で守るものである。**したがって「本番に単価が
0 件」は欠陥ではなく設計**であり、chart へ直書きして解決してよいものではない（価格改定のたびに
chart の変更とデプロイが要る形になる）。

本 PR では本番 `values.yaml` を develop の状態へ戻し、**経路B（`values-local.yaml`）の `report`
セクションへの単価追加だけを残した**（`LlmPriceTable` がサービスごとに独立して構成されるため、
経路B でも report 分が ¥0 のままだった）。

**残る課題**（#243 へ記録した）: 本番経路で月次上限 ¥15,000 の判定が発火しないという帰結は残る。
「本番の単価をどこから与えるか」（ArgoCD の値・外部 Secret・起動時取得のいずれか）は未決であり、
判断を仰ぐ。
