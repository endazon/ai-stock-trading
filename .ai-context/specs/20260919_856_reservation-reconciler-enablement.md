...
3. **`ProtectiveStopCoverageLost`（`Remediation=None`）を突合の終端化で発行する** ——
   **予約行だけからはエントリーと手仕舞いレグを区別できない**（予約は `PositionEffect` を持たず、
   ...
   保護逆指値ストアの active 行を総当たりで突き合わせれば判別の余地はある。ただし不完全な判別であり、
   外れたときに倒れる先が二重決済を誘発する側なので採らない。外れ方は 2 通り: **行が消えている**場合と、
   `IProtectiveStopOrderStore` に**逆引きの入口が無く**（口は `Save` / `Find(Guid)`〔エントリー ID 引き〕/
   `FindActive(int)` の 3 つ）、しかも **EntryDecisionId につき高々 1 行（最新の試行のみ）**しか持たないため
   **試行番号が進んだ**場合〔#882 3 巡目監査 N-2 の訂正。当初「入口が `FindActive` だけ」と書いたのは不正確〕。
...
| `.ai-context/specs/2026*.md`（`20260718_141` / `20260719_141` / `20260729_279`） | 確定済みの凍結記録。本文プロズを後から書き換えない。当時の記述は当時の事実として正しい |
| `.ai-context/specs/20260919_848_…`（#851 の記録） | 🔴 **本文は書き換えないが、`［2026-09-19 追記 / #856］` を 3 箇所へ入れた**（#882 3 巡目監査 N-7）。`.claude/rules/traceability.repo.md` により `.ai-context/specs/` は経過追記が可であり、この文書は「滞留 `Reserved` は自動では解決しない」を前提に読まれるため、追記が無いと**読者を人手経路へ誤誘導する**（同じ概念が 3 箇所にあり、離れているので 1 箇所では届かない） |
...
🔴 **［#882 3 巡目監査 N-7］数え方の誤りを記録する。** 2 巡目で「`自動の突合は既定で無効` の残りは 1 件」と報告したが、
これは**語の完全一致で数えた**結果であり、**同じ概念の別表現を 3 件落としていた**
（`20260919_848_…` の「滞留 `Reserved` は自動では解決しない」／「有効化そのものは別 issue」／
「自動リコンサイルが無効のあいだは人が解決する」）。**規則 9 が言う「誤りの側の文字列で走査する」は、
語を 1 つ選んで一致を数えることではない** —— 同じことを別の語で書いた箇所を、語の側から列挙し直す必要がある。
本 PR では `件を走査` / `自動の突合は既定で無効` / `突合` / `予約解放` / `BatchSize` / `逆向き` / `唯一の入口` /
`FindActive` の 8 語で引き直した。
