...
| # | 状況 | 挙動 |
| --- | --- | --- |
| 1 | `/policy` の確認ボタンで、この要求では確定を確かめられなかった（同じプロセスで先に `/report approve` 済み・照会の失敗の後の押し直し・版落ち） | 従来どおり案を引かず適用しない。応答に「この版を既に確定していて入れ替えがまだ適用されていないなら、設定画面から変更してください」を添える |
