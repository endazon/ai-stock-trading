...
- 決定: [IADR-0037](../adr/IADR-0037_async-contract-format-reevaluation.md)（AsyncAPI 不採用・**代替として `MessageUrn` 回帰ガードを後続 issue/PR で実装せよ**と明示）、
  [IADR-0079](../adr/IADR-0079_event-backward-compat-contract-test.md)（`EventBackwardCompatibilityTests`＝プロパティ単位の後方互換 snapshot）。
...
現行の契約ガード [IADR-0079](../adr/IADR-0079_event-backward-compat-contract-test.md) `EventBackwardCompatibilityTests` は、snapshot のキーを
...
名前空間の移動が起き得ない構成ではない。[IADR-0037](../adr/IADR-0037_async-contract-format-reevaluation.md) のフォローアップが指示した `MessageUrn` 回帰テストは
...
- **対象**: `AiStockTrading.Shared.Contracts.Tests` へ `EventMessageUrnTests` を追加。
  併せて [IADR-0079](../adr/IADR-0079_event-backward-compat-contract-test.md)「既知の限界」へ、本テストとの検出範囲の分担を追記する（Issue 受け入れ基準③）。
- **対象外**:
...
2. **母集合の網羅（`[Fact]`）**
   ...
   - 母集合を [IADR-0079](../adr/IADR-0079_event-backward-compat-contract-test.md) と同じ `EventTypeDiscovery` に単一化し、監査カバレッジ
...
### 3. [IADR-0079](../adr/IADR-0079_event-backward-compat-contract-test.md)「既知の限界」への追記
...
移行時は本テスト（および同じ前提に立つ [IADR-0106](../adr/IADR-0106_consumer-endpoint-name-uniqueness.md) のキュー名固定テスト）を、
Wolverine の識別子規約へ更新するか、移行後の識別子固定テストへ置き換える必要がある。
本 PR 時点で本リポジトリに Wolverine 移行の Issue は起票されていない。

## 実装 ADR の要否

**新規 IADR は作成しない**。本作業は [IADR-0037](../adr/IADR-0037_async-contract-format-reevaluation.md) が「決定」節で明示的に指示した後続実装
...
本作業仕様書と [IADR-0079](../adr/IADR-0079_event-backward-compat-contract-test.md) への追記に記録し、決定の権威は既存 IADR に置く。
