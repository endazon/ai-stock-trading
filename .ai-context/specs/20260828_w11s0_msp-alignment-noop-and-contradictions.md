...
**母集合の再確認（規則1: 誤りの文字列 `新 ADR` で全文走査）**

［2026-08-28 追記 / #579 レビュー指摘］🔴 **当初ここに書いた「12 件ヒット」は誤りで、下表も 1 ファイル
取りこぼしていた。** 数え直した実測は次のとおりである（`git grep -n "新 ADR" origin/develop`）。

| 区分 | 箇所数 | ファイル数 |
| --- | --- | --- |
| 走査の全ヒット | **23** | **16** |
| うち凍結記録（`.ai-context/adr/` 4 箇所 3 ファイル ＋ `.ai-context/specs/` 4 箇所 1 ファイル）＝除外 | 8 | 4 |
| **非凍結の母集合** | **15** | **12** |
| うち是正した箇所（`AGENTS.md:42` / `CLAUDE.md:105`） | 2 | 2 |
| **据え置き（正しい用法）** | **13** | **10** |

**取りこぼしていたのは `.github/PULL_REQUEST_TEMPLATE.md`（21・24 行目）である。** 用法は
「計画 ADR の逸脱理由を記入する欄」で正しく、是正は不要だが、**「1 件ずつ確認して据え置き」と
書いた母集合に入っていなかった**。本 PR がまさに是正しようとしている「母集合の取り違え」と
同型の記録漏れであり、下表へ追加した。

据え置き 13 箇所は、次の理由で**すべて正しい用法**である（全件を個別に確認した）。
...
| `.github/PULL_REQUEST_TEMPLATE.md:21,24` | 「逸脱がある場合はその理由と新 ADR の有無を明記する」「ADR 制約からの逸脱: なし / あり（理由・新 ADR: ）」 | **PR 本文で申告するのは計画 ADR からの逸脱**である（IADR は自分で起こすもので「有無を申告する」対象ではない）。正しい ［2026-08-28 追記 / #579］ |

→ **新規の是正は 0 件**（是正したのは `AGENTS.md:42` / `CLAUDE.md:105` の 2 箇所のみ）。
...
| --- | --- | --- | --- | --- |
| 1 | `新 ADR` / `新ADR` | (3) の追加確認 | **15 箇所 / 12 ファイル**（全 23 箇所 16 ファイルから凍結記録 8 箇所 4 ファイルを除外）［2026-08-28 訂正 / #579］ | 0（是正は既知の 2 箇所のみ。残り 13 箇所は正当な計画 ADR 参照） |
