...
- 関連 IADR: 本作業で新規 [IADR-0064](../adr/IADR-0064_official-source-connectors.md)。安全既定は [IADR-0022](../adr/IADR-0022_information-collection-safe-sourcing.md) を踏襲
- 対象 Issue: #9（Slice B）
...
- `DelayingRateLimiter` はフェイク `IClock`＋フェイク待機（待機した時間だけ時計を進める）で決定的に検証する（実時間を
  待たない）。時刻抽象は `TimeProvider` ではなく本リポジトリの既存慣行 `IClock` に合わせる（IADR-0064 決定 2・選択肢 4）。
...
- 実装ADR: [IADR-0064](../adr/IADR-0064_official-source-connectors.md)（本スライス）、[IADR-0022](../adr/IADR-0022_information-collection-safe-sourcing.md)（安全既定・データ分離）
