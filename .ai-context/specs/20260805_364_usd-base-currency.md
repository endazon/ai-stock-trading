...
- 関連 ADR: ADR-0016（計画リポ） 決定6（空売り解禁の自己資金 $5,000）、
  ADR-0018（計画リポ）（既定値の確定単一値）、
  ADR-0022（計画リポ）（為替レート源と鮮度）
...
- 計画書リンク: 05_trading-assumptions.md（計画リポ） **§3**（基準通貨〔判定〕= USD・利用者決定 2026-07-31）・§5
