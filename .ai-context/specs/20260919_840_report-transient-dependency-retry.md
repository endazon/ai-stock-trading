...
2. **観測**: 同じハンドラが失敗を分類して、生成 1 回ぶんの観測（`ReportDependencyProbe`・AsyncLocal）へ記録する。
   - 一過性: トークン取得不能・接続不能（`HttpRequestException`）・タイムアウト（LLM を除く）・5xx・408・429
     ・**401**（［2026-09-19 追記 / #866］監査の実測。上流の JwtBearer 設定取得器〔IdentityModel 8.0.1〕の
     バックオフにより、**Keycloak 回復から 24.6 秒は正しいトークンでも 401** になる）
   - 恒常: **403**・その他の 4xx（**待っても変わらない**。`GrpcAssumptionsClient.IsRetryable` と同じ考え方。
     403＝ロール未付与などの設定誤りは再試行しても直らないので恒常のまま残す）
3. **見送りと再試行**: 「その種別が使う入力が未供給」かつ「その入力の取得で一過性の失敗を観測した」とき、
   ...
   **30 秒 → 60 → 120 → 240 → …（上限は通常の巡回間隔）**へ縮める。**上限 5 回**（構成可）。
   🔴 **［2026-09-19 追記 / #866］見送りが生成窓の終端を跨ぐなら見送らない。** `ReportSchedule.Due` は
   月報を当月内・週報を当 ISO 週内でしか due にしないため（月報の窓は最短 7 時間＝最終営業日 17:00〜月末 24:00 JST）、
   見送っている間に窓が閉じると**その期間は二度と due にならず、縮退版も上限到達の警告も出ない**（無音の消失＝
   本変更前からの退行）。**次に試す時刻（`clock.UtcNow` ＋ 次の待ち時間。回数は消費せずに先読みする）に
   その PeriodKey がまだ `Due` に含まれるか**を確かめ、含まれなければ見送らず 4 の縮退版を出す。
4. **上限到達・恒常的な失敗・次の試行時刻に窓が閉じている場合**: 縮退した報告書を**従来どおり生成・提示**し、
   常駐が警告を残す（沈黙しない）。窓の終端は上限到達と**別の文言**で残す（原因が違う）。
   ［2026-09-19 追記 / #866］**窓が閉じて対象から外れた期間の見送り回数は 1 巡回の終わりに捨てる**（`RetainOnly`）。
   生成による解放が起きない期間の回数がプロセス内に残り続けるのを避ける。
5. **見せ方**: 未供給だった入力を `reports.UnsuppliedInputs`（新列）へ記録し、
   - ドラフト通知（`ReportDraftPresented.Summary`）の数値行の直後に警告行を足す（散文より優先）。
     ［2026-09-19 追記 / #866］**未供給があるドラフトの提示通知は `NotificationSeverity.Warning`**
     （無ければ従来どおり `Info`）。判定は要約の警告行の印（`Shared.Contracts` の
     `ReportSummaryMarkers.UnsuppliedWarningPrefix`）で行い、**イベントの形は変えない**
...
| `NotificationService/Features/Notifications/NotificationFormatter.cs` | ［#866］未供給があるドラフト提示は `Warning` |
| `Shared.Contracts/Events/ReportDraftPresented.cs` | ［#866］要約の警告行の印を両サービスが引く定数にする（契約の形は不変） |
...
| 11 | ［#866］**見送りが生成窓の終端を跨ぐなら見送らず**、その巡回で縮退した報告書＋窓の終端を言う警告が出る（月報・週報） | 単体（生成器・常駐のログ） |
| 12 | ［#866］監査の再現: 月末最終営業日の深夜に一過性の失敗で見送っても、**窓が閉じる前に月報が出る**（否定形: 翌日の巡回で月報が永久に出ない、にしない） | 単体（巡回のカデンツを再現） |
| 13 | ［#866］窓が閉じて対象から外れた期間の**見送り回数が残らない** | 単体 |
| 14 | ［#866］**401 は見送る**（一過性）／**403 は見送らない**（恒常）——分類・生成器の双方で | 単体（ハンドラ・生成器） |
| 15 | ［#866］未供給があるドラフト提示の通知は **Warning**／無ければ **Info**（否定形） | 単体（通知の整形） |

## 未検証・保留

IADR-0352「保留」を参照（gRPC 経路・トークン取得失敗の細分・確定の機械的拒否）。

［2026-09-19 追記 / #866］監査の指摘のうち**記録に留めたもの**（コードは直さない）:

- ゲートウェイ側の 504 / 502 は一過性として最大 5 回再試行され、モデル課金の**後**に 5xx を返す構成では費用が増え得る。
  **429 の `Retry-After` は見ていない**（倍々の待ち時間で近似する）。→ IADR-0352「結果・残余リスク」へ追記済み。
- `ReportInput` の語彙に上位方針・前期方針が無いため、#839（上位方針が空の週報が確定される）は本仕組みでは捕まらない。
  語彙を足せば同じ警告経路に乗る。→ #839 へコメントで残した。
- 「上流の 401 が一過性であり得る窓」は**実測により解消**（一過性へ移した。上記 決定 2）。
