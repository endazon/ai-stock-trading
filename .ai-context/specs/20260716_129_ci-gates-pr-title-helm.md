...
- 実装判断: [IADR-0058](../adr/IADR-0058_helm-chart-ci-gate.md)（本作業で起票。Helm chart の CI ゲート方式）
...
- k8s スキーマ適合検査（kubeconform 等）。→ [IADR-0058](../adr/IADR-0058_helm-chart-ci-gate.md) にフォローアップとして記す。
- `ci.yml` の変更（共有ファイルの競合回避。独立ワークフローで足りる）。

## 設計

詳細は [IADR-0058](../adr/IADR-0058_helm-chart-ci-gate.md)。要点:
...
| `docs/ai-workflow.md` | 必須チェックへ `PR Title` を追記（`Helm` はパスフィルタのため推奨に留める。理由は [IADR-0058](../adr/IADR-0058_helm-chart-ci-gate.md)） |
...
- **必須チェック化**: リポ設定（ブランチ保護）はコード変更では完結しないため、`docs/ai-workflow.md`
  ...
  （GitHub 仕様）。本作業では推奨に留める（[IADR-0058](../adr/IADR-0058_helm-chart-ci-gate.md) のフォローアップ参照）。
