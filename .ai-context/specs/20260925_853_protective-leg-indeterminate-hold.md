...
type: spec
status: accepted
related_ids: [FR-05, FR-10, FR-11, FR-12, UC-02, UC-06, ADR-0040, IADR-0057, IADR-0074, IADR-0092, IADR-0117, IADR-0210, IADR-0342, IADR-0344, IADR-0347, IADR-0362, IADR-0371, IADR-0424, IADR-0428]
...
## ［2026-09-25 追記 / PR #1005 監査］監査の指摘への対応

監査の判定は GO-with-nits（今夜の S1 の建玉を悪化させる経路は無い）。対応は IADR-0428 の同日の追記に記録した。

| 指摘 | 対応 | テスト |
| --- | --- | --- |
| 2. 照会不明の見送り（IADR-0424）が送信結果待ちの行をブローカー側の株数に数える | 送信結果待ちがあれば `Unknown` | T-10-1079 |
| 3. 逆指値の予約そのものが例外で落ちると無音 | エントリー同時: 送信結果待ちを残し `None` と Critical。ガード: 行は変えず `None` と Critical・`CloseFailed` | T-10-1062（続き）・T-10-1068（続き） |
| 1. 配備の順序・戻しの禁止 | 発注経路の Runbook に追記 | —（文書） |
| 5. 保護レグの突合が「保護の記録がありません」の Critical になる | `ProtectiveLeg`（Active 行の StopDecisionId／この試行の CloseDecisionId）として張らず Information | T-10-1071（続き）・T-10-1078（続き） |
| 7. IADR-0362 決定 3 の追記・序数の固定 | IADR-0362 に追記、`StopDispatchIndeterminate = 5` を序数テストへ | 既存の序数テスト |
| 4・6（記録のみ） | IADR-0428 の追加の残余リスクとテスト仕様書の残余リスクへ | — |

変異: 送信結果待ちを数える（9 件中 1 件赤）・予約の例外を捕まえない（エントリー同時 19 件中 1 件赤／ガード 17 件中 1 件赤）・保護レグを判別しない（14 件中 2 件赤）。

## ［2026-09-25 追記 / PR #1005 再監査］

- 予約の失敗の保護喪失を `None` から `StopReservationFailed`（値 6）へ。通知は「予約できず未送信」、エントリー同時はガードが約 30 秒後に張ると伝えて
  手で逆指値を置かせない。テスト: T-10-1074 / 1075 / 1076（続き）、T-10-1062 / 1068（続き・期待値を更新）、序数テスト。
- ガードの予約の失敗の通知を 1 時間ごとに絞る（T-10-1068 続きに巡回 3 回の検査を追加）。
- IADR-0428 の残余リスクに「張り直しが成り立つのは約定後」（#1013）を追記。
- origin/develop（9e133b55）をマージ。衝突は `docs/tests/FR-10_risk-controls-tests.md` の trace ブロックだけ（キーごとの和集合・節はすべて保持を機械で確認）。
- 変異: ガードの通知を巡回ごとに出す（17 件中 1 件赤）・通知が予約の失敗を保護喪失の文面で書く（66 件中 2 件赤）。
