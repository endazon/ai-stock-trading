...
> 🔴 **本節の `月報 §N` は実装の出力（ゴールデン `monthly-supplied.md`）の節番号である。**
> 本文の他の箇所（前掲の背景・コード内コメント）は**計画書 `04_report-templates.md` の節番号**を引いており、
> 両者は月報でのみ食い違う（実装 §5=稼働率分布 / §6=三者比較、計画 §5=三者比較 / §6.2=稼働率分布）。
> **この乖離は本 issue が持ち込んだものではない**（develop 時点の `monthly-supplied.md` が既にこの並びで、
> 本 PR の diff に月報の見出しの増減は無い）。IADR-0269 決定5 が日報のみ計画の並びへ寄せ「週報・月報は不変」と
> 明記したため残っている。**番号を入れ替えてはならない** —— 入れ替えると実出力に存在しない節を指すことになる。

1. **実装の月報 §6**（三者比較。**計画書では §5**）に SIMULATE / 実弾の実値が出る。段に到達していない列は「該当なし」、
   到達済みで約定 0 件の列は「0 件」と**書き分かれる**
2. 日報 §1 の稼働率行と**実装の月報 §5**（稼働率分布。**計画書では §6.2**）に実値が出る
