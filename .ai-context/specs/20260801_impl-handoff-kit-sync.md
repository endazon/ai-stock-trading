...
- 対象:
  - `planning` submodule の pin 前進（`10d8ce2` → `9cd3499`。作業中に計画側で #98 / #105 / #107 / #110 / #113 / #116 / #119 / #125 / #127 / #132 / #135 / #138 / #141 / #143 が続けてマージされたため計 13 巡取り込んでいる）
...
- [ ] `planning` submodule が `origin/main` の先端（`9cd3499`）を指す
...
### 第 13 巡（pin `9cd3499`）— #142 / #139 とも解消。キット側の残課題は無くなった

計画側 PR #141（issue #140）と PR #143（issue #139）で、本リポから起票した 2 件が反映された。

- **#142（= 計画側 #140）**: `scripts.test.js` が GitHub Actions 上でだけ失敗する回帰が是正された。
  取り込み後に**両モードで実測**して確認した（キットとバイト一致のまま 85 件合格）。

  | 条件 | 結果 |
  | --- | --- |
  | ローカル（`GITHUB_ACTIONS` 未設定） | ✅ 85 件合格 |
  | `GITHUB_ACTIONS=true`（CI と同条件） | ✅ 85 件合格（是正前はここで失敗した） |

- **#139**: `check-doc-links.js` が検査対象外にした範囲を報告するようになった。
  未 populate な submodule 配下を模した環境で実測すると、除外件数が submodule 別に出る。

  ```
  ::notice::未 populate の submodule 配下 733 件のリンクを検査対象外にした（planning: 733 件）。
           この範囲は本実行では検査されていない。…
  ```

  併せて提案していた `check-commit-messages.js` の skip 可視化（計画 ADR 実在性検査・IADR 検査）も
  `notice` として入った。「notice は実行時の呼び出し側でのみ出す。`loadExisting*` の内部に置くと
  未 populate を模したテストのフィクスチャが本物のアノテーションを漏らす（#140 と同型）」という
  注記まであり、同じ轍を踏まない構造になっている。

これで本リポから起票した 15 件（#97 / #103 / #106 / #109 / #112 / #115 / #118 / #120 / #122 / #124 /
#131 / #134 / #137 / #139 / #142）はすべて反映済みとなり、キットとの残差分は
「本リポ固有の実コンテンツ」「HOWTO Part B-5 の置換点」「本リポ固有の CI ジョブ」だけになった。
