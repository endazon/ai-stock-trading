...
- 計画書リンク: 02_requirements FR-10（計画リポ） ／ 05_trading-assumptions §5（計画リポ）
...
2026-08-31 期限）に依存する（ADR-0019（計画リポ））。
...
- [x] 統制系 FR-10 の 3 点セット（境界値・プロパティベース・否定形）が揃っている（**第 3 段階で完了**。
      [テスト仕様書](../../docs/tests/FR-10_risk-controls-tests.md)「第 3 段階で埋めた穴」）
- [x] 計画書の誤り・不足を環流した（**第 3 段階で完了**。拒否理由コードの不足＋空売り比率 50% の
      構造的含意の 2 件。いずれも起草のみで送付は未実施）

## テスト方針

[テスト戦略](../../docs/tests/README.md) §2 の 3 点セット（境界値テーブル・プロパティベース・否定形）で写像する。
詳細は [テスト仕様書 FR-10](../../docs/tests/FR-10_risk-controls-tests.md)。
...
| 区分 | 内容 |
| --- | --- |
| テスト | 否定形 2 件（T-10-127 分割発注 / T-10-128 決済偽装）・プロパティ 1 件（T-10-156 比率の等価形）。詳細は[テスト仕様書](../../docs/tests/FR-10_risk-controls-tests.md)「第 3 段階で埋めた穴」 |
| 環流 | 空売り比率 50% の構造的含意を新しい環流文書へ起草（20260804_adr0016-short-ratio-denominator（環流記録））。既存の環流文書（拒否理由コードの不足）へ「送付は未実施」の注記と相互リンクを補った |
...
- 計画への環流: 拒否理由コードの不足（環流記録）・
  空売り比率 50% の構造的含意（環流記録）（いずれも送付は未実施）
- 機能仕様書: [FR-10 リスク統制](../../docs/functional/FR-10_risk-controls.md)
- テスト仕様書: [FR-10 リスク統制（再実装）](../../docs/tests/FR-10_risk-controls-tests.md)・
  [FR-10 リスクガードコア（再実装前）](../../docs/tests/FR-10_risk-guard-core-tests.md)
- データ仕様書: [リスク管理ドメインの集約](../../docs/data/risk-management-aggregates.md)
