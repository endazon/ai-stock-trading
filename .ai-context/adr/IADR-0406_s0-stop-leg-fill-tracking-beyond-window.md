...
- **残余 5（Active な保護記録の走査上限）**: S0 のレグを足すための `protectiveStops.FindActive(batchSize)` は、約定追跡の
  1 巡回の記録件数の上限（`FillPolling:BatchSize`・既定 200）をそのまま使う。Active な保護記録（S0・S1 合算・作成の古い順）が
  200 件を超えると、新しい建玉の S0 のレグが走査から外れ、24 時間を超えた約定は本決定でも拾えない（元の欠落が再現する）。
  稼働口座の Active 行は数件であり（2026-09-25 時点で 2 行）、上限に達する配置は現実的でないため、別の設定値には分けない。
  Active 行がこの規模に近づく運用へ移るときは、走査上限を独立させる（PR #973 のレビュー指摘）。
