...
> **［2026-09-26 追記 / #826］2 回目の再検証（`origin/develop` = `f23f2fa3`）。**
> - 項目 1・2・4・5 は閉じた（1 と 5 は IADR-0413、2 と 4 は上の 2026-09-25 追記）。本 PR は、試験で固定されていなかった分岐
>   （リスク管理の検知ログの S0〜S3 の列挙・打ち消しの時刻・最大の約定数・読めない免除の行）を試験で固定した。
> - 項目 3（同一銘柄の S0・S2 併存）は**残る**。FR-10 の機能仕様書の「S0 の建玉に対するガードの挙動は、同じ銘柄に S1 の建玉が無い限り
