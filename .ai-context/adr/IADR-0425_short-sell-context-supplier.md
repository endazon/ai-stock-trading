...
- 悪い影響・トレードオフ（残余リスク）:
  ...
  - 配備: `values.yaml` は order-execution に `auth: true` を足し、risk-management の `OrderExecution__BaseUrl` は**空（未結線）**のまま置いた。結線は配備の判断（SIMULATE では結線しても結果は変わらない）。
  - 照会の予算とキャッシュはプロセス内（発注執行の複製が複数になれば予算も複数になる。現状は単一）。同じ銘柄の同時の要求はそれぞれ照会し得る（予算は数える）。借株可否の答えは最長 60 秒古い。
