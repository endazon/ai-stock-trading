...
（一時的な 0・既定値の 0）を**区別できなくなる**。どちらも `CapitalBaselineUnavailable` になり、
拒否理由からは「残高が無い」のか「照会が壊れている」のかが読めない。
**受け入れる理由**: 残高 0 の口座では比率上限がすべて 0 であり、**いずれにせよ新規建ては 1 株も通らない**。
変わるのは**止まる／止まらない**ではなく**記録される理由**だけであり、
区別が要るなら照会のログ（`資産純値が 0 以下です totalAssets=…`）が残している。
