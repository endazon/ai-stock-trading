...
- `IMoomooTradeClient.GetAccountEquityInBaseAsync` を新設し、`Currency=Currency_USD` / `RefreshCache=true` で照会する。
  ...
  🔴 **［2026-09-19 是正 / #874 の監査］通貨の欠落も採らない。** 初稿は `HasCurrency && != USD` と書いており、
...
- 🔴 **応答に評価額が無いことを理由に口座種別まで捨てない。** 欄を分けてあり、捨てると口座種別依存の統制
  ...
  🔴 **［2026-09-19 限定 / #874 の監査］射程は「応答に値が無い場合」に限る。** 評価額の照会が
...
- 🔴 **供給の可否は「当日より前の取引日の行があるか」で決まる**（鮮度はその次の条件である）。実測した帰結:
  ...
  | 金曜 23:55 ET に観測 → 月曜の寄り付き（経過およそ 2.4 日） | 供給される |
  | 三連休（金曜 23:55 ET → 火曜の寄り付き。経過およそ 3.4 日） | 供給される |
  | 木曜 23:55 ET が最後 → 火曜の寄り付き（経過およそ 4.4 日） | 🔴 供給されない（鮮度切れ） |
...
- 🔴 **［2026-09-19 是正 / #874 の監査］`ShortSellEvaluator` へは `null` をそのまま渡す。**
