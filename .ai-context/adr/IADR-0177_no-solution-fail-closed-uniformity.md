...
### 決定3補: 呼び出し側の重複ガードは**消さない**

決定3 で `Threshold` 側に `multiple <= 0m` の判定を入れた結果、`ProfitabilityGate` が呼び出し**前**に持つ同じ判定は**結果として重なった**。**それでも消さない。**

**二重の fail-closed を減らす利得より、片方を消したときに気付けない損失のほうが大きい** —— **#461 の欠陥そのものが「呼び出し側だけが守っていた」形で生まれている**。

さらに、**同じ条件式の他の節は「冗長」ではない**。`roundTrip is null` / `roundTrip <= 0m` は**依然として効いている** —— `Threshold` は負の費用を 0 へ正規化して**しきい値 0 を返す**ため、ここで止めないと **0 以上の期待利益がすべて `Viable`** になる（＝全通過）。**「この条件式は冗長」と一括りにして整理すると、効いている節まで消える。** その旨をコードのコメントに残した。

**規則の正は式の側（`Threshold`）にある。** 呼び出し側はその写しであり、**写しを直しても他の呼び出し側は守られない**（決定3 の理由と同じ）。
