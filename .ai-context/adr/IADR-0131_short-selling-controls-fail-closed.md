...
- 関連する計画書 ID: **FR-10**（リスク統制・空売り 8 規則・拒否理由 7 種）／ FR-19・FR-20（境界）／ UC-06 ／
  **ADR-0016**（空売りの段階解禁と専用統制）・ADR-0009（3 統制の優先順位・手仕舞い不停止）・ADR-0003 ／
  ...
  06_daytrading-review §4.1（計画リポ）（クラス A/B/C の定義）
...
（feedback/20260804_adr0016-stop-order-rejection-reason.md（環流記録））。
...
> （原因〔都度の借株需給 vs 期間の経過〕も解除条件も異なり、畳むと監査ログの理由が実態と食い違うため）。
> **本段落の写像は廃止**し、`ShortSellEvaluator` は `BuyInBanned` を列挙する。
> `BannedSymbol` を用いないという判断は裁定でも維持された（30 日リストは `BannedSymbol` の
> 禁止銘柄リストとは別の空売り専用リストである）。追随は [#374](https://github.com/endazon/ai-stock-trading/issues/374)。
...
- 悪い影響・トレードオフ:
  ...
  - `StopOrderRequired` は計画に無いコード名である（決定3・環流待ち）
    → **2026-08-04 に計画が同名で追認**（ADR-0016 決定10 の改訂・#374）。解消済み
...
- フォローアップ:
  ...
  - 計画への環流: `StopOrderRequired` のコード名・強制買戻し禁止の理由コード
    → **2026-08-04 に裁定済み**（拒否理由 9 種へ改訂。`StopOrderRequired` は追認、`BuyInBanned` は新設）。
    実装の追随は #374・[IADR-0134](IADR-0134_rejection-reason-ordinal-and-plan-registry-transcription.md)
