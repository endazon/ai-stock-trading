...
> **［2026-09-24 追記 / PR #931 監査］上の「その判断は成績のどこにも寄与しない」は、数量を持つ判断については偽である。**
> 再生の注文は目標建玉ではなく**差分**であり、`BacktestSimulator` が `SignedInventory` で積み上げる。
> したがって数量を持つ判断を外すと、**残した判断が AI の実際には取らなかった経路を走る** ——
> 入口を外せば残した出口が裸の空売りを建て（監査の実測: 6/2 の Buy +10 を外し 6/3 の Sell −10 を残すと、
> 再生はフラットから −10 を出した）、出口を外せば買い建てが開いたまま残る。DSR・最大 DD はその架空の経路を
> 測り、それでも `Passed = true` が出得た。外した判断は「寄与しない」のではなく、**残した判断の成績を歪めて寄与する**。
>
> - 🔴 **数量を持つ判断（`SignedQuantity != 0`）を 1 件でも外したら判定を組まない。** 新しい遮断理由
>   `Stage0GateCheck.ExcludedDecisionAltersReplayPath` を `Stage0ReplayEvaluation.Prepare` が返す
>   （決定5 と同じ駆動側の事前条件。判定器の 7 条件は増やさない）。`AllDecisionsExcluded` を先に判定する
>   ——残した判断が無ければ、歪む経路そのものが無い。
> - **見送り（数量 0）の除外は遮断しない。** 注文を作らないため、外しても残した判断の経路は変わらない。
>   見送りも除外として数える（本決定の 2 つ目の箇条）は**維持する**。
> - 🔴 **帰結: 現時点で「外して進める」のは見送りだけである。** 数量を持つ判断を外しつつ残りを正しく再生するには、
>   注文を差分ではなく目標建玉として解釈する等、再生の意味そのものを変える設計が要る（本追記の射程外）。
>   それまでは数量を持つ判断の除外は Stage 0 を成立させない側に倒れ、ADR-0036 フォローアップ 5
>   （除外で Stage 0 が成立しなくなったときの計画への報告）の材料になる。
> - 固定するテスト: T-15-113（監査のプローブ。入口／出口の双方を外す 2 ケース）・T-15-114（見送りだけを
>   外した場合は遮断せず、再生の注文列が見送りを持たない記録集合と一致する）。T-15-108 は外す記録を見送りへ改めた。
