...
**基盤との乖離として受容する**（`ADR-0029` 決定 6）。段 2 以降の proto も本プロジェクトへ置く。
...
- パターン: `**/AiStockTrading.Shared.Grpc/obj/**/*.cs`（`reason` は必須。G3）。
- **G4（除外は自動生成の部分集合）は素通りしない**: `findSourceFiles` は `obj/` を走査しないので、
  この除外が手書きを飲み込む経路は構造的に存在しない（かつ protoc の出力は `<auto-generated />` を持つ）。
- **`**/obj/**/*.cs` のような広いパターンにはしない** —— 将来ほかの生成物を巻き込む。
  プロジェクトを名指しし、proto の置き場が増えたときに**明示的に足させる**。
