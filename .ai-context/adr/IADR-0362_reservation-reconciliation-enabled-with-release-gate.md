...
- 🔴 上の 2 つ（**「commit 済み → 再走査されない」幾何**）は [#890](https://github.com/endazon/ai-stock-trading/issues/890)
  ...
  🔴 **［2026-09-23 追記 / [#890](https://github.com/endazon/ai-stock-trading/issues/890)・IADR-0371］
  中断で失われる側は是正した。** 記録（保護レグ不在の Critical）と発行（`OrderExecuted`）を、その予約の
  `MarkCompleted` を commit した**直後**に 1 件ずつ行う形へ移した（出口＝`IReservationReconciliationSink`。
  #890 の方向 (a) を、**所見だけでなく発行にも**広げた）。上の「監査のプローブ実測」が指す挙動は
  もう起きない（T-10-646 / T-10-650 / T-10-651）。**残るのは (b)・(c) 側である** ——
  durable outbox は依然として未配線で、確定と出口のあいだの窓は縮んだが**ゼロではない**。
  なお本 ADR 決定 3 の「記録は発行より先」は**覆っていない**（適用単位が巡回から 1 件へ狭まっただけである）。
  ただし**発行が落ちる巡回では巡回サマリが出なくなった**（T-10-609 の 2 つ目の表明を意図的に反転した。IADR-0371）。
