...
- **実行中に実績が見積りを超えたら停止し、途中までの記録を保存して報告する**（ADR-0033 決定5.3）。
  ...
  🔴 **裏返すと、超過幅は `VoteCount` に比例する**（`VoteCount` 回の呼び出し × 1 判断あたりの費用まで見積りを
  超え得る）。多数決回数を大きく承認する運用では、この上振れ幅も承認額に織り込むこと（PR #713 の AI レビュー
  指摘を受けて `docs/blocked-tasks.md` B-7 に運用上の注意として明記した）。
