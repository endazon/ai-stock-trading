...
**本 IADR の起草過程で、実装方針の判定を 4 回訂正している**（うち 1 件は AI レビュー指摘による）。経緯は作業仕様書に残した。とくに次の 2 件は方法論上の教訓として本 IADR に引く。
...
> あわせて、項目 7 の不成立時の帰結が当初の記録より重いことも判明した。ADR-0019 は「Stage 0 の合格判定を実施できない。**ADR-0005（計画リポ） の有料枠の検討へ移る**（月次データ費用上限〔現在 0 円配分〕の見直しを伴う）」と定めている。**決定 6 は、この有料枠の検討そのものを回避する意味を持つ。**
...
> **［2026-08-07 追記・本決定は計画の裁定を得て [IADR-0158](IADR-0158_short-sell-borrow-permit-primary-gate.md) が実装へ確定した］**
> 本決定が「計画へ環流する」とした内容は **ADR-0016 決定3 の 2026-08-06 改訂**として裁定された
> （endazon/project-planning#204）。IADR-0158 が**後継**として実装を確定している——一次ゲートを
> `ShortPermit`（＝`IsShortPermit`）へ移し既存の `BorrowUnavailable` へ写像する（新しい拒否理由は追加しない）／
> 20% 閾値はコード・テスト・計画適合検査の 3 経路で残置を担保する／`ShortFeeRate` は写像も費用接続もしない
> （[#417](https://github.com/endazon/ai-stock-trading/issues/417)）。**本決定の内容は覆されていない。**
...
ADR-0023（計画リポ） は Stooq が取得不能であることと回避実装の禁止を決めたが、**代替源を定めていない**。moomoo がその答えになる。これにより [IADR-0138](IADR-0138_stage0-drawdown-tolerance-tightening.md) が「実効は #382 の解決に依存する」とした条件が解ける見込みが立つ。
