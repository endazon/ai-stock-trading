...
- 関連する計画書 ID: FR-04（AI 判断）、FR-06（報告書）、FR-11（監査ログ）、FR-16（数値はコード集計が権威）、
  ...
  ADR-0003（計画リポ）（不確実なら取引しない）、
  ADR-0011（計画リポ）（LLM モデル固定・基盤既定層の追随）
...
不正 JSON は `ReadFromJsonAsync` が `JsonException` を投げるため、外側の例外ハンドラ（伝送の失敗＝送信不可）に
落ちて区別できない。読み取りを個別に握って「応答不正」へ分ける（決定3 の切り分けを実際に成立させるため）。
...
### 決定4: 費用計測は「送信が成立したか（`Sent=true`）」だけで決め、本文の扱いより前に一度行う

拒否は `Sent=true` かつモデルへ実送信済み＝**課金が発生している**（上流も「拒否でも `Sent=true` を保つ＝課金集計の
意味を壊さない」と決定している）。空応答・上限到達も同様で、とくに上限到達は**思考トークンを消費し切って本文が空に
なる形で課金される**（IADR-0101）。したがって計測は本文の扱い（破棄するか否か）と独立させ、`Sent` 判定の直後に
一度だけ行う（計測点は egress 1 箇所のまま・IADR-0055。best-effort＝計測失敗は応答を壊さない・現行と同じ）。

これは現行からの挙動変更である（従来は本文が非空の成功応答のみ計測しており、拒否・空応答・上限到達の課金が計上漏れ
していた）。`Sent=false`（越境させておらず費用が発生していない）・非 2xx・例外は従来どおり計測しない。計上漏れは
NFR の費用統制（月次上限・#23）を過少評価させるため、送信が成立した応答は一律で計上する。
...
- 費用（NFR・#23）: 送信が成立した応答（拒否・空応答・上限到達を含む）のトークンが計上される（従来は計上漏れ）。
