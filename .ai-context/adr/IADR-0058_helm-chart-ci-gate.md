...
- 関連 ADR: [IADR-0049](./IADR-0049_integration-e2e-foundation.md)（実基盤 E2E は既定 CI から分離し `integration.yml` で実走する）
...
実基盤依存の検査は `integration.yml`（nightly / dispatch）が担うという既存の切り分け（[IADR-0049](./IADR-0049_integration-e2e-foundation.md) / #82）を
...
| `helm template --validate` / `--dry-run=server` | 実 API サーバ依存。[IADR-0049](./IADR-0049_integration-e2e-foundation.md) の切り分け（実基盤は integration.yml）に反し、既定 CI が不安定になる |
