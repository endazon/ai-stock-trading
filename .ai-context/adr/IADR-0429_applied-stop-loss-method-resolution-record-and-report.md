...
>   「見送り…の承認は内訳に含めていません」と括弧に書く。複数の手法が適用された日も S0〜S3 の間だけで数える。すべて見送りの月は空の内訳を出さない。**記録の無い承認もある月は
>   「照合できた承認では S0〜S3 のいずれも適用されませんでした（照合できた解決結果はすべて見送り）」と限定する**（不明を「適用なし」へ潰さない。PR #1008 の監査）。
> - **監査で揃えた書式（PR #1008）**: ⑤ 見送りの区分名と件数の間に半角空白を置かない（計画の書式 `…でない）<e> 件`。S0〜S3 は従来どおり空白あり）。
>   ⑥ 未知の手法の理由を計画の語「未知の値」に揃えた（「未知の手法の値のため S0 と同じ扱いにした」→「未知の値のため S0 と同じ扱いにした」）。
