...
> 🔴 ［2026-08-29 追記 / #564］**上の残余リスク（fail-open 側）は
> [IADR-0267](IADR-0267_information-degradation-state-heartbeat-and-fail-closed.md) で解消した。**
> 収集サービスが**毎巡回 1 件の現況観測**（`InformationSourceStateObserved`）を出し、受け側は
> **有効な観測が「止めるものは無い」と言っているときだけ新規建てを通す**（未観測・失効はいずれも
> 「不明」＝止める）。**本 ADR の決定 1〜4 はすべて生きている** —— 決定4「状態はプロセス内（非永続）」も
> 維持したままで、**永続化ではなく鮮度で「不明」を表現する**ことで解いた。
