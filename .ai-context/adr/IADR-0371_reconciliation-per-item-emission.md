...
- 🔴 **T-10-609 の 2 つ目の表明を反転させた**（既存テストの意図的な変更）。発行が落ちる巡回では
  **巡回サマリが出なくなる**（是正前は発行より先に出ていた）。サマリは「巡回が回りきって初めて言えること」であり、
  ...
  T-10-609 の中心的主張は変わらない。テストは `NotContain` で新しい事実を固定する。
  ［2026-09-23 追記 / PR #914 監査］ **当初ここに「末尾の `foreach` が黙って戻るのも同時に塞ぐ」と書いていたが、
  誤りである。** 監査が実測した: 明細のループを末尾へ戻しても T-10-609 も T-10-650 も緑のままだった
  （どちらも巡回が完走しないため末尾に到達しない）。**完走する巡回で 1 件につき 1 行であること**を
  T-10-653 で別に固定した（戻すと 4 行になり赤）。
- DI 登録の変更は無い（`Program.cs` 不変。常駐が自身を sink として渡すため）。
- 配備（Helm values）の変更は無い。

## テスト（T-10-646 〜 T-10-653）
