...
**不在が統制の有効を意味する形にしない**（IADR-0163 決定2 の規律）—— ここで既定を「止める」にすると、
未配線という*設定漏れ*が全経路の新規建てを止める。

### 決定2: 止めるのは Open だけ。判定は建玉効果が確定する地点に置き、LLM 呼び出しの前へ倒さない

`PositionEffectResolver.Resolve` へ `requireKnownHoldingForOpen`（既定 `false`＝従来どおり）を足し、
判断サービスが `_heldPosition.IsEnabled` を渡す。**決済（Close）の 2 分岐はこの判定より前に在り**、
