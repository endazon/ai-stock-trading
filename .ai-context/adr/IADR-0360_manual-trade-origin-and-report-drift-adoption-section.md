...
- 🔴 **期間で切った在庫の畳み込みそのものが持つ欠陥（[#892](https://github.com/endazon/ai-stock-trading/issues/892)）は
  本 IADR では直さない。** 決定 4 のクランプが塞いだのは「**取り込み**が在庫 0 に当たって新しく建てる」側である。
  **同じ根（報告書の在庫が期間で切られている）から出るもう 1 つの顔** ——「**システム自身の決済約定**が
  在庫 0 に当たって新しく建てる」—— は残っている（実測: 当期に前期建玉 100 株を 250 で決済し現在値 300 のとき
  評価損益 **−5,000**。取り込みが 1 件も無くても起きる）。
  🔴 **同じクランプでは直せない。** 取り込みは在庫を減らすだけの操作だが、**約定は建てる操作でもある**ため、
  「在庫 0 の売り」を一律に無視すると当期に始めた正当な新規ショートまで消える。区別には
  「**期間の開始時点の在庫**」という新しい入力が要り、計画（数値の定義）に触れ得る。**#892 で扱う。**
