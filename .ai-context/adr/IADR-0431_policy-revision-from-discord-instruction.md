...
- 新規は**当日（JST）の日報だけ**。土台は直近の確定済み日報（方針・`BasedOn`・`AssumptionsVersion` を引き継ぐ。数値を発明しない）。
  ...
  確定しても方針に効かない報告書を作らない）。
- 🔴 **営業日にまだ自動生成されていない当日の日報は作らない**（利用者裁定 2026-09-26・PR #1020 の監査）。作ると自動生成は既存の行を
  踏まない規則（IADR-0115 決定 3）でスキップされ、その日の数値入りの日報が失われる。境界（既定 16:00 JST）の前も、境界を過ぎて
  まだ生成されていないときも 409 で断り、「自動生成の後に /policy を実行すればそのドラフトを改訂する」と案内する。休場日（週末・
  構成の休場日）と自動生成が無効な構成では作ってよい（止める生成が無い）。判定は自動生成と同じ構成（`Reports:AutoGeneration`）を使う。
- 保存は読んだ時点の版で楽観排他を掛ける。LLM を待つ間に報告書が更新されたら（自動生成・別の改訂・確定）保存せず 409。
  **保存の後の提示が失敗しても 409 にしない**（保存と提示の間の並行更新。EF では例外にもなり得る）——409 は「何も保存していない」
  側の応答であり、保存済みの案を失敗と伝えてしまう。`presented=false`（確認ボタンなし）で返す。
...
- 応答の方針・理由・説明は、報告書サービスが Discord のメンション構文とマスクリンク `[表示](URL)` を**幅ゼロ空白の挿入だけ**で
  崩して返す（IADR-0116 決定 3 と同じ位置。`ReportSummarySanitizer.BreakMentions` ＋マスクリンク。素の URL は行き先がそのまま
  見えるので崩さない）。Bot はメンション抑止の出口（IADR-0359）で投稿する。
- 🔴 **表示される方針は、確定される（保存される）方針と、幅ゼロ空白の挿入を除いて完全に同一である**（PR #1020 の再監査）。
  正規化（制御文字の除去・CRLF→LF・前後の空白の除去）は検証（`PolicyRevisionProposalParser`）の 1 回だけで行い、保存するのは
  その結果である。表示では空行を畳まない・文字を落とさない・切り詰めない（当初は `ReportSummarySanitizer.Sanitize` を使っており、
  3 行以上の改行を畳み境界語を取り除いていた）。収集情報の境界語（`<<<UNTRUSTED_DATA` / `UNTRUSTED_DATA>>>`）を方針・理由・説明の
  どこかに含む出力は、検証が案全体を捨てる（取り除くと食い違い、素通しすると境界を偽装できるため）。
- 🔴 **確定される方針は全文を見せる（PR #1020 の監査 BLOCKING 1・ADR-0003）。** 報告書サービスは方針を切り詰めずに返し（上限は
  検証の 2000 文字）、Bot は ①見出し ②方針の全文（1 通 1800 文字を超えれば `【方針案 i/n】` の通に分割）③入れ替え案と説明、の順に
  送り、**確認ボタンは ③ にだけ**付ける。②を送れなければ ③（ボタン）は送らない。切り詰めてよいのは確定されない表示だけ
  （入れ替え案の理由を先に 60 文字へ、次に説明）。当初は 1 通に収めるため方針を残りの長さで切り詰めており、入れ替え案 10 件×
  理由 200 文字では方針が空のままボタンが出た（監査が実測）。
- `/policy` の period が書式外なら形式を案内する（「許可されていない」と読み違えさせない）。
- 送り順とボタンの位置は `PolicyRevisionReplySender` が持つ（Discord.Net の相互作用から切り出して試験する）。途中の通が送れなければ
  ボタンを出さず、「版 N は承認待ちのまま保存されている（まだ確定していない）。全文を見てから確定するには /policy で指示し直す。
  把握済みなら /report approve でも確定できる」と知らせる。承認待ちにできなかった案（presented=false）は報告書サービスの案内を
  見出しに添える。
...
- 自動生成が見送り続けて（依存先の障害）その日の日報が生成されないと、営業日は `/policy` で当日の日報を作れない
  （既存の未確定の報告書を period で指定すれば改訂できる）。裁定どおり、数値入りの日報を失うほうを避けた。
