...
- 応答の方針・理由・説明は報告書サービスが `ReportSummarySanitizer` を通し、Discord のマスクリンク `[表示](URL)` も崩して返す
  （IADR-0116 決定 3 と同じ位置。幅ゼロ空白の挿入だけで読める内容は変えない。素の URL は行き先がそのまま見えるので崩さない）。
  Bot はメンション抑止の出口（IADR-0359）で投稿する。
