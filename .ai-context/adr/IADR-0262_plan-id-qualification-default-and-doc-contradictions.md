...
**母集合の再確認（規則1: 誤りの文字列「新 ADR」で全文走査）**: **非凍結の母集合は 15 箇所 / 12 ファイル**
（全 23 箇所 / 16 ファイルから、凍結記録である `.ai-context/adr/`・`.ai-context/specs/` の 8 箇所 / 4 ファイルを
除外した数）。是正したのは `AGENTS.md:42` と `CLAUDE.md:105` の **2 箇所**で、残り 13 箇所は全件個別に
確認したところ**すべて正しい用法**であった（計画 ADR を指すべき文脈で計画 ADR を指していた。詳細は
作業仕様書の表を参照）。

> ［2026-08-28 追記 / #579 レビュー指摘］🔴 **当初ここには「12件ヒット」と書いており、これは誤りだった。**
> あわせて作業仕様書の据え置き一覧が `.github/PULL_REQUEST_TEMPLATE.md`（21・24 行目）を取りこぼしていた
> （用法は正しく是正不要だが、**「1 件ずつ確認した」と書いた母集合に入っていなかった**）。**本決定が
> 是正しようとしている「母集合の取り違え」と同型の記録漏れが、監査結果の記録自体に残っていた**という
> ことであり、数え方（何を分母にし、何を除外したか）を上のとおり明示する形へ改めた。
