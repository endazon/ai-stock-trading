...
- `OrderAmendmentService`: 訂正・取消の同期経路。moomoo 選択時はそもそも配線しない（実弾側に訂正・取消の口を作らない）。
  🔴 **［2026-09-19 追記 / [#847](https://github.com/endazon/ai-stock-trading/issues/847)］「moomoo 選択時はそもそも配線しない」は覆った**
  （IADR-0357 決定 2）——**取消**は全構成で配線され、利用者の手仕舞い取消から呼ばれる（訂正は従来どおり
  ペーパー専用）。ただし**本 ADR の判断は変わらない**: 取消は利用者の明示操作でしか動かず、
  通常運用の約定追跡には依然として使えない。むしろ逆で、**取消の結果が確認できないときの本当の終端は
  本 ADR のポーリングが運ぶ**（IADR-0357 決定 3）。
