...
🔴 **この「1 つも無い」の根拠は、API 名を記憶で列挙した走査ではない。** 起案時の初版は記憶で 17 種を
...
- **実 Discord への投稿では検証していない。** 固定したのは電線に載る形（Webhook は
  `"allowed_mentions":{"parse":[]}`、Bot は `{"parse":[],"roles":[],"users":[],"replied_user":false}`）である。
