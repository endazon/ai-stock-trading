...
3. **武装から 24 時間を超えて約定した S0 は数えられない（既存の欠落）。** 約定追跡（`OrderFillPoller`）は
   ...
   ［2026-09-25 追記 / #958］是正した（[IADR-0406](IADR-0406_s0-stop-leg-fill-tracking-beyond-window.md)）。保護記録が Active な S0 のレグは
   追跡上限の対象外とし、ガードは S0 のレグの終端を観測したら保護記録を完了させる前にレグの記録の追跡の起点を観測の時刻へ進める。
