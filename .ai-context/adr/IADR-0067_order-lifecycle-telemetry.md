...
- フォローアップ:
  ...
  - Risk 側射影 Consumer の到着順序: MassTransit は異なるイベント型間の順序を保証せず、稀に約定・訂正・取消が
    承認の射影より先に処理されると落ちる。実運用の順序では承認が先行し（既存 IADR-0018 の AppendFill と同じ前提）、
    かつ最小標本ガード付きの窓集計のため単発の取りこぼしは無嫌疑側（fail-safe）に倒れるため許容する。厳密な順序保証が
    要るなら承認欠如時のバッファリングを検討する（過剰実装を避けるため本 PR では持たない）
