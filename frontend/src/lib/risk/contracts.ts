...
export interface RiskStatusView {
  ...
  //
  // 利用者が証券会社のアプリから直接売買した結果を、取引台帳へ取り込んだ件数である。
  // 🔴 これらの取り込みは**実現損益を記録していない**（不明であって 0 ではない）。したがって
  // 当日損益・基準資金は実際の口座とずれ得る。**件数を出す目的は、そのずれを画面から知る手段を与えること**である。
  // 🔴 **0 は「取り込みが無かった」という事実**であり、未供給ではない（台帳は常に読める）。
  // 本画面から取り込みは行わない（窓口は owner 権限の API）。
  driftAdoptionCountToday: number;
}
