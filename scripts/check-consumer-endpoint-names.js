 ...
 *
 * 🔴 **本検査器はパスだけを見て、名前空間を一切見ない。** `serviceNameConstantIn` が読むのは
 * 名前空間ではなく `ServiceName` 定数の**文字列リテラル**であり、`wiringOf` /
 * `forbiddenTopologyCallsIn` が見るのは API 呼び出し名である。したがって名前空間の改名は
 * 本検査器に影響しない —— この性質は模擬ツリーで実証してある。
 *
 * 🔴 **旧記述の訂正（IADR-0261）。** ここには「AST は移送では `namespace` 宣言を 1 行も変えない
 * （＝パスは新樹形・名前空間は据え置きという AST 固有の組み合わせ）」と書いてあったが、
 * **その方針は撤回された**（IADR-0259 決定 5 が撤回・確定し、IADR-0261 が実施した）。
 * AST のサービスのルート名前空間は基盤 MSP と同じ `<Name>Service` であり、
 * EF の `ModelSnapshot` が持つ FQN 文字列も同時に書き換え済みである
 * （`Foundation` / `Composable` の名前空間セグメントも除去した。フォルダ名は据え置き）。
 *
