...
#   OPEND_ENABLED（true / false）で切り替える。**未設定なら前回リリースの値を引き継ぐ**（#626, IADR-0275）。
...
# #626, IADR-0275: `helm upgrade --install` は `--reuse-values` を使わない（-f values-local.yaml との
...
# #626, IADR-0275: 前回リリースの値を個別に引き継ぐ Helm values のキー定義: <top>.<nested>|<環境変数>。
  ...
  if [ -n "$clobber" ] && [ "$FORCE_EMPTY_VALUES" != "1" ]; then
    {
      ...
      echo "       空で上書きすると前回の設定を失うため中断しました（#626 / IADR-0275）:"
      printf '%s' "$clobber" | sed 's/^/         - /'
...
echo "==> [3/4] broker.tier / opend.enabled (前回リリースの値を引き継ぐ・#626 / IADR-0275)"
resolve_ast_value_overrides
