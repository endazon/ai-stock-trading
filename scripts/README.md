...
| `list-test-projects.js` | テストプロジェクトを**命名ではなく内容**（`Microsoft.NET.Test.Sdk` / `xunit` への参照）で発見し、パス順に**ラウンドロビン**でシャードへ分配する（IADR-0208 決定 10）。`--shard <i> --of <n>` / `--count` / `--slnx <path>`（シャード用 solution を生成）。**塊で切らない** —— パス順に並ぶため塊で切ると同じサービスが 1 シャードへ固まり所要が偏る。🔴 **0 件なら失敗させる**（「テストが 1 本も無い状態で緑」を作らない）。solution を生成するのは、1 プロジェクトずつ `dotnet test` を回すと**1 回あたり約 4.9 秒の起動コスト**（実測で較正）が本数だけ積み上がり、51 本で**約 250 秒**の無駄になるためである。`--self-test`（14 件。**分配の網羅性と排他性**、生成 XML の妥当性・件数保存・escape を固定） | 標準出力（一覧） |
