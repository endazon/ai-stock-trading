...
| `k8s-local-deploy.sh` / `k8s-local-deploy.test.sh` | ローカル k8s へのデプロイ（image import → `ast-secrets` 同期 → `broker.tier`/`opend.enabled`/`discord.bot.*` の前回値引き継ぎ → helm upgrade → OpenD を除く Deployment への `kubectl rollout restart`）と、その Bash テスト（kubectl/helm スタブ・実クラスタ不要。#263 / #626 / #673） | — |
