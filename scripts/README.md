...
| `check-proto-contracts.js` | **east-west gRPC の proto 契約**（`backend/Shared/<Project>/Protos/<unit>/<service>/v<N>/*.proto`）の**配置・versioning 規約と後方互換**を検査する（NFR / MSP:ADR-0029 / MSP:ADR-0075 / IADR-0328 決定 4 / IADR-0331 決定 5 / #745）。🔴 **proto の破壊的変更はコンパイルで止まらない** —— フィールド番号を付け替えても両側が再生成されるのでビルドは緑のまま通り、**古いピアだけが黙って別のフィールドを読む**。規約は **R1**（置き場とパス・`package`・`csharp_namespace` の一致。`<unit>` は `aistocktrading`）／**R2**（番号は message 内で一意・1..536870911・19000..19999 は不可）／**R3**（自分の `reserved` の番号・名前を再利用しない）／**R4**（`syntax = "proto3"`）。後方互換は baseline（`proto-contract-baseline.json`）との比較で、**非破壊の追加でも差分がある限り exit 1**（スナップショット試験。`--update` で差分を PR のレビュー対象に載せる）。破壊的変更は `proto-breaking-allowlist.json` の承認エントリ（`key`/`reason`/`approvedBy`/`issue`/`date` すべて必須）で通すが、🔴 **フィールド削除時の `reserved` 不在と `reserved` の再利用は承認でも通らない**。**0 件走査では緑を返さない**（走査の基点を取り違えた「沈黙の exit 0」を作らない）。**基盤（microservices-platform）の同名スクリプトからの移植**であり、判定ロジックは同一・走査の基点（`src/<unit>/backend/` → `backend/`）とユニット除外の撤去だけが違う（乖離は受容する）。**見ないもの**: `import` 先の型の解決・`extend`・option の意味論・protoc が生成する C# の API 差分。`--self-test`（41 件） | 標準出力（レポート） |
