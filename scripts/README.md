...
| `helm-release-drift.js` | **稼働中の helm リリースと、チャートを描画した結果の差（設定の未反映）を出す**（NFR / #1022 / IADR-0439。手順は chart README「配備の前後でリリースとチャートの差を確かめる」）。`helm get manifest` と、`helm get values` を与えた `helm template`（`--values` で values-local.yaml 等を後ろに重ねる）を比べ、追加・削除・変更された資源と、ワークロードのコンテナごとの env のキーの追加・削除・変更・image の変更を出す。**OpenD の Deployment が変わるかを必ず 1 行で示す**。🔴 **読み取り専用**（helm は `get manifest` / `get values` / `template` / `version` しか呼ばず、それ以外は例外で拒む。kubectl は呼ばない）。🔴 **秘密を出さない**（manifest の行を出さない・Secret は中身を出さない・secretKeyRef は参照先ごと伏せる・機密らしい名前の平文と資格情報入りの URL も伏せる・リリースの values は 0600 の一時ファイルにだけ書いて消す）。`--live`/`--rendered` で 2 つのファイルを比べる。`--self-test` は同梱の fixture（`scripts/fixtures/helm-release-drift/`）だけで走り、helm もクラスタも使わない（`scripts.repo.test.js` 経由で CI の `scripts-tests` が走らせる） | 標準出力。終了コード 0 差なし／1 差あり（OpenD は不変）／3 差あり（OpenD の Deployment が変わる）／2 使い方の誤り・helm の失敗 |
