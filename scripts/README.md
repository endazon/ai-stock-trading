...
| `check-planning-adr-range.js` | **週次バックログ監査の項目 6（計画 ADR レンジ鮮度）を Claude ステップの前に決定的に解決する**（NFR / #717 / #710）。`.claude/rules/traceability.repo.md` の宣言 `ADR-0001..NNNN`（`lib/plan-ranges.js`）と、計画リポジトリ `projects/ai-stock-trading/07_adr/` の実在最大番号（`gh api …/contents`。env `PLANNING_REPO_TOKEN` を子プロセスの `GH_TOKEN` へ写す。`GITHUB_TOKEN` では 404 になることが実測済み）を突き合わせ、`--out <path.json>` に `status: ok / behind / ahead / unverified` と `reason` を書く。**常に exit 0**（fail-open。secret 不在・API 失敗・宣言不読はいずれも `unverified` と理由。項目 6 の検証不能で他 5 項目を巻き込まない）。`behind` / `ahead` / `unverified` は CI アノテーション（warning）にも出す。`--self-test`（9 件）は実ネットワークを叩かない | 標準出力（判定）・JSON ファイル |
