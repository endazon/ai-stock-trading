...
| `lib/git-read.js` | 検査器共通（#1009）。git の出力を読む exec に渡す上限 `GIT_MAX_BUFFER`（256 MiB。既定の 1 MiB では索引 README 1 枚で `ENOBUFS` になる）と、読み取り失敗を「浅いクローンのため辿れない＝設計どおり skip してよい」と読めるかの判定 `isShallowSkip`（`git rev-parse --is-shallow-repository`。**`ENOBUFS` は常に false**、浅さを判定できなければ false）。利用は `check-adr-index-addendum-loss.js` / `check-adr-index-sync.js` / `check-commit-messages.js`（判定）と `gen-changelog.js`（上限のみ） | — |
...
| `check-adr-index-addendum-loss.js` | `.ai-context/adr/README.md` の**索引行**（`IADR-XXXX` を第 1 セルに持つテーブル行）に在った**日付つき追記ブロック**（`［YYYY-MM-DD 追記 …］`）が消えていないかを検査する（NFR-01 / #875 / IADR-0363）。🔴 **索引行は 1 行が数千文字あり、行の一部が消えても `git diff` は「1 行削除・1 行追加」としか出さない** —— 差分としては読めず、`check-adr-index-sync.js`（索引行が**変更**されたかしか見ない）も捕まえない。**追加条件は「同型の事故 2 回」で、着手時点で 2 回に達していた**（IADR-0210 ← ［2026-09-18 追記 / #820］ が最長共通接頭辞での解決で切断、IADR-0118 ← ［2026-09-19 追記 / #849］ が develop の取り込みで丸ごと消失）。🔴 **書いた検査器を develop の履歴へ当てたら 3 件目が出た**（IADR-0327 ← ［2026-09-11 追記 / #743］。develop に現存・復元は #886）——「2 回」は着手の条件であって被害の総量ではない。**印の単位の多重集合**（行ごと。同じ IADR 番号の行が複数あれば、合算せず**行ごとの出現回数の最大値**を採る＝重複を 1 行へ畳む是正を消失と数えない。#955 / IADR-0400）で `base`（マージベース）・`theirs`（統合ブランチ先端）・`ours`（HEAD）を突き合わせ、(1) base に在って ours に無い印、(2) **我々が触った行**で theirs に在って ours に無い印 を error にする。触っていない行は theirs を要求しない（3-way マージで統合ブランチ側が採られるため。偽陽性は検査を外させる）。🔴 **「触った」は `ours` を起点に読む** ——「マージベースに行が無い」は**統合ブランチ側が足した**場合にも起きるので、これを「我々が触った」と読むと索引行が develop に 1 本増えるたびに在庫の PR が全部赤になる（初版がこれを踏み、開いている 10 本のうち 4 本を赤にした）。**衝突を「マージベースの本文へ戻して」解決した形は見逃す**（規則 3 を緩めると上の偽陽性が戻るため、意図して受け入れた。IADR-0363 決定 6）。Markdown の明示リンク `[#N](url)` は短縮形 `#N` へ正規化してから突き合わせる（規約どおりの是正を止めない）。`--range=A..B`（2 ドット）は厳格モード（A の印をすべて要求。事故の再現用）、`--range=A...B` / 既定は 3 ドット。意図的な撤去はコミット本文へ `[remove-adr-addendum] <IADR> <印>` を**行として単独で**書いて 1 件ずつ宣言する（**全体スキップは用意しない**。名指しは `notice`、`*`〔その行の全印を解放〕は**同じ行の他の消失を巻き込んで隠す**ため `warn` へ格上げ）。🔴 **規則 4（重複）**: HEAD（手元では作業ツリー）の索引に**同じ IADR 番号の索引行が 2 本以上**あれば error（状態の検査。衝突を両側の行を並べる和集合で解くと生じる。IADR-0354・IADR-0369 の実例は PR #954 で是正。逃げ道なし。範囲を決められなくても作業ツリーで検査する。#955 / IADR-0400）。射程は**索引行だけ**（IADR 本体・`docs/` の trace ブロックは別途）。🔴 **読めなかったら赤**: 版を読めなかったとき skip（warn・exit 0）するのは**浅いクローンの場合だけ**で、それ以外（`ENOBUFS` を含む）は error（#1009。索引 README が 1 MiB を超えた 909241f5 から、既定 `maxBuffer` の `ENOBUFS` を skip として緑を返していた）。`--self-test` あり | 標準出力（レポート） |
| `check-realm-export.js` | `infra/keycloak/realm-export.json` の client / role の description・realm attributes・name 系が Keycloak の varchar(255) を超えていないか（超えると import が SQLSTATE 22001 で失敗し Keycloak が起動しない。#787・基盤 MSP#18 と同型）。`--self-test` あり | CI（static-checks） |
...
| `check-consumer-endpoint-names.js` | サービスを跨ぐ RabbitMQ キュー名の衝突を検査する（ADR-0013 / IADR-0129。前身の MassTransit 規則は IADR-0106）。不変条件は **N1**（`ServiceName` 定数がサービス跨ぎで一意）／**N2**（トポロジの直接指定の禁止）／**N3**（Wolverine 配線は必ず共通ヘルパ経由）と、メタ検査 **M1**（走査サービス数 ≥ 11）／**M2**（Wolverine 配線サービス数 ≥ 10）／**M3**（走査した本番 `.cs` ≥ 550。IADR-0258 決定 3）。🔴 **M1 と M3 は同じことを測っていない** —— M1 は「1 ファイルでもマッチしたサービス」を数えるため、**パス判定が痩せて各サービス 1 ファイルしか当たらなくなっても M1・M2 は通り、N2 だけが静かに空振りする**。走査対象の判定（`isProductionServiceFile`）は**許可リストではなく除外リスト**であり、「テストであるか」だけを見て落とす（IADR-0258 決定 2）—— 許可リストは**知らない階層を黙って捨てる側へ倒れ、その捨て方は緑で報告される**。これにより現行構成（`<Svc>/src/<Svc>.Api/**`）と VSA 統合後（`<Svc>/**`・テストは `<Svc>/Tests/**`）が同じ規則で分かれる。`CONSUMER_ENDPOINT_NAMES_ROOT` で任意のツリーを走査できる（模擬ツリーでの実証用）。`--self-test`（29 件） | 標準出力（レポート） |
| `check-observability-assets.js` | **業務メトリクスとダッシュボードの乖離**を検査する（NFR-07 / #287 / IADR-0255）。🔴 **乖離はエラーを出さない** —— 系列名がずれたパネルは空のグラフを描き、**空のグラフは「異常が起きていない」と読める**。監視しているつもりで何も見ていない状態が気付かれずに続くため、機械で止める。見るのは **D1/D2/D3**（ダッシュボード JSON が妥当・各パネルが `expr` を持つ・`uid` が一意）／**D4/D5**（同一ダッシュボード内のパネル `id` が一意・`gridPos` の矩形が重ならない。#939）／**A1/A2**（アラートの形〔PascalCase の名前・空でない `expr`・既知の `severity`・`summary`・**`for:` が正の期間か、直前のコメントに「for は置かない」の印**〕と `expr` が引く系列の実在。`expr` はブロック／折り畳みスカラーも本文まで読む）／**R1**（`expr` が引く `ast_*` 系列がコード側のレジストリ `BusinessMetricNames.cs` に実在する）／**R2**（レジストリの各計器を少なくとも 1 枚が引いている＝**宣言はあるが誰も見ない計器**を検出）／**E1**（dev の otel-collector が metrics を `debug`＝標準出力のみにしか出さない＝**既定では計装が有効でも外部へ送らない**）／**M1**（ダッシュボード数・計器数の下限。探索が壊れて 0 件になると無条件に緑になる）。コード名 → Prometheus 名の変換規則は「ドットを `_` へ」＋「Counter は `_total`」＋「Histogram は `_bucket`/`_count`/`_sum`」の 3 つで閉じる（**計器に `unit` を与えない**ことでこの規則を 1 本に保っている）。**見ないもの**: Prometheus に実際に系列が出ているか（実バックエンド依存・射程外）、クエリが意味的に正しいか（人が読む）。`--self-test`（37 件） | 標準出力（レポート） |
| `check-frontend-empty-frames.js` | `frontend/src` 配下に「`.gitkeep` のみのディレクトリ」が無いことを検査する（MSP/ADR-0069 決定 5 / #663）。計画 ADR-0069 は MSP/ADR-0065 決定 4（バックエンド 8 要素標準の `.gitkeep` 枠置きの撤回）と同じ理由（**枠が「適合の見え方」を作る**）がフロントエンドにも及ぶと定め、「`.gitkeep` のみのディレクトリを置かない」を決定 1 とした。本リポジトリは #529（`IADR-0290` 決定 2）で実体の無い区分に枠を置く運用を採っていたが、`MSP/ADR-0069` が同決定 2 を覆したため 17 件の枠を撤去し、再発を本検査器で止める。**検査する述語は 1 つだけ**（`MSP/ADR-0069` 決定 5 の指定どおり）——各区分の実体の有無・feature 内部分割の充足はこの検査の対象にしない。判定は**葉ディレクトリ**（子ディレクトリを持たないディレクトリ）に絞り、直下のファイルが `.gitkeep` だけ、または 0 件であれば枠とする。子ディレクトリを持つ祖先は自分自身を判定せず、**最も深い、具体的な枠ディレクトリだけを報告する**。`FRONTEND_EMPTY_FRAMES_ROOT` で任意のツリーを走査できる（自己試験・模擬ツリーでの実証用）。`--self-test`（9 件） | 標準出力（レポート） |
| `check-proto-contracts.js` | **east-west gRPC の proto 契約**（`backend/Shared/<Project>/Protos/<unit>/<service>/v<N>/*.proto`）の**配置・versioning 規約と後方互換**を検査する（NFR / MSP:ADR-0029 / MSP:ADR-0075 / IADR-0328 決定 4 / IADR-0331 決定 5 / #745）。🔴 **proto の破壊的変更はコンパイルで止まらない** —— フィールド番号を付け替えても両側が再生成されるのでビルドは緑のまま通り、**古いピアだけが黙って別のフィールドを読む**。規約は **R1**（置き場とパス・`package`・`csharp_namespace` の一致。`<unit>` は allowlist ＝ `aistocktrading`〔自リポ所有〕/ `platform`〔基盤所有の契約の写し。IADR-0332 / #746〕。**allowlist であって「何でも通る」ではなく**、綴り誤り・所有者不明の契約の持ち込みは落とす）／**R2**（番号は message 内で一意・1..536870911・19000..19999 は不可）／**R3**（自分の `reserved` の番号・名前を再利用しない）／**R4**（`syntax = "proto3"`）。後方互換は baseline（`proto-contract-baseline.json`）との比較で、**非破壊の追加でも差分がある限り exit 1**（スナップショット試験。`--update` で差分を PR のレビュー対象に載せる）。破壊的変更は `proto-breaking-allowlist.json` の承認エントリ（`key`/`reason`/`approvedBy`/`issue`/`date` すべて必須）で通すが、🔴 **フィールド削除時の `reserved` 不在と `reserved` の再利用は承認でも通らない**。**0 件走査では緑を返さない**（走査の基点を取り違えた「沈黙の exit 0」を作らない）。**基盤（microservices-platform）の同名スクリプトからの移植**であり、判定ロジックは同一・走査の基点（`src/<unit>/backend/` → `backend/`）とユニット除外の撤去だけが違う（乖離は受容する）。**見ないもの**: `import` 先の型の解決・`extend`・option の意味論・protoc が生成する C# の API 差分。`--self-test`（42 件） | 標準出力（レポート） |
...
| `validate-runtime-scaffold.js` | 実行環境スキャフォールド（docker-compose / appsettings / `.env.example`）の静的検査（IADR-0048）。ホストプロジェクトの所在（`hostDir()`）は **3 通り**を見る —— `<Svc>/src/<Svc>.Api`（標準構成）／`<Svc>/src/<Svc>.Worker`（旧）／`<Svc>`（**VSA 統合後**。IADR-0258 決定 1）。**3 つとも無ければ従来どおり `.Api` の名前で報告する**（失敗メッセージの読み方を変えない）。`RUNTIME_SCAFFOLD_ROOT` で任意のツリーを検査できる —— 実ツリーは候補 1 でしかヒットしないため、**統合後の候補は模擬ツリーでしか実証できない** | 標準出力（レポート） |
...
| `detect-changed-areas.js` | 変更ファイル一覧から **backend のビルド・テストを走らせる必要があるか**を判定し、`$GITHUB_OUTPUT` へ `backend=true|false` を書く（IADR-0208 決定 11）。🔴 **既定は「走らせる」で、許可リストに全件が収まるときだけ skip する。** 誤りの倒れ方が非対称だからである —— 余計に走らせる誤りは遅いだけで**気付ける**が、**走らせない誤りは退行を無検証でマージし、緑のまま誰も気付かない**。未知のパス・変更 0 件（＝差分の取得に失敗した可能性）はいずれも走らせる側へ倒す。🔴 **`.github/workflows/**` は許可リストに入れない**（CI 自身を変える PR で skip するとその変更を検証できない）。**`.editorconfig` も走らせる** —— `backend` フラグ 1 本で `backend-test` と `lint` の両方を決めており、`dotnet format --verify-no-changes` はこの入力にまさに反応するためである。**`--git`** で `HEAD^1..HEAD` の差分取得まで自分で行う（CI の各ジョブはこの 1 行でゲートできる。`pull_request` では `refs/pull/N/merge` が checkout されるため `HEAD^1` が base、`push` では直前のコミットで、**分岐なしに両方を賄える**。`fetch-depth: 2` で足りる）。**差分が取れなくても例外を投げず空を返す** —— 速さのための判定を門に化けさせない。判定は `^backend/` 前置だけを見るため VSA 統合後のパス形（`backend/Services/<Svc>/Program.cs` 等）にも**無改修で当たる**が、その事実は自己試験で固定してある（IADR-0258）。`--self-test`（30 件） | 標準出力＋`$GITHUB_OUTPUT` |
| `list-test-projects.js` | テストプロジェクトを**命名ではなく内容**（`Microsoft.NET.Test.Sdk` / `xunit` への参照）で発見し、**重み付き LPT（Longest Processing Time first）貪欲**でシャードへ分配する（IADR-0208 決定 10 / IADR-0257）。`--shard <i> --of <n>` / `--count` / `--weights` / `--slnx <path>`（シャード用 solution を生成）。**塊で切らない** —— パス順に並ぶため塊で切ると同じサービスが 1 シャードへ固まり所要が偏る。**ラウンドロビンでも足りない** —— 重みを見ないため重いプロジェクトが偶然 1 本へ寄ると残りが遊ぶ（実測: 51 本を 4 分割したとき旧ラウンドロビンの makespan は 1,107〔全体の 32.4%〕、LPT は 854〔25.0%。理想 853.5 の 1.001 倍〕）。🔴 **重みは JSON へ持たず毎 run 走査で導く**（各テストプロジェクト配下の `.cs` の `[Fact` / `[Theory` の出現数。読めなければ 1 へ倒す fail-open）—— コミットした重みは必ず実体とずれ、**ずれても誰も気付かない**。🔴 **0 件なら失敗させる**（「テストが 1 本も無い状態で緑」を作らない）。solution を生成するのは、1 プロジェクトずつ `dotnet test` を回すと**1 回あたり約 4.9 秒の起動コスト**（実測で較正）が本数だけ積み上がり、51 本で**約 250 秒**の無駄になるためである。`--self-test`（31 件。**分配の網羅性と排他性**、LPT の決定性・重い順の積み方・makespan の最悪保証・重み走査の fail-open、生成 XML の妥当性・件数保存・escape を固定） | 標準出力（一覧） |
| `summarize-test-failures.js` | `dotnet test` が残した TRX から**実際に落ちたテスト**の完全修飾名・メッセージ・スタックトレースを取り出し、ジョブログ末尾と `$GITHUB_STEP_SUMMARY` へ書く（NFR / #596 / IADR-0277）。🔴 **並列実行のログは読み違える** —— `dotnet test <solution>` は各 VSTest のコンソールロガーが同じ標準出力へ同期せずに書くため、サマリ塊が他プロジェクトの逐次出力に割り込まれて分断される。実測では `Test Run Failed.` が `Total tests:` と `Passed:` の間へ入り込み、**ログ末尾に最も近いサマリは別プロジェクトの `Test Run Successful.`** だった。その結果「テスト失敗 0 件なのに exit 1」と読まれ、テストホストの異常終了を疑う issue が立った（実際は 3 観測すべてが名前の付いたアサーション失敗）。TRX は構造化データなので割り込みの影響を受けない。🔴 **TRX を 1 件も読めなければ落ちる**（0 件検査で「失敗テストなし」と報告して緑にするのは、この誤診を機械で再生産する形である）。🔴 **TRX は読めたが失敗 0 件のときだけ**「ホスト異常終了・ビルド失敗・後続ステップの失敗を疑え」と次に見る成果物（blame の `Sequence.xml` / dump・`--diag` の診断ログ）を名指しする —— これが本物のホスト異常終了の形である。壊れた TRX は**読み飛ばさず** malformed として報告し、**失敗 0 件でも「一部が不明」と案内する**（「0 件」と読ませない）。`--self-test`（15 件） | 標準出力＋実行サマリ |
| `k8s-local-deploy.sh` / `k8s-local-deploy.test.sh` | ローカル k8s へのデプロイ（image import → secret 供給〔`AST_ESO`: ESO 所有＝作らない・事前確認と管理外 Secret での中断〔`--adopt-existing-secrets` で警告に下げる〕／従来経路＝`ast-secrets` 同期〕→ `broker.tier`/`opend.enabled`/`discord.bot.*`〔ESO 所有では渡さない〕の前回値引き継ぎ → helm upgrade → OpenD を除く Deployment への `kubectl rollout restart`）と、その Bash テスト（kubectl/helm スタブ・実クラスタ不要。#263 / #626 / #673 / #795） | — |
| `cutover-count-reconcile.sh` / `cutover-count-reconcile.test.sh` | **再実装版への切替（#346）の前後で保全必須データが 1 行も欠けていないことを突き合わせる**（FR-11 / NFR-09 / NFR-10 / IADR-0287。手順は `docs/migration/20260903_cutover-and-retention.md`）。`snapshot <out.tsv>` が 7 サービス DB の全 38 テーブルで `件数 / 時刻列 min / max / 未確定件数 / 内容指紋`（各行 md5 の昇順連結の md5＝行順非依存）を採り、`compare <before> <after>` が 2 つの TSV だけを読んで欠損・件数差・min/max 差・指紋差・**未確定予約（`order_dispatch_reservations.State=0`）の減少**を FAIL（exit 1）にする。after にだけあるテーブル・`__EFMigrationsHistory` の増加は NOTE（新スキーマ適用は正常）。`controls` は統制状態と切替前チェックの現在値 29 項目を key/value で出す（意味づけは移行仕様書）。🔴 **母集合は manifest（`AST_CUTOVER_MANIFEST`）に明示列挙し、実行時に `pg_tables` と双方向照合して片方にしか無ければ部分出力せず exit 2**（部分的なスナップショットは「全数」と読み違えられる）。manifest と EF ModelSnapshot の腐りは `scripts.repo.test.js` が CI で止める。**SELECT しか発行しない**（テストがスタブの全 SQL 記録から書き込み語の不在を検査）。接続は `AST_PSQL`（例 `kubectl -n platform-infra exec -i deploy/postgres -- psql -U ai`）、リハーサル用コピーは `AST_DB_PREFIX=cutover_rehearsal_`。🔴 `run_sql` は stdin を `/dev/null` へ落とす（`kubectl exec -i` 越しの psql が呼び出し元 while ループの stdin を飲み込み、母集合の残りを黙って読み飛ばす）。Bash テスト 47 件（psql スタブ・実 DB 不要・`AST_CUTOVER_LIB=1` で source） | TSV / 標準出力 |
| `host-liveness-monitor.ps1` / `host-liveness-monitor.test.ps1` | **ホスト（Windows）で 5 分ごとに走らせる、クラスタの外からの死活監視**（FR-10 / #937 / IADR-0344 追記（17）。登録・手順は `docs/operations/host-liveness-monitor-runbook.md`。**登録はオーナーが行う**）。PowerShell 7。米国株の通常取引時間（共有カーネル `MarketHours` の規則の写し）だけ、Kubernetes API（`/readyz`）・市場監視と発注執行の Pod の Ready・2 つの生存要約（「損切り評価は稼働中」「S1）の保護は継続中」）の鮮度を**読むだけ**で判定し、Windows の通知・任意の Discord Webhook（環境変数 `AST_HOST_LIVENESS_DISCORD_WEBHOOK`・値は表示しない）・ログファイルで知らせる。遷移で鳴らし・警報は 30 分ごとに鳴らし直し・回復を 1 回。要約が無いことを「保有なし」と読まない。`-DryRun -IgnoreMarketHours` で試走。テストは純ロジックと kubectl を関数で差し替えた観測の層・本体（T-10-1040〜1049・`AST_HOST_LIVENESS_LIB=1` で dot-source） | 状態 / ログ（`%LOCALAPPDATA%/ai-stock-trading/host-liveness`） |
...
| `helm-release-drift.js` | **稼働中の helm リリースと、チャートを描画した結果の差（設定の未反映）を出す**（NFR / #1022 / IADR-0439。手順は chart README「配備の前後でリリースとチャートの差を確かめる」）。`helm get manifest` と、`helm get values` を与えた `helm template`（`--values` で values-local.yaml 等を後ろに重ねる）を比べ、追加・削除・変更された資源と、ワークロードのコンテナごとの env のキーの追加・削除・変更・image の変更を出す。**OpenD の Deployment が変わるかを必ず 1 行で示す**。🔴 **読み取り専用**（helm の引数は**許可した形だけ**で組む＝`get manifest` / `get values` / `template` / `version` と `-n` / `-o yaml` / `-f` / `--is-upgrade` / `--no-hooks` / `--skip-tests` / `--short`。それ以外・値が `-` で始まるものは例外。利用者の与える `--release` / `--namespace` / `--chart` / `--values` も書式外・`-` 始まり・URL・存在しないパスを拒む。kubectl は呼ばない。kube の接続先は `KUBECONFIG` / `HELM_KUBECONTEXT` で選ぶ。helm の実行ファイルは `--helm` / `HELM_RELEASE_DRIFT_HELM` で運用者が選べる）。🔴 **秘密を出さない・迷ったら伏せる**（manifest の行を出さない・Secret は中身を出さない・secretKeyRef は参照先ごと伏せる・平文でもキー名を語に分けて機密らしい語を含むもの、URL の userinfo・`/webhooks/<id>/<token>`・クエリの鍵・トークンらしいパス要素・接続文字列の `Pass=` / `Pwd=` / `Password=` / `AccountKey=`・トークンらしい長い値を伏せる。リリースの values は OS の一時ディレクトリに実行中だけ置き、終了・失敗・SIGINT / SIGTERM / SIGHUP で消す。`mode 0600` は POSIX でだけ効く）。`--live`/`--rendered` で 2 つのファイルを比べる。`--self-test` は同梱の fixture（`scripts/fixtures/helm-release-drift/`。資格情報の見張り値は `credential-cases.js` が連結で組む）だけで走り、helm もクラスタも使わない（`scripts.repo.test.js` 経由で CI の `scripts-tests` が走らせる） | 標準出力。終了コード 0 差なし／1 差あり（OpenD は不変）／3 差あり（OpenD の Deployment が変わる）／2 使い方の誤り・helm の失敗 |
| `opend-build.sh` | moomoo OpenD コンテナのビルド | — |
| `e2e-local-infra.sh` | 実コンテナ統合 E2E 用のローカル基盤起動 | — |
| `check-planning-adr-range.js` | **週次バックログ監査の項目 6（計画 ID レンジ鮮度）を Claude ステップの前に決定的に解決する**（NFR / #717 / #710 / planning#591）。`.claude/rules/traceability.repo.md` の宣言（FR/UC/SC は `check-test-traceability.js` の `readPlanIds()`、ADR は `lib/plan-ranges.js`）と、計画リポジトリが公開する `tools/doc-checks/kg-ranges.json`（実物からの導出結果。planning の ADR-0093 決定 1。`gh api …/contents` を `Accept: application/vnd.github.raw` で 1 回取得。env `PLANNING_REPO_TOKEN` を子プロセスの `GH_TOKEN` へ写す。`GITHUB_TOKEN` では 404 になることが実測済み）を **FR / UC / SC / ADR の 4 種**で突き合わせ、`--out <path.json>` に `status: ok / behind / ahead / unverified`・`reason`・**`scanned`（突合できた種別数）**・`ranges`（種別ごとの内訳）を書く。🔴 **`scanned: 0` は「ずれが無い」ではなく「検査が動いていない」である**（planning ADR-0093 決定 3）。**常に exit 0**（fail-open。secret 不在・API 失敗・宣言不読はいずれも `unverified` と理由。項目 6 の検証不能で他 5 項目を巻き込まない）。`behind` / `ahead` / `unverified` は CI アノテーション（warning）にも出す。`--self-test`（14 件・陽性対照を含む）は実ネットワークを叩かない | 標準出力（判定）・JSON ファイル |
| `check-backlog-audit-output.js` | **週次バックログ監査（`backlog-audit.yml`）が run success でも結果 issue（#483 相当。タイトル `chore(NFR): バックログ定期監査の結果`）を更新できていない事象を検出する**（NFR / #711）。`--run-start <ISO8601>`（または env `RUN_START_TIME`）で渡された run 開始時刻より `gh issue view <n> --json updatedAt` の `updated_at` が新しいかだけを見る（本文の質は見ない）。同時刻は**不合格側に倒す**（見逃す方向より誤って fail する方向を安全とみなす）。issue 不在・`gh` 失敗・run 開始時刻が渡されていない場合はいずれも終了コード 1（fail-loud——「今週は指摘なしだった」と「今週は黙って落ちた」を区別できない状態を再生産しないため）。`gh` 呼び出しは差し替え可能（`fetchIssue(issueNumber, execFn)`）にしてあり、`--self-test`（9 件）は実ネットワークを叩かない。env `EXECUTION_FILE`（Claude ステップの実行記録）が読めれば、fail 時に**サブエージェントへの委任・ターン数**を手掛かりとして併記する（2026-09-09 の実測: AI が Agent ツールへ監査全体を委任し 2 ターンで終了→ issue 未更新。`--disallowedTools "Agent,Task"` で塞いだ） | 標準出力（判定） |
...
node scripts/check-observability-assets.js         # ダッシュボードとコード側計器の一致を検査（#287）
node scripts/check-observability-assets.js --self-test  # 検査ロジック自体の自己試験
node scripts/check-frontend-empty-frames.js         # frontend/src の .gitkeep のみの枠を検査（MSP/ADR-0069 決定 5）
node scripts/check-frontend-empty-frames.js --self-test  # 検査ロジック自体の自己試験
node scripts/check-proto-contracts.js               # proto 契約の配置・versioning・後方互換を検査（#745）
node scripts/check-proto-contracts.js --update      # baseline を現状で更新（承認済みの破壊的変更を消費）
node scripts/check-proto-contracts.js --self-test   # 検査ロジック自体の自己試験
...
> （小文字 `tests`）、新: `backend/Services/<Svc>/Tests/**`（VSA 統合後。大文字始まり `Tests`。
> 基盤 microservices-platform IADR-0282 決定 1 の樹形に合わせる）。🔴 **着手前の設計書はこの検査器を
> 「`.Tests` で終わるので無改修で拾う」と評価していたが誤りだった** —— `Tests` は `.Tests` 接尾でも
> `tests` 完全一致でもなく、統合後は 1 件も拾えなかった。**素朴に `e.name === 'Tests'` を足すと
> `backend/Tests/`（横断テスト）が新たに条件へ当たり現行の母集合が変わる**ため、新樹形の判定は
> `backend/Services/<Svc>/Tests` という**位置**まで見て区別する（`isNewLayoutServiceTestsDir`）。
> 本検査器は必須 FR の参照が全滅すれば赤で落ちる fail-closed だが、**部分移行時**（1 サービスだけ
> 新樹形へ移送された状態）は、そのサービスのテストだけが母集合から静かに落ちても他サービスが必須 FR を
> 参照していれば緑のままであり得る。これを塞ぐのが **T1**（`check-consumer-endpoint-names.js` の M4 と
> 同型。新旧いずれかの樹形のサービスディレクトリが実在するのに、その樹形からテストを 1 件も走査できて
> いなければ落とす。静的な下限ではなく樹形の実在から動的に導く——AST は移送済みサービスが現時点で
...
- **例外**: fail-open が事故ではなく**決定**である検査器には求めない（旧 `check-planning-pin-freshness.js`
  ...
  **その旨をヘッダへ書くこと。**
- **git の出力を読む exec には `maxBuffer` を渡し、読めなかったことを skip として exit 0 で返さない**
  （#1009）。Node の既定 `maxBuffer` は 1 MiB で、超えると `ENOBUFS` になる。索引 README が 1 MiB を
  超えた時点から `check-adr-index-addendum-loss.js` はこれを「版を取得できなかったため skip した」として
  緑を返し、CI でも検査が止まっていた（同じ `ENOBUFS` はテストでも #569 で起きており、2 回目）。
  上限は `lib/git-read.js` の `GIT_MAX_BUFFER`、skip してよい読み取り失敗（浅いクローン）の判定は
  同 `isShallowSkip`（**`ENOBUFS` は常に skip しない**）を使う。
...
| `adr-index-sync` | `check-adr-index-sync.js`（IADR 本文と索引行の同時変更。差分を読めなかったとき skip するのは浅いクローンの場合だけで、それ以外は赤。#1009） |
| `adr-index-addendum-loss` | `check-adr-index-addendum-loss.js --self-test` と本検査（索引行の日付つき追記ブロックの消失と、索引行の IADR 番号の重複。`fetch-depth: 0` が必要。#875 / #955） |
...
| `shell-scripts` | `k8s-local-deploy.test.sh` / `deploy/opend/entrypoint.test.sh` / `cutover-count-reconcile.test.sh` / `host-liveness-monitor.test.ps1`（`pwsh`。本リポ固有） |
