...
| `check-observability-assets.js` | **業務メトリクスとダッシュボードの乖離**を検査する（NFR-07 / #287 / IADR-0255）。🔴 **乖離はエラーを出さない** —— 系列名がずれたパネルは空のグラフを描き、**空のグラフは「異常が起きていない」と読める**。監視しているつもりで何も見ていない状態が気付かれずに続くため、機械で止める。見るのは **D1/D2/D3**（ダッシュボード JSON が妥当・各パネルが `expr` を持つ・`uid` が一意）／**R1**（`expr` が引く `ast_*` 系列がコード側のレジストリ `BusinessMetricNames.cs` に実在する）／**R2**（レジストリの各計器を少なくとも 1 枚が引いている＝**宣言はあるが誰も見ない計器**を検出）／**E1**（dev の otel-collector が metrics を `debug`＝標準出力のみにしか出さない＝**既定では計装が有効でも外部へ送らない**）／**M1**（ダッシュボード数・計器数の下限。探索が壊れて 0 件になると無条件に緑になる）。コード名 → Prometheus 名の変換規則は「ドットを `_` へ」＋「Counter は `_total`」＋「Histogram は `_bucket`/`_count`/`_sum`」の 3 つで閉じる（**計器に `unit` を与えない**ことでこの規則を 1 本に保っている）。**見ないもの**: Prometheus に実際に系列が出ているか（実バックエンド依存・射程外）、クエリが意味的に正しいか（人が読む）。`--self-test`（16 件） | 標準出力（レポート） |
