...
> 起点は**リスク統制**、**段階ゲート**、および**設定変更・一時停止・緊急停止のユースケース**である（本画面は当該統制の状態を閲覧する参照面）。
> 計画リポジトリの画面設計が空だった時点の素案であり、計画側へ環流済みである。データ源は RiskManagementService
...
（リスク設定と統制状態は Risk の既存 OwnerOnly 契約を消費する参照優先の別 feature とし、破壊的操作は Bot 側に委ねるという決定 2・安全既定）。
...
0. **維持率・空売りの現況（`ShortSellingStatusView`・#340。空売り段階解禁の計画 ADR の決定 3/7/9/15）** —— **本画面の最上位に置く。**
   計画の画面設計（統制状態参照画面）の原文: 「マージンコールは口座を失う唯一の経路であり、現物取引には存在しなかった指標である」。
   ...
   - **供給が無い値を「正常値」に見せない（供給可否はサーバが宣言する）**: 応答は指標ごとに供給可否
     ...
     **未知の供給可否は未供給へ倒す**（値があるように見せない）。
     ...
     | 項目 | 供給が無い理由 |
     | --- | --- |
     | 維持率 | `UnavailableMaintenanceMarginSnapshotSource` が常に `null`。PoC 項目 3 で「実弾口座でのみ照会でき SIMULATE では照会 API 自体が失敗」と実測。**Stage 1 の全期間にわたって表示できない**（空売り段階解禁の計画 ADR の決定 7 の 2026-08-07 追記）。**これは不具合ではなく、供給が無いという事実の正しい表現である** |
     ...
     | **強制買戻しの発生回数**（空売り段階解禁の計画 ADR の決定 15・#424） | 推定台帳（事後推定の実装 ADR・#419）は入ったが、**推定が起きたときにしか行を書かない**ため、行数 0 は「観測が一度も届いていない」と「観測して 0 件だった」を区別できない。計画は本項目へ**「0 件と表示してはならない」**と名指ししている（未供給表示規約の決定 2） |

0-2. **供給が無い値の表示規約（全画面共通・2026-08-07 追加）** —— 3 状態を**両方向**に守る。
   ...
   - **「—」は対象なし専用の記号である。** 未供給を「—」で描かない（未供給表示規約の決定 3）。
...
2-2. **Stage 1 の進捗（#334。合格集計から内蔵 `paper` を構造的に排除する）**: 経過営業日数 / 目標・取引件数 / 最小件数を表示し、
...
2-3. **発注先の変更履歴（#334・段階ゲートの要求 (2)）**: 日時・変更前後・変更者・理由を新しい順に一覧。
