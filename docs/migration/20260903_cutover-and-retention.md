...
| 4 | **基盤側の KB の扱い** | 基盤の破棄の裁定を**受け入れる**（KB の写しは派生物であり、正は本システムの `reports` にある）。そのかわり **確定報告書を KB へ入れ直す手段を本システム側に用意する**（別 issue として起票する）。基盤の切替の窓の前に、4 つのサービス用クライアントの secret を画面で変えていないことを確かめる | 基盤の切替の後、学習サイクルが過去の確定報告書を KB から引けない |
