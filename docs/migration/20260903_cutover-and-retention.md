...
iadrs: [IADR-0057, IADR-0059, IADR-0074, IADR-0109, IADR-0287, IADR-0341, IADR-0350, IADR-0436, MSP:IADR-0459]
specs: [20260903_346_cutover-preparation, 20260926_346_cutover-plan-decisions, 20260902_204_pre-golive-audit-update, 20260919_849_ledger-drift-adoption, 20260926_1028_report-kb-reingest]
issues: [#346, #344, #204, #137, #141, #342, #24, #339, #849, #1028, planning#28]
-->
...
## 現況（2026-09-26 再測定）と利用者が決めること

> 本節は 2026-09-26 の再測定である。準備（全数表・突合スクリプト・リハーサル）は 2026-09-03 から変わっていない。
> **変わったのは周辺の前提**であり、承認事項のうち 2 つは前提が動いた（下表の「前提の変化」）。**判断は利用者に残し、AI の推奨を並べる。**

### 再測定の結果（読み取りのみ）

| 項目 | 2026-09-03 の記録 | 2026-09-26 の実測 | 帰結 |
| --- | --- | --- | --- |
| 保全対象 | 7 DB・38 テーブル | `bash scripts/cutover-count-reconcile.sh manifest` → 7 DB・38 テーブル（`ledger` 24／`state` 12／`reserved` 1／`dedup` 1） | 変化なし |
| バックアップ | 未記入 | **クラスタに CronJob は 1 つも無い。** 業務台帳の入った `postgres-data`（2Gi・local-path）の PV は回収方針 `Delete` で、2026-09-15 に作り直されている | **7 年保持の台帳にバックアップが 1 本も無い。** クラスタや namespace を作り直すと消える（2026-09-04 に一度起きた） |
| 秘密情報の置き場 | `ast-secrets` は手動 Secret（`scripts/k8s-local-deploy.sh` が値を保持） | **`ast-secrets` は ExternalSecret が所有し、値の正は Vault（`vault-data` PVC）** | 設定の保全の対象が Secret から Vault へ移った。Vault のバックアップも要る |
| 基盤側の切替 | 「KB の保全は基盤の切替計画へ委ねる」（承認事項 4） | **基盤は自分の切替で、文書 DB・索引（Qdrant）・可観測性データを破棄すると裁定済み。** Postgres と Keycloak の PVC は消さず、本システムの DB と realm には触らない。基盤の realm の作り直しで、本システムが使う 4 つのサービス用クライアントの secret は宣言値（開発用の既定）に戻り、**実行時に付けたロールの付与（宣言に無い利用者への `trading-owner` など）は失われる**（宣言にある付与だけが戻る） | **確定報告書の KB 上の写しは消える。** 正は本システムの `reports` テーブル（本文つき）に残るので、基盤の切替の後に**確定報告書を KB へ入れ直す**経路が要る（今は確定時の 1 回しか保存しない）。**［2026-09-26 追記］入れ直す経路を用意した**（`POST /reports/knowledge-base/reingest`・所有者専用・冪等。手順は運用仕様書 §基盤の切替の後の KB への入れ直し）。クライアント secret を画面で変えていれば、基盤の切替の後にサービス間認証が落ちる。**実行時に `trading-owner` を付けた利用者は、基盤の切替の後に owner 操作に入れなくなる** |
| 旧実装の廃止（ブランチ） | `origin` 33 本（マージ済み 4・未マージ 29） | `origin` 6 本（`develop`・`main`・CHANGELOG 自動更新・作業中の PR 3 本）。マージ時の自動削除が有効 | **ブランチの棚卸しは実質終わっている。** 削除の判断は不要 |
| 前段ゲート | #204・#342・#24 open | #204（go-live 前監査）open、#342（moomoo PoC）open・`blocked:env`、#24（Hetzner）open、親の #344 open | 変化なし。**切替の実施はまだ決められない** |

### 利用者が決めること（推奨つき）

| # | 決めること | AI の推奨 | 決めないと何が止まるか |
| --- | --- | --- | --- |
| 1 | **切替の実施可否と日時** | 前段ゲート（#204 の Conditional-Go 条件・#342・#24）が閉じるまで**実施しない**。閉じたら週末（土曜 07:00 JST〜月曜 08:00 JST）の窓で、本書の手順 0〜7 を通す | 切替そのもの（本 issue の完了） |
| 2 | **バックアップの保管先・保管期間・リストア試験の頻度** | **切替を待たずに今から始める**（上表: 台帳のバックアップが 1 本も無い）。対象は 7 DB と Vault。保管先は**クラスタの外に 2 か所**（例: ホストのディスク上の暗号化された置き場＋外部のオブジェクトストレージ）。保管期間は**業務台帳を含む dump を 7 年**、日次の世代は 30 日。リストア試験は**四半期に 1 回と切替の前**。手順案は [運用仕様書 §バックアップ・リストア](../operations/operations.md#バックアップリストア) | 7 年保持の担保（現状は 1 回のクラスタ作り直しで全損する）。切替の手順 2 |
| 3 | **旧実装の廃止の範囲** | ブランチは対応不要（上表）。残るのは文書の追随（`CLAUDE.md`・`AGENTS.md`・README の「移送中」の記述）と、#344 の残存 issue のトリアージ。**どちらも切替の後**に行う | 本 issue の最後の区分（切替の後でよい） |
| 4 | **基盤側の KB の扱い** | 基盤の破棄の裁定を**受け入れる**（KB の写しは派生物であり、正は本システムの `reports` にある）。そのかわり **確定報告書を KB へ入れ直す手段を本システム側に用意する**（別 issue として起票する。**［2026-09-26 追記］用意した**: `POST /reports/knowledge-base/reingest`。基盤の切替の後に 1 回実行する）。基盤の切替の窓の前に、4 つのサービス用クライアントの secret を画面で変えていないこと、**および realm `platform` で実行時に付けたロールの付与（`trading-owner` / `trading-service`）を書き出しておき、切替の後に付け直す**ことを確かめる（基盤の移行仕様書の手順 0 と手順 6 がこの書き出しと復元を持つ） | 基盤の切替の後、学習サイクルが過去の確定報告書を KB から引けない |

## 移行概要

| 項目 | 内容 |
| --- | --- |
| 移行対象 | データ（7 サービス DB・38 テーブル）／統制状態（リスク統制設定・取引ガード・段階ゲート・停止系）／設定（Helm values・`ast-secrets`） |
...
**母集合**: 7 つの `DbContext` の全 `DbSet`（38。走査と除外の記録は作業仕様書 §保全対象の母集合）。
...
- 対象外（作業仕様書に理由を記録）: DB を持たない 4 サービス（情報収集・取引判断・通知・バックテスト）、基盤側の DB（確定済み報告書の KB 保存先 `document_svc` など。基盤の切替計画に委ねる＝§承認事項）、RabbitMQ のキュー（凍結でドレインする）、Helm values / `ast-secrets`（設定であり台帳ではない。値の保持は `scripts/k8s-local-deploy.sh` が担う）。**［2026-09-26 追記］連結ローカルでは `ast-secrets` は ExternalSecret が所有し、値の正は Vault である**（§現況）。設定の保全は Vault のバックアップで行う。
...
bash scripts/cutover-count-reconcile.sh manifest        # 保全対象 38 テーブルの全数表
...
- **ブランチ**: **［2026-09-26 再測定］`origin` は 6 本（`develop`・`main`・CHANGELOG 自動更新・作業中の PR）で、マージ時の自動削除が有効。棚卸しは実質終わっている。** 以下は 2026-09-03 の記録: `origin` に 33 本（2026-09-03 実測）。`origin/develop` へマージ済みは 4 本（`automation/changelog-update-develop` 等）。未マージ 29 本は open PR（#651・#649）・棚卸し中の docs ブランチ・古い chore ブランチが混在する。**削除は利用者承認後**、PR に紐づかないものから順に。
...
> **推奨と前提の変化は §現況（2026-09-26 再測定）と利用者が決めること の表にある。** とくに 2 は切替を待たずに始めることを推奨する（業務台帳のバックアップが 1 本も無い）。
