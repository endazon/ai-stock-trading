...
iadrs: [IADR-0057, IADR-0059, IADR-0074, IADR-0109, IADR-0287, IADR-0341, IADR-0350, MSP:IADR-0459]
specs: [20260903_346_cutover-preparation, 20260926_346_cutover-plan-decisions, 20260902_204_pre-golive-audit-update, 20260919_849_ledger-drift-adoption]
...
| 基盤側の切替 | 「KB の保全は基盤の切替計画へ委ねる」（承認事項 4） | **基盤は自分の切替で、文書 DB・索引（Qdrant）・可観測性データを破棄すると裁定済み。** Postgres と Keycloak の PVC は消さず、本システムの DB と realm には触らない。基盤の realm の作り直しで、本システムが使う 4 つのサービス用クライアントの secret は宣言値（開発用の既定）に戻り、**実行時に付けたロールの付与（宣言に無い利用者への `trading-owner` など）は失われる**（宣言にある付与だけが戻る） | **確定報告書の KB 上の写しは消える。** 正は本システムの `reports` テーブル（本文つき）に残るので、基盤の切替の後に**確定報告書を KB へ入れ直す**経路が要る（今は確定時の 1 回しか保存しない）。クライアント secret を画面で変えていれば、基盤の切替の後にサービス間認証が落ちる。**実行時に `trading-owner` を付けた利用者は、基盤の切替の後に owner 操作に入れなくなる** |
...
| 4 | **基盤側の KB の扱い** | 基盤の破棄の裁定を**受け入れる**（KB の写しは派生物であり、正は本システムの `reports` にある）。そのかわり **確定報告書を KB へ入れ直す手段を本システム側に用意する**（別 issue として起票する）。基盤の切替の窓の前に、4 つのサービス用クライアントの secret を画面で変えていないこと、**および realm `platform` で実行時に付けたロールの付与（`trading-owner` / `trading-service`）を書き出しておき、切替の後に付け直す**ことを確かめる（基盤の移行仕様書の手順 0 と手順 6 がこの書き出しと復元を持つ） | 基盤の切替の後、学習サイクルが過去の確定報告書を KB から引けない |
