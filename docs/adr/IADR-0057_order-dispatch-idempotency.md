...
- **再処理時にブローカへ照会して既存注文を確認してから再発注する**: 最も情報量が多く取りこぼしも無いが、
  ...
  照会によるリコンサイルは**後続 issue に分離**し、本 PR は「二重発注しない」ことだけを確定させる。
