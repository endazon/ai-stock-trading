...
- フォローアップ（重複解消・claude-review）: 平均取得単価法の畳み込みは `RiskManagementService` の `PortfolioProjection.Apply`
  と本 `PnlAggregator.Apply` で重複実装している（意図的な同方式踏襲）。方式変更時の片側更新ドリフトを避けるため、将来は
  `CostCalculator`/`TradingAssumptions` と同様に共有ドメイン（例: `ConfigurationService.Domain` もしくは新設の共有計算ライブラリ）へ
  畳み込みを集約することを検討する。
