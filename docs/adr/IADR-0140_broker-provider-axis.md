...
利用者裁定（質問票 第 11 回 Q1・2026-08-02。[INDEX 決定 46](../../planning/projects/ai-stock-trading/INDEX.md)）により、
計画は次を確定した。

> 「運用段階（Stage）」と「**発注先**（`moomoo REAL` / `moomoo SIMULATE` / 内蔵 `paper`）」を**独立した 2 軸**
> として扱う。… **段階が定める動作モードは既定の組み合わせを示すにとどまる**（[FR-20](../../planning/projects/ai-stock-trading/02_requirements/01_requirements.md)）
...
警告を表示する（例: Stage 1 のまま `moomoo REAL`）」（[05_screens](../../planning/projects/ai-stock-trading/05_screens/01_screens.md) SC-02 入力表）と定める。
**発注そのものの可否には触れていない。**
