...
- **ADR-0003 のプロンプトインジェクション対策フォローアップ（入力サニタイズ・出典限定）は未着手**。本作業は KB（＝収集ニュース由来）
  の取得文脈を判断プロンプトへ注入する経路そのものであり、ADR-0003 が課すこのフォローアップが直撃する箇所である。本 PR は決定3の
  「参考情報は方針・制約を上書きしない」権威順序の明示に留め、サニタイズ・出典制限は実装していない。実 KB 疎通（#82 系）で
  取得文脈が実データになる段で具体化する必要があり、放置されないよう本節に明記して後続へ申し送る（計画側 ADR-0003 フォローアップ欄の
  更新は `/plan-feedback` で別途検討）。
