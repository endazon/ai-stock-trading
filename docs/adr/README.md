...
> 注（暫定欠番）: `IADR-0037`〜`IADR-0041` は並行作業中の複数ブランチが採番中で、番号衝突回避のため本 issue #14（対話的確定・明細）の IADR は調整済みの `IADR-0042` を用いる。`origin/develop` 上の当該番号は他 PR のマージで充足される暫定欠番であり、上記「欠番なし」ルールは全 PR マージ後に満たされる想定。並行が収束した時点で採番を再確認する。
