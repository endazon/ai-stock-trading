...
>
> 注（`IADR-0059` の番号衝突と解消）: 並行ブランチの PR #144（重複排除ストアのパージ）と PR #143（OpenD 本番化）が、いずれも `develop` の最新版号を見ないまま `IADR-0059` を採番し、両者のマージで**同番号 2 ファイル**が生じた。先着（#144・先にマージ）が `IADR-0059` を保持し、後着（#143）を `IADR-0061` へ採番し直して解消した（両者とも索引未登録だったため、本 PR で 0059・0061 の 2 行を追記した）。
>
> 注（`IADR-0060` は採番済み・在庫中）: 上記の採番し直しで `0060` ではなく **`0061`** を用いたのは、`IADR-0060` を**未マージの PR #145**（FR-17・バージョン付き全体前提条件の s2s 読み取り）が既に採番しているためである（同 PR は本 PR より先に作成された＝先着）。**本索引に `IADR-0060` の行が無いのは #145 が未マージだからであり、欠番ではない**。#145 のマージで充足される。**`0060` を新規に採番してはならない**。
>
...
| IADR-0062 | 実 LLM 接続の実運用化は既定オフの全量ログ・タイムアウト構成化・空既定の設定サーフェスに限定し、s2s トークンとリトライは足さない | Accepted |
| IADR-0063 | Discord Bot は Gateway 常駐＋多層認証とし、既定 no-op・owner トークンで kill switch を呼ぶ | Accepted |
| IADR-0064 | バージョン付き全体前提条件は s2s 読み取り API ＋共有クライアント（キャッシュ・イベント無効化・last-known-good）で解決する | Accepted |
| IADR-0065 | 公式ソースは「ソース単位で有効化する多ソース合成＋ソース単位レート制限」で束ね、推測実装はしない | Accepted |
