...
4. **差金決済ガードの適用範囲**: FR-19 は「差金決済防止のガードは**日本株の現物取引**における差金決済規制に
   ...
   明記している（2026-08-01 改訂・project-planning#81 の利用者裁定）。実装は市場・商品種別を問わず適用していた。
...
> 計画へ環流した。**裁定待ち**であり、実装は現行のままである。計画が別案（例: 禁止銘柄を Close 除外、
> または Close 許可＋監査記録）を採る場合は新 IADR で改める。
...
> 2026-07 時点の記述が残っており、本決定の根拠（FR-19 の 2026-08-01 改訂・project-planning#81）と齟齬する。
...
- フォローアップ:
  ...
  - 計画側の裁定待ち: 禁止銘柄・市場ガードの手仕舞い適用（作業仕様書 未決事項 1）
