...
- 関連する計画書 ID: **FR-20**（Stage 1 の合格判定は `SIMULATE` のみで集計）／ **FR-11** ／
  [06_daytrading-review §4.1 条件1](../../planning/projects/ai-stock-trading/06_technical/06_daytrading-review.md)
...
- **残余リスク**:
  - **内蔵 `paper` で発生したクラス C の拒否は件数に計上されない。** これは計画の明示的な裁定
    ...
    （未供給として止まる）。この非対称が計画の意図どおりかは、必要なら計画側へ確認する論点として残す。
...
- 計画: [06_daytrading-review §4.1 条件1](../../planning/projects/ai-stock-trading/06_technical/06_daytrading-review.md)／
