...
**ただし自動停止の発火範囲は実弾段階に限られる**（`StageGate.AssessWithdrawal` の実測）。過大に見積もらないよう記録する。

| 段階 | 判定 | `HaltNewEntries` | kill switch |
| --- | --- | --- | --- |
| Stage 0（検証） | `default` 分岐＝`Triggered: false` | false | **起動しない** |
| Stage 1（ペーパー） | 乖離が説明不能なら `Triggered: true` | **false** | **起動しない**（降格提案＋通知のみ・IADR-0085） |
| Stage 2/3（実弾） | 実DD ≥ バックテスト最大DD × 倍率 | true | **起動する** |

`EvaluateWithdrawal` が `killSwitch.Engage` を呼ぶのは `HaltNewEntries: true` のときだけである。
現在の段階は Stage 0（`InMemoryStageGateStore` の既定・enum 値 0）で、Stage 2/3 は実弾＝`LiveTradingReleased=false`
（IADR-0111 の閂 0）により到達不能。したがって **経路B の現構成で自動停止が起きることはない**。

本決定の実利は次の 2 点であり、「いま止まる」ことではない。

1. Stage 1（ペーパー）へ到達した後、説明不能な乖離を降格提案として検出・通知できる（kill switch は起動しない）。
2. 将来 Stage 2/3 が解禁されたとき、撤退基準が**最初から効いている**（後付けで有効化し忘れる経路を塞ぐ）。
...
- 実DD が段階実績台帳へ latch され、ADR-0008 の撤退基準が**実際に評価される**（決定3）。
  自動 kill switch の発火は **Stage 2/3（実弾段階）に限られる**ため、Stage 0 の現構成では起きない。
  Stage 1 到達後はペーパー乖離が降格提案として通知される（kill switch は起動しない）。
  将来 Stage 2/3 が解禁された時点では、条件成立で自動停止し**解除するまで新規建てが止まる**
  （解除は確認フレーズ必須・IADR-0097）。いずれも経路B 限定で、本番既定は従来どおり無効。
