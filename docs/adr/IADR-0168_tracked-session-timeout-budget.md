...
- **文字列の走査は完全な C# 字句解析ではない。** #447 のレビューで**2 度**穴が見つかり、2 度とも塞いだ。
  1 度目: 補間文字列の穴（`$"…{ ここはコード }…"`）を非コード扱いしていた → **コードとして扱う**ようにした。
  2 度目: その修正が「いま逐語か」を**単一のフラグ**で持っていたため、**穴の中に文字列が入れ子になると内側が外側の種別を上書き**し、
  入れ子を挟んだ次の穴を見失っていた（実測: `$@"a{1 + "z"}b{host.TrackActivity()}c"` が 0 件）→ **frames のスタック**で持つ形へ直した。
  **なお生文字列リテラル（`"""…"""`）は扱っていない。** 同型の既存検査（`check-banned-settled-cash-sources.js`）も同じ簡略化を採っている。
  **この 2 度の見落としは、「検査を書く」こと自体が検査を要する作業であることを示している**——検査の正しさは、検査対象と同じくテストで固定するしかない。
