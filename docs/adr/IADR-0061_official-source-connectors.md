...
2. **ドメインの純粋なトークンバケット＋ Worker の待機アダプタ（採用）** — 送信前に自制でき、時計を注入
   しないドメイン純関数として決定的に単体検証できる。

### 4. 時刻の抽象化（待機アダプタ・EDINET の日付）

1. .NET 標準の `TimeProvider` を使う — 標準 API だが、本リポジトリでの使用は 2 アダプタ（`PaperBrokerAdapter`・
   `MoomooBrokerAdapter`）のみで、**テストで偽装している箇所は無く**、`FakeTimeProvider`
   （`Microsoft.Extensions.Time.Testing`）は `Directory.Packages.props` に未登録。採用すると中央パッケージ管理への
   追加（並行 PR との衝突面）を伴う。
2. **各サービスの `IClock` ポート＋`SystemClock`（採用）** — 本リポジトリの既存慣行。9 サービスが同型の `IClock` を持ち、
   22 のテストがこれを偽装している。新規パッケージ不要で、待機（`Task.Delay`）は関数として注入すればフェイク待機で
   決定的に検証できる。**同一目的の抽象化を二重に持ち込まない**ことを優先した。
...
2. **レート制限**: ドメインに純粋な `TokenBucket`（時計を引数で受ける状態機械）を置き、Worker の `DelayingRateLimiter`
   が送信前に待機する。時刻は**本リポジトリの既存慣行に合わせて `IClock` ポート**（各サービスが持つ同型の抽象・
   `SystemClock`）で供給し、待機（`Task.Delay`）は関数として注入する（テストはフェイク時計＋フェイク待機で実時間を
   使わない）。既定値は**各ソースの公表上限より保守側**（finnhub 30/分・sec-edgar 5/秒・edinet 1/分・boj 1/分・
   fred 60/分）に固定する。実測に基づく調整は運用後（ADR-0004 フォローアップ）。
...
- 時刻抽象は既存慣行（`IClock`）に合わせることで、同一目的の抽象化の二重化と中央パッケージ管理への追加を避けられる。
