...
- 冪等性: 消費者再配送（`UseAiStockTradingRetry`）での二重発注を防ぐため、`DecisionId` をキーに既存の発注結果があれば
  ...
  再試行で再発注し得る。ペーパーでは実害小だが、moomoo 実発注では二重発注が実害になるため、発注前予約（outbox /
  DecisionId 予約行）で place を冪等化する設計が moomoo 実装までに必要（下記フォローアップ）。
- フォローアップ: ADR-0002 PoC（デモ発注）→ moomoo アダプタ実装（＋発注の outbox 冪等化）→ Vault 秘匿 →
  段階ゲートと連動した実弾解禁の設計。
