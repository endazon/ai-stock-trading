...
3. **保持期間に下限 7 日のクランプを設ける**（`RetentionPolicy.MinimumRetentionDays`）。
   `RetentionDays: 0` のような設定ミスでも、7 日より新しい行は対象にならない。**設定ミスが
   冪等性を壊せない**ようにするのが目的で、これが「進行中の誤削除防止」の最後の砦である。
