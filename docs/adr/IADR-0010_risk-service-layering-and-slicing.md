...
- フォローアップ: Slice B で Worker ホストとインフラアダプタ、Slice C で損切り機械執行を実装する。Slice B の
  PostgreSQL 版 `IRiskSettingsStore` は、設定更新の read-modify-write（`RiskSettingsService`）でロストアップデートを
  防ぐため楽観的排他制御（バージョン番号での CAS）を導入する（本 PR のインメモリ実装は人手・低頻度前提の暫定）。
