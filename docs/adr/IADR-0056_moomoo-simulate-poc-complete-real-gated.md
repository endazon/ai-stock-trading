...
- 関連 IADR: [IADR-0016](IADR-0016_safe-broker-execution.md)（安全既定 paper・moomoo ゲート）、[IADR-0053](IADR-0053_moomoo-opend-dockerization.md)（OpenD Docker/常駐）
...
- **OpenD 常駐が確立**（#124 / IADR-0053）。RSA 暗号化で in-cluster（cross-network）trade 接続が成立。
...
4. **上流への環流**: ADR-0002 の Accepted 化は上流の triage に委ねる。PoC 結果・無人運用の追検証は plan-feedback（`feedback/20260715_adr0002-opend-unattended-limited.md` / project-planning#25）で環流済み。ADR-0002 が Accepted 化されるまでの間も、本 IADR を根拠に **SIMULATE 限定での実装・利用**を可能とする。
