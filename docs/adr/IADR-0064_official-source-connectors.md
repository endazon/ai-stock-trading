---
title: IADR-0064 公式ソースは「ソース単位で有効化する多ソース合成＋ソース単位レート制限」で束ね、推測実装はしない
type: impl-adr
status: Accepted
...
# IADR-0064: 公式ソースは「ソース単位で有効化する多ソース合成＋ソース単位レート制限」で束ね、推測実装はしない
...
> 注（採番の経緯）: 本 IADR の番号は並行採番の影響で二度変わっている。当初 PR #149 は `IADR-0061` を採番したが、
> 並行 PR #146（`IADR-0059` の番号衝突解消）が後着（#143・OpenD 本番化）を同じ `IADR-0061` へ採番し直して
> **本 PR より先に develop へマージした**ため、先着尊重（マージ順）の運用に従い、当時の空き番号 `IADR-0065` へ
> 採番し直した。その後、`IADR-0060` が**恒久的な欠番**として残ることが判明し（#146 が `0060` を予約扱いした
> 根拠の PR #145 が、実際には `0060` ではなく別番号でマージされたため）、`0061`〜`0065` を 1 つずつ繰り下げる
> 是正により本 IADR は最終的に **`IADR-0064`** となった。**決定内容は一度も変わっていない**。経緯は
> [作業仕様書](../specs/20260717_iadr-0060-gap-renumber.md) と [索引](README.md) を参照。
