...
- 実DD ドライバは `Math.Max` で**決して値を下げない**。よってリセットが 1 巡回分打ち消される最悪ケースでも、
...
- 差し戻し先は Stage 0／Stage 1 であり、`AssessWithdrawal` の実DD 判定は Stage 2/3 でしか評価されない。
  再昇格までの間に次のサンプリング（既定 300 秒）が現在 DD を基準に観測しなおす
  （`ObservedDrawdownRefreshServiceTests.差し戻しリセット後は現在DDから観測しなおす`）。
