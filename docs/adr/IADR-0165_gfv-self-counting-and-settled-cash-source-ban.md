...
- 帰結として、**1 件でも記録されれば 2 件目で恒久的に現金口座の新規建てが止まる**（※ 2026-08-08 追補: 「恒久的」は解消した。計画 ADR-0028 決定2 が「利用者の明示的な操作で解除する」と定め、[IADR-0182](IADR-0182_gfv-violation-clearing.md) が実装した。**失効を設けない方針そのものは ADR-0028 決定1 が追認しており不変である**。環流も完了）****。
