...
adrs: [ADR-0003, ADR-0007, ADR-0009, ADR-0016, ADR-0021, ADR-0025]
...
issues: [#12, #13, #17, #19, #332, #333, #375, #425, #49]
-->
...
> 。本書は `RiskEvaluator` の**全違反理由の判定マトリクス**（FR-10/19/20 横断）も収録する。
>
> **2026-08-04 改訂（#332・IADR-0132: 商品種別は 3 値を単一情報源とし、実効値で照合する。商品種別ガードと差金決済ガードは適用範囲を絞る）**:
> 商品種別を **3 値**（現物 / 信用買い / 空売り）へ改め（ADR-0016 決定1）、
> 商品種別ガードの適用を**新規建てのみ**に、差金決済防止ガードの適用を**日本株の現物取引のみ**に絞った。
>
> **2026-08-06 改訂（#375・ADR-0021（計画リポ）・IADR-0153: 口座種別を観測として供給し、不明・食い違い・供給欠如をすべて新規建ての停止へ倒す）**:
...
>
> **2026-08-07 改訂（#425・ADR-0025（計画リポ） 決定2・IADR-0165: GFV 発生回数は約定の事後検出で自前計数し、決済済み資金の代替値は構造で遮断する）**:
...
- 段階別の可否（Stage 1＝3 種／Stage 2＝現物のみ／Stage 3＝条件付き解禁。ADR-0016 決定8）の**強制は
  FR-20の担当**であり、本ガードの有効集合と AND で効かせる（厳しい方が効く）。
...
| ProductTypeDisabled | —| 適用 | **非適用**（#332・ADR-0009。無効な種別の建玉を手仕舞えなくしない） |
| MarketDisabled | —| 適用 | 適用 |
| BannedSymbol | —| 適用 | 適用 |
| SameDayReentry | —| 適用（**現物 && (日本株 ‖ 現金口座)**。#332 / **#375**） | 非適用 |
| BrokerAccountTypeUnverified | —| 適用（**内蔵 paper を除く**。#375） | **非適用**（ADR-0009。口座種別が分からないことを理由に建玉を閉じられなくしない） |
...
- **フェイルセーフの原則**（NFR / ADR-0003 / ADR-0009）: 新規建て（Open）は止めるが、保有ポジションの
  ...
  **商品種別は 3 値化に伴い Close 非適用へ改めた**——既定では空売りが無効であり、
  ...
  自動売却する経路を残さないため。#380 / IADR-0132: 商品種別は 3 値を単一情報源とし、実効値で照合する。商品種別ガードと差金決済ガードは適用範囲を絞る）。
...
- [x] **GFV 発生回数を自前で計数し、記録が監査ログに残る**（#425・ADR-0025 決定2）
...
- 実装ADR: IADR-0165: GFV 発生回数は約定の事後検出で自前計数し、決済済み資金の代替値は構造で遮断する（**GFV の自前計数と決済済み資金の代替値の遮断**・#425）、IADR-0153: 口座種別を観測として供給し、不明・食い違い・供給欠如をすべて新規建ての停止へ倒す（**口座種別の供給と fail-closed**・#375）、IADR-0132: 商品種別は 3 値を単一情報源とし、実効値で照合する。商品種別ガードと差金決済ガードは適用範囲を絞る（商品種別 3 値化・ガードの適用範囲）、IADR-0131: 空売り専用統制は「新規売り建て」を起点に判定し、外部照会が欠けたら通さない。拒否理由はクラス分類を実装の単一情報源として持つ（空売り統制）、IADR-0004: エントリー/手仕舞いは建玉効果（PositionEffect）で判定し、売買方向から分離する（建玉効果）、IADR-0006: 相場操縦パターン禁止はガード設定＋判定ポートの拡張点として用意し、検知本体は後続スライスとする（相場操縦拡張点）、IADR-0038: ドテン/部分決済は取引判断が符号付きポジションのゼロ跨ぎで Close+Open に分解する（ドテン/部分決済の注文分解）、IADR-0040: 相場操縦パターン検知は自口座の直近発注統計に対する純関数ヒューリスティックで判定する（相場操縦検知アルゴリズム）
- 作業仕様書: 仕様書: GFV 発生回数の自前計数と、決済済み資金の代替値を構造で禁じる、仕様書: 米国口座の現金口座対応（#375 / ADR-0021 決定4）、作業仕様書: 取引ガードの再実装、仕様書: 相場操縦パターン検知アルゴリズムの実装
...
- **GFV 違反記録の失効が計画に無い**（#425 / IADR-0165 決定4）: `GoodFaithViolationLimitReached` の解除条件は
  ...
  **1 件でも記録されれば 2 件目で恒久的に現金口座の新規建てが止まる**。計画へ環流済み
  （feedback/20260807_adr0025-gfv-counting-open-points.md（環流記録））。
- **自前計数の限界を運用手順へ落とす先が未定**（#425・ADR-0025 §結果のフォローアップ）: ブローカー側の GFV 表示との
  目視突合の頻度・不一致時の行動が決まっていない。同じ環流記録で計画へ返した。
- 相場操縦検知の具体閾値（IADR-0040 の初期値）は運用データで較正する（フォローアップ）。本番 DI 登録は実注文履歴テレメトリ確定後。
- 回転売買・ドテン/部分決済の注文分解方針は IADR-0038: ドテン/部分決済は取引判断が符号付きポジションのゼロ跨ぎで Close+Open に分解する（符号付きポジションのゼロ跨ぎで Close+Open に分解）で確定済み。分解ロジックの実装結線は信用有効化スライスで行う（後続）。


## GFV 違反による停止の解除（#464・ADR-0028 決定2/決定3・IADR-0182: GFV 違反による停止の解除（記録は消さず解除を追記する））
...
4. **理由必須**（決定2 の「原因の是正が済んでいることの確認」）。**理由は利用者が入力する**。
   **定型文で埋めない**（IADR-0182: GFV 違反による停止の解除（記録は消さず解除を追記する） 決定5）——
