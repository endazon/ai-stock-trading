...
- 実装ADR: [IADR-0004](../adr/IADR-0004_position-effect-entry-scoping.md)（建玉効果）、[IADR-0006](../adr/IADR-0006_manipulation-guard-extension-point.md)（相場操縦拡張点）、[IADR-0037](../adr/IADR-0037_order-decomposition-position-effect.md)（ドテン/部分決済の注文分解）

## 未決事項

- 相場操縦検知の具体閾値（過剰な訂正/取消の統計）は運用データ待ち（後続スライス）。
- 回転売買・ドテン/部分決済の注文分解方針は [IADR-0037](../adr/IADR-0037_order-decomposition-position-effect.md)（符号付きポジションのゼロ跨ぎで Close+Open に分解）で確定済み。分解ロジックの実装結線は信用有効化スライスで行う（後続）。
