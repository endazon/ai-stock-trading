...
status: draft
related_ids: [FR-19, FR-10, FR-11, FR-20, UC-01, UC-02, UC-06, ADR-0007, ADR-0003, ADR-0009, ADR-0016, ADR-0021, ADR-0025, IADR-0132, IADR-0153, IADR-0165, IADR-0182]
...
- **フェイルセーフの原則**（NFR / ADR-0003 / ADR-0009）: 新規建て（Open）は止めるが、保有ポジションの
  ...
  **商品種別は 3 値化（#332）に伴い Close 非適用へ改めた**——既定では空売りが無効であり、
  ...
  （禁止銘柄・市場の Close 適用は **【✅ 裁定済み 2026-08-04・ADR-0007 追補（質問票 第 1 回 Q3・Q4・planning#179）】** **選択肢 A＝全注文適用で確定**。
...
（2026-08-07 Accepted）が、`GoodFaithViolationLimitReached` による停止の**解除条件**を確定した。

| 決定 | 実装 |
| --- | --- |
| **1. 違反記録は失効させない**（期間経過で消えない） | 従前どおり（IADR-0165 決定4 の追認）。**変更していない** |
| **2. 停止は利用者の明示的な操作でのみ解除する。** 原因の是正の確認を伴い、**誰が・いつ・どの記録に対して**を監査ログ（FR-11）へ | 解除行の追記＋`GoodFaithViolationsCleared` の発行 |
| **3. 解除の窓口は Discord Bot。** 画面（SC-02 / SC-03）からは解除できない | `/gfv clear`。**BFF / フロントに経路を作らない** |
...
4. **理由必須**（決定2 の「原因の是正が済んでいることの確認」）。**理由は利用者が入力する**。
   **定型文で埋めない**（[IADR-0182](../adr/IADR-0182_gfv-violation-clearing.md) 決定5）——
   決定3 により Discord Bot が唯一の窓口である以上、定型文にすると**全解除の理由が同一文字列になり**、
   ADR-0028 §理由 の「後から『なぜ解除したか』を追える」が成立しない
...
ADR-0028 が明記している。**決定2 の解除は「記録が積まれた場合」の解除であり、供給が無い場合の拒否を
...
- [x] **否定形**: **画面（SC-02 / SC-03）からは解除できない**（BFF の登録ルート集合を実際に列挙して突合する構造テスト。ルートが増えれば赤くなる）
