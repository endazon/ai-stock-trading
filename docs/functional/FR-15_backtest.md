...
| 役割 | 実装 | 備考 |
| --- | --- | --- |
| 取得（実データ源） | `IHistoricalBarSource` | 非同期。戻り値 `HistoricalBarLoad(Bars, Gaps)` で欠測を銘柄と理由つきで残す |
| 実アダプタ | `StooqHistoricalBarSource` | ADR-0004 が検証・学習用に採用した Stooq（日足 EOD・登録不要・日米両市場）。送信前に `IRateLimiter` で自制 |
| 安全既定 | `NoOpHistoricalBarSource` | `Backtest:BarData:Provider` 既定 `none`＝**外部へ 1 リクエストも出さない**。未知 provider・不正 URL も警告して no-op |
