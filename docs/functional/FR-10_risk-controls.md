...
- [本書が受け持つ範囲](#本書が受け持つ範囲)
- [概要](#概要)
- [金額系の統制上限 3 値（#329 第 1 段階）](#金額系の統制上限-3-値329-第-1-段階)
- [既定値の確定単一値（#329 第 1 段階）](#既定値の確定単一値329-第-1-段階)
- [空売り専用統制 8 規則（#329 第 2 段階）](#空売り専用統制-8-規則329-第-2-段階)
- [3 統制の優先順位（#329 第 2 段階）](#3-統制の優先順位329-第-2-段階)
- [維持率割れによる建玉の自動縮小](#維持率割れによる建玉の自動縮小)
- [借株料の累計（#465。借株料は「建玉 × 取引日」で積み、未供給の日を別テーブル・別イベントで持つ）](#借株料の累計465借株料は建玉--取引日で積み未供給の日を別テーブル別イベントで持つ)
- [機能詳細（日次損失上限）](#機能詳細日次損失上限)
- [情報収集の縮退による新規建て停止（不明は止める。#337 / #564）](#情報収集の縮退による新規建て停止不明は止める337--564)
- [処理フロー / 状態遷移](#処理フロー--状態遷移)
- [例外・エラー処理](#例外エラー処理)
- [受け入れ基準](#受け入れ基準)
- [関連仕様](#関連仕様)
- [未決事項](#未決事項)
...
> Issue #33 で拡充する。ここに未記載の項目は作業仕様書 仕様書: リスク評価コアの是正
> と関連する実装 ADR（既定値の逆算・サイジング責務・建玉効果・段階資金上限・日次損失基準）を一次情報とする。
>
> **#330 による追記（2026-08-04）**: **維持率割れによる建玉の自動縮小**の節を追加した（発動条件・回復目標・
> 対象選択・縮退・記録先 4 つ・未実装の供給元）。`approved` 済みの本書に対する**追記のみ**であり、
> 既存の節は変更していない（維持率割れの自動縮小の決定的規則＝算式・発動条件・最小限・記録先。
> 作業仕様書: 維持率割れによる建玉の自動縮小）。
>
> **再実装による更新（2026-08-04・第 1〜3 段階）**: 計画大改定に伴い、**金額系の統制上限 3 値**の節
...
>
> **#419 による更新（2026-08-07）**: 計画側で**空売り段階解禁の計画 ADR の決定 4 が改訂され**（2026-08-06 の利用者裁定）、
> **強制買戻しの検知が「建玉消失と約定履歴の突合による事後推定」へ置き換わった**。
> あわせて**決定 15 に「発生有無・発生回数の集計元」の追記**が入った。節「規則 8 の供給 — 強制買戻しは**事後推定**する」を追加した
> （強制買戻しは建玉消失と自らの決済指示の突合で事後推定し、過剰推定へ倒す。
> 作業仕様書: 強制買戻しの事後推定と 30 日禁止の供給）。
> **30 日の禁止期間・`BuyInBanned`（クラス A）の扱いは変更していない**（決定 4 改訂が「変更しない」と明示）。
>
> **#420 による更新（2026-08-07）**: 計画側で**同計画 ADR の決定 7 に追記**が入り（2026-08-07 の利用者裁定）、
> **維持率の算式（純資産 ÷ 建玉評価額の合計）・複数建玉時の適用閾値
...
> （維持率の適用閾値は口座単位＝建玉ごとの閾値の最大値とし、算式と条文選択を単一情報源へ寄せる。
> 作業仕様書: 維持率の適用閾値を口座単位へ揃える）。
> **自前閾値 40%・回復目標 +5pt・株価下限 $5.00 の値は変更していない。**
>
> **#564 による更新（2026-08-29）**: 節「**情報収集の縮退による新規建て停止（不明は止める）**」を追加した。
> **`approved` 済みの本書に対する追記のみ**であり、既存の節は変更していない。
> **統制そのものは #337 で結線済み**であり、本追記は**その統制が「供給が途切れたときにどちらへ倒れるか」**を
> 明文化したものである（従前は fail-open だった）。**金額系・空売り系の統制の値・判定は変更していない。**
>
> **#417 による更新（2026-08-07）**: 計画側で**同計画 ADR の決定 3 が改訂され**（2026-08-06 の利用者裁定）、
> **空売りの一次ゲートが借株料 20% から借株可否（`IsShortPermit`）へ移った**。「空売り専用統制 8 規則」の規則 3 と
> 「判定の入力と縮退」に反映し、節「規則 3 の一次ゲート — 借株可否」を追加した（一次ゲートは借株可否とし、
> 20% の借株料閾値は「発火しない既知の統制」として残置する。
> 仕様書: 空売りの一次ゲートを `IsShortPermit`（借株可否）へ移す）。**20% 閾値は削除せず
...
| 読む場面 | 節 |
| --- | --- |
| 1 注文・1 日あたりの発注金額上限や保有建玉数上限の値を確認したい | §金額系の統制上限 3 値 |
| 日次損失上限・最大DD・連敗時縮小など既定値そのものを確認したい | §既定値の確定単一値 |
| 空売り（信用売り）に掛かる統制の一覧・拒否理由を確認したい | §空売り専用統制 8 規則 |
| kill switch・日次損失ロックアウト・一時停止のどれが優先されるか確認したい | §3 統制の優先順位 |
| 維持率割れで自動決済がいつ・どう動くか確認したい | §維持率割れによる建玉の自動縮小 |
| 借株料の累計がどう記録されるか確認したい | §借株料の累計 |
| 未実装・未供給の項目を確認したい | 各節末尾の「未実装の供給元」「供給は開始していない」 |

---

## 本書が受け持つ範囲

- 機能要求: リスク統制、監査ログ、全体前提条件の既定値。
- ユースケース: 定時取引サイクル・価格変動トリガー取引（検証段）、設定変更・一時停止・緊急停止。
- 業務フロー: 取引サイクル（発注前の決定的判定）。
- 関連する計画 ADR: 生成AIの売買判断のガードレール／一時停止とロックアウトの分離（手仕舞い不停止）／
  空売りの段階解禁（保有建玉数 3・空売り統制）／**リスク統制既定値の確定単一値**。
- 計画書: 全体前提条件 §5（リスク統制・取引ガードの既定値）・§3（基準通貨）。
...
手仕舞い（Close）をブロックしないフェイルセーフ（非機能要件と、AI 判断のガードレールの計画 ADR）で構成する。

## 金額系の統制上限 3 値（#329 第 1 段階）

計画 §5 は金額で表す統制上限を**固定額ではなく equity（自己資金）に対する割合**で保持することを確定した
（利用者委任に基づく決定 2026-08-02）。資金の増減に応じて各上限が比例調整され、
...
### 比率から金額への解決点は 1 つ（equity 比保持の決定 1）
...
### 決済（Close）は日次枠を消費しない（equity 比保持の決定 4 / #302）
...
3 統制の優先順位を定めた計画 ADR の「手仕舞い・損切りは止めない」を金額系上限でも壊さないための必須条件である。

### 複数上限の競合は常に厳しい方が効く

計画のリスク統制の要求は「同一の注文に複数の上限が掛かる場合は常に厳しい方が効く」と定める。実装では次の 2 つの
...
## 既定値の確定単一値（#329 第 1 段階）

計画は 2026-07-23 にレンジを単一値へ確定し、リスク統制既定値の計画 ADR がこれを段階ゲート運用の計画 ADR の部分改定として記録した。
...
| `InitialEquityUsd = 3_000m` ＋ `EquityCurrency = Usd` | 計画の確定値そのもの | 計画適合検査の抽出元。単位の取り違えを検知可能にする |
...
| 4 | 維持率の閾値 | **40% と規制要求の厳しい方**（空売り＝`max($5.00 ÷ 株価, 30%)`・境界 **$12.50**）。**適用は口座単位**＝保有建玉と当該注文の閾値の**最大値** | `MaintenanceMarginBreach` |
...
（※）#329 で実装側が先行して新設し（空売り統制の実装 ADR の決定 3）、**2026-08-04 に計画が同名で追認**した（空売り段階解禁の計画 ADR の決定 10 の改訂・拒否理由 7 種 → **9 種**）。
（※※）**`BorrowUnavailable` へ写像してはならない**（同決定 10 の 2026-08-04 追記）。前者は**都度の借株需給**による
locate 失敗、後者は**期間の経過**で解除される禁止状態であり、原因も解除条件も異なる。畳むと監査ログの
理由が実態と食い違い、日報・月報の「強制買戻しの発生有無・発生回数」（決定15）を復元できなくなる。
`BannedSymbol`（クラス C）にも混ぜない——30 日リストは利用者が登録する禁止銘柄リストとは別の空売り専用リストである（#374）。

> **$16.67 と $12.50 を混同しない**。$16.67 は規制側の内訳が固定額 $5.00 から時価の 30% へ入れ替わる点、
> $12.50（＝ $5.00 ÷ 40%）は自前閾値との大小が入れ替わる点である（計画 §5 の 2026-08-01 追補2 で是正）。

### 規則 3 の一次ゲート — 借株可否（#417。空売り段階解禁の計画 ADR の決定 3 の 2026-08-06 改訂）

**規則 3 の一次ゲートは借株可否であり、借株料の閾値ではない。**

| 決定 3 が当初前提としたこと | 実測（moomoo 連携 PoC の計画 ADR の項目 3・実弾の信用口座・8 銘柄・1 時点） |
| --- | --- |
...
| 順 | 条件 | 拒否理由 | 根拠 |
| --- | --- | --- | --- |
| 0 | 文脈そのものが `null`（照会経路が無い） | `BorrowUnavailable` | 決定 3・空売り統制の実装 ADR の決定 2 |
| **1（一次ゲート）** | **`ShortPermit == false`**（供給元は moomoo `TrdGetMarginRatio.IsShortPermit`） | **`BorrowUnavailable`** | **決定 3 改訂・一次ゲートの実装 ADR の決定 1** |
| 2 | 料率が `null`（照会不能） | `BorrowUnavailable` | 決定 3 の未改訂部分 |
| 3（二次） | 料率 > 年率 20% | `BorrowCostExceeded` | 決定 3・決定 10（**残置**） |

- **新しい拒否理由コードは追加しない**（決定 3 改訂が明示）。`BorrowUnavailable` は「都度の借株需給に
...
- **一次ゲートが立てば二次（料率）は評価しない。** 借株できない銘柄に「料率が高い」も併記すると、
  監査ログの理由が実態より多弁になり、原因（可否 or 料率）の切り分けが濁る。
...
（moomoo 連携 PoC の計画 ADR の項目 5 は**未確認**）。計画は 2026-08-06 の利用者裁定で決定 4 を改訂し、
...
決定 4 の目的は「**同じ銘柄で繰り返さない**」ことである。誤りの代価は同じ重さではない。
...
#### 記録先と「推定」の明示（空売り段階解禁の計画 ADR の決定 4・決定 15）

| 記録先 | 内容 | 実装 |
| --- | --- | --- |
| 監査ログ | 消失した建玉・突合した自らの決済約定・処理中の決済・推定日時・禁止期限 | `BuyInInferred` → `AuditEntryFactory` |
| Discord 通知 | 上記＋「**強制買戻しと推定**」「確定した事実として扱わないでください」 | `NotificationFormatter`（Critical） |
| 日報 | 当日の**発生有無（推定）** | `ReportRenderer` §4 |
| 月報 | 当月の**発生回数**「強制買戻し（推定）N 件」 | 同上 |

**発生回数の集計元は推定の件数であり、`BuyInBanned`（拒否理由）の件数ではない**（決定 15 の 2026-08-06 追記）。
...
**供給が無い間は 0 件と表示しない**（決定 15）。日報・月報は「照会できませんでした（供給元がありません）」と書く。
これは自動縮小の既定（空列＝発動なし）と**向きが違う**——あちらは発火元が存在せず「発動なし」が事実として
正しいが、**推定経路は実在し発火し得る**ためである。

**照会 API は実装済みである**（#463。推定の照会 API と、観測の到達の記録）。
`GET /risk-controls/buy-in-inferences?from=&to=`（**`OwnerOrService`**）が**期間が観測に覆われているかと
推定行を同じ応答で返す**。2 つに分けると、呼び出し側が推定行だけを見て 0 件と判断する経路が作れるためである。
...
| **報告期間が観測に覆われていない**（`periodCovered` が `false`／旧版応答で項目が無い） | **`null`** —— **台帳が空でも 0 件にしない** |
| 期間が観測に覆われており推定 0 件 | **空列（正当な 0）** —— **正当な 0 を未供給に見せない** |
| 期間が観測に覆われており推定あり | 推定行 |

**「観測の到達の記録」の要求がこの区別を可能にしている。** 推定台帳は**推定が起きたときにしか行を書かない**ため、
行数 0 だけでは「観測が一度も届いていない（統制がまったく働いていない）」と「観測して 0 件だった（正常）」を
区別できない。**観測が届いた取引日を 1 日 1 行で永続化し、推定の有無に関わらず記録する。**

**［2026-08-08 改定］粒度は取引日の集合である**（計画側の観測到達記録の要求と、その裁定による）。
従前の「単一の最終観測時刻」では**報告期間を観測が覆っていたかを判定できず**、
**初回観測より前の期間**（初回観測が 8/20 で 7 月分の月報を作る）や**観測が途中で止まった期間**が
「正当な 0」として報告されていた。**覆っている＝期間の営業日がすべて観測されている**であり、
**1 日でも欠ければ未供給へ倒す。**

- **`RiskManagement:BaseUrl` 未設定なら未供給のまま**（`Unsupplied…`）＝安全側の既定である。
- **残余リスク: 祝日を含む営業日暦がまだ無い**（[#407](https://github.com/endazon/ai-stock-trading/issues/407)）。
  現在の暦（週末のみ除外）では祝日が「観測されていない営業日」として現れ**未供給へ倒れる**——
  **安全側**の誤りであり、逆にはならない。
- **統制状態参照画面は引き続き「取得できていません」を表示する。** 観測到達の記録側は入ったため
  結線できる状態だが、#463 の受け入れ基準の対象外として別 issue に切り出した。

### 規則 4 の適用は**口座単位**である（#420。空売り段階解禁の計画 ADR の決定 7 の 2026-08-07 追記）
...
| 空売り・株価 $5.00 未満 | 4210(c)(2) | **実装しない**（決定 7 が対象から外しており発火しない） |

したがって建玉ごとに異なる閾値が出る。**口座へ適用するのはその最大値**（最も厳しいもの）であり、
**当該注文が成立した場合の建玉もその候補に含める**（リスク統制の「常に厳しい方が効く」の延長）。
...
| 状況 | 振る舞い |
| --- | --- |
| **借株が許可されない（`ShortPermit == false` ＝ `IsShortPermit=False`）** | **`BorrowUnavailable` で拒否（一次ゲート。空売り段階解禁の計画 ADR の決定 3 の 2026-08-06 改訂）。料率が上限内でも通さない** |
| 借株料を照会できない（`null`） | `BorrowUnavailable` で拒否（同決定 3。**改訂は一次ゲートを移しただけで本縮退は残る**） |
| 強制買戻しの 30 日禁止期間中 | **`BuyInBanned`** で拒否。借株が成立していても立つ（`BorrowUnavailable` とは独立） |
| 照会経路そのものが無い（文脈が `null`） | 同上。**「照会できないなら通す」に倒さない** |
| 維持率の束（`MarginSnapshot`）が供給されず、かつ空売り建玉を保有している | `MaintenanceMarginBreach` で拒否（空売り統制の実装 ADR の決定 4） |
| 束はあるが**信頼できない**（株価・数量が 0 以下／必要証拠金が負）、または建玉 0 件で維持率を導出できない | 同上で拒否（#420。適用閾値の実装 ADR の決定 4）。**壊れた分母は維持率を実際より良く見せる**ため「導出できたか」で代用しない |
| 対象市場が米国株以外 | `ShortSellDisabled` で拒否（USD 建ての $5.00 下限を円建て株価が素通りするため） |

空売り比率 50% は**建玉総額に対する**比率であるため、**ロング建玉が無ければ 1 件目の空売りで 100% となり
成立しない**（＝空売りだけの建玉構成は作れない）。空売り段階解禁の計画 ADR の決定 9 の趣旨どおりの帰結である。
...
（2026-08-04 の環流記録。現行実装は「文字どおり維持」であり、裁定までは変更しない）。

### 情報収集の縮退による新規建て停止（不明は止める。#337 / #564）

情報収集が縮退している（必要な情報源が欠測している）あいだ、**新規建てだけを止める**。
**手仕舞い・損切りは止めない** —— 「止められない」より「閉じられない」ほうが危険であり、
この不変条件は判定コアの新規建て短絡が構造的に担保する（他の統制とまったく同じ位置・同じ形）。

停止の状態は、情報収集の側から**イベントで**供給される。ここで問題になるのは**供給が途切れたとき**である。

| 供給の状態 | 振る舞い | なぜ |
| --- | --- | --- |
| 現況を観測できていて、**止めるべき縮退が無い** | **新規建てを通す** | 唯一の「通してよい」状態である |
| 現況を観測できていて、**止めるべき縮退がある** | **新規建てを止める** | 統制の正常作動 |
| **現況をまだ一度も受け取っていない**（起動直後・**再起動直後**） | **新規建てを止める** | **「記録が無い」は「健全」ではない** |
| **最後の現況が失効した**（供給が途切れた） | **新規建てを止める** | 古い現況を無期限に信じない |

> 🔴 **是正した欠陥（#564）**: 従前、縮退の状態は**状態が変わったときだけ**届き、受け側はプロセス内に
> 保持していた。そのため**縮退が続いている最中にリスク管理サービスが再起動すると、次の変化まで停止が届かず、
> 情報が欠測したまま新規建てが再開しうる**状態だった。**統制が黙って外れる向き（フェイルオープン）である。**
>
> 是正は 2 つで対になっている。**片方だけでは効かない。**
> ① 情報収集が**巡回ごとに現況（止めているものの全量）を言い直す**——**止めるものが無いときも必ず言う**。
> ② リスク管理は**有効な現況が「止めるものは無い」と言っているときだけ新規建てを通す**。
> ①だけでは「再起動から次の巡回まで」が素通りのままであり、②だけでは復元されないまま止まり続ける。

観測には**有効期間**があり、情報収集の側が巡回間隔から宣言する（受け手は巡回間隔を知らないため）。
**受け手はその宣言を上下限でクランプする** —— 宣言をそのまま信じると、構成を誤ったときに
**鮮度の要求が事実上消え、是正した欠陥がそのまま戻る**。

**状態が変わったときのイベント（縮退・回復）は引き続き即時に効く**（次の巡回を待たない）。
ただし**鮮度を与えるのは現況の観測だけ**である —— **1 件の回復は「他の情報源も健全である」ことを保証しない**。

**拒否理由は増やしていない。** 「縮退している」と「縮退しているか分からない」は同じ理由で拒否する
（同じ統制が 2 つの理由に割れると、統制違反の集計と段階昇格の判定が分かれる）。区別は監査台帳
（現況の観測が全量で残る）とログが付ける。

### 拒否理由のクラス分類（空売り段階解禁の計画 ADR の決定 10）
...
**現金口座対応（#375。米国口座種別の計画 ADR の決定 4-5 と、口座種別供給の実装 ADR の決定 5）で 3 種を追加した**（序数は末尾 25/26/27）。

| 拒否理由 | クラス | 解除条件 | 根拠 |
| --- | --- | --- | --- |
| `CashAccountSettlementHold` | **A** | T+1 の決済 | 米国口座種別の計画 ADR の決定 4-5 が明示。制度・決済由来の事象であり `BannedSymbol`（クラス C）へ混ぜない |
| `GoodFaithViolationLimitReached` | **A** | **利用者の明示的な解除操作**（Discord `/gfv clear`。GFV 解除の計画 ADR の決定 2・3） | 統制が設計どおり作動した記録。**「違反記録の失効」ではない**——同計画 ADR の決定 1 が「記録は失効させない」と定め、解除は記録を消さず**追記**で表す |
| `BrokerAccountTypeUnverified` | **B** | 照会の成功（と設定値の一致） | 「取引を止めている状態そのものの記録」。kill switch / pause / 段階制約と同じ区分 |

**3 種を 1 つに畳まないのは拒否理由のクラス分類の規律による**——解除条件が互いに異なり、畳むと監査ログと
日報・月報が実態と食い違う（「決済待ちで止まっている」「口座が使えなくなりかけている」「OpenD が落ちている」は
運用者が採るべき行動がまったく違う）。**計画も同じ理由で 3 種を分けている**——起案時に計画が明示したのは
1 種のみだったが、2026-08-06 の環流を受けて **米国口座種別の計画 ADR の決定 4-5 が 2026-08-07 に 3 種を同名・同クラスで追認した**。**計画との差異は解消している。**
...
「動かす」統制である**——システムが自ら決済注文を発注する（設定変更・緊急停止のユースケースの比較表）。マージンコールは口座を
失う唯一の経路であり、米国市場の開場は日本時間の深夜（22:30〜翌 5:00）であって人の応答を待つ余地が無い。

| 項目 | 規則 | 出典 |
| --- | --- | --- |
| 発動条件 | **維持率 ≦ 適用閾値**（閾値ちょうどで発動＝**割り込む前に**動く） | §5・リスク統制の要求・設定変更のユースケース／等号の解釈は自動縮小の実装 ADR の決定 3 |
| 適用閾値 | 保有建玉の閾値のうち**最も厳しいもの**（`MaintenanceMarginPolicy.AppliedThreshold`） | 空売り段階解禁の計画 ADR の決定 7（**2026-08-07 追記が確定**）／自動縮小の決定 2・**適用閾値の決定 1** |
| 回復目標 | **適用閾値 + 5 ポイント**（閾値 40% なら 45%、規制側が効いて 50% なら 55%。**連動する**） | §5（2026-08-02 追補） |
| 縮小量 | 回復目標へ届く**最小限**（株数は切り上げ・**部分決済**する） | §5・設定変更のユースケース（一律 50% 減・建玉 1 件丸ごとは棄却） |
| 対象順 | **必要証拠金の降順**（同値は評価額降順→銘柄→市場で決定的に解く） | §5・設定変更のユースケース（含み損順・建玉時刻順は棄却） |
| 介在するもの | **無し**（利用者の承認も LLM 呼び出しも介在しない。同じ入力からは常に同じ出力） | 設定変更のユースケース「完全に機械的である」・AI 判断のガードレール |
| 3 統制との関係 | **いずれかが成立していても動く**（本統制自体が手仕舞いであるため） | 設定変更のユースケース・3 統制の優先順位を定めた計画 ADR |
...
#420・適用閾値の実装 ADR の決定 1。**新規建ての評価側と同じ算出点である**）。

```
維持率 R = 純資産 ÷ 建玉評価額の合計       … 計画 ADR の決定 7 の 2026-08-07 追記が確定（規制側の実効維持率と分母を揃える）
必要な削減 X = V − 純資産 ÷ 回復目標        … R ≧ 回復目標 なら X ≦ 0 ＝ 何もしない
```

**維持率の分子（純資産）は日中の実測値**であり、統制上限の基準 equity（`PortfolioSnapshot.Capital`＝
前営業日終値時点・当日不変。equity 比保持の決定 2）とは**別物**である。1 つにまとめると、日中に維持率が
落ちても判定が動かない（統制が沈黙する）。
...
| 同じ状況で空売りを積み増そうとする | **拒否する**（`MaintenanceMarginBreach`） | 縮小側で塞げない迂回を**積み増し側（#329・空売り統制の決定 4）が塞ぐ** |
| 回復目標へ到達できない（純資産が尽きている） | **全量決済する** | 何もしないのは最も危険な状態で統制が沈黙することを意味する |
| 全量決済した後の維持率 | **「建玉なし」**（0% ではない） | 建玉が無くなれば維持率という概念自体が消える |
| 値として成立しない建玉（株価・数量が 0 以下／必要証拠金が負）が 1 件でもある | **スナップショット全体を信頼せず決済しない**。ただし「健全ゆえの無動作」と区別できる状態（`SnapshotUntrusted`）を返し警告ログを残す | フィードが壊れている証拠であり、壊れたデータで決済する方が危険。**該当建玉だけ除くと分母が縮んで維持率が良く見え過少縮小へ倒れる**（自動縮小の決定 8） |
...
**定時常駐が `MaintenanceMarginReductionService.Evaluate()` を巡回する。** 従前は本サービスを解決して
呼ぶ本番コードが 1 行も無く（「供給なし」ではなく「未結線」）、供給元が実装されても発動し得なかった。
既定は**有効**（撤退・実DD の定期評価ドライバの既定無効とは意図的に異なる。実装ADR参照）——供給元が
「供給なし」を返す間は構造的に不活性のため、駆動自体の有効/無効ではなく供給元の実装差し替えが
発動可否の単一の制御点である。休場日はスキップする（ブローカー口座照会が意味のある値を返さないため）。
...
## 借株料の累計（#465。借株料は「建玉 × 取引日」で積み、未供給の日を別テーブル・別イベントで持つ）

空売り段階解禁の計画 ADR の決定 15 は統制状態参照画面と報告書へ「借株料の累計」を求めていたが、**その値をどう作るかを定めていなかった**。
借株料の累計を定めた計画 ADR（2026-08-07 Accepted）が 5 点を確定させたため、**記録側**を実装した。

| 同計画 ADR の決定 | 実装 |
| --- | --- |
| 決定 1 建玉を保有している**日ごと**に積む | 計上は「**建玉 × 取引日**」で 1 件（`BorrowFeeAccrualService.Accrue`）。主キーにより**冪等**（二度計上しても金額が狂わず、2 度目はイベントも出さない） |
| 決定 2 **建玉ごと**に積み、銘柄・口座へは**合算で導出**する | 一次識別子は (銘柄, 市場)。銘柄別・口座全体・月次は**すべて日次行の合算**（`SummarizeAccount` / `SummarizePosition` / `SummarizeByPosition`）。**別々に積むストア・列を持たない** |
| 決定 3 計上日の属する日・月へ帰属させる（**按分しない**） | 集計は期間で行を絞り込むだけ。**生涯累計と月次合計は別の値であり同じ語で呼ばない**（口座全体の生涯累計という入口は作らない） |
| 決定 4 **計上日の料率**を用いる。取得できなかった日は **0 として積まず未供給として記録する** | 料率は計上ごとに引数で受ける。未供給は**金額の欄を持たない別の型・別テーブル**（下記） |
| 決定 5 自前で日次に積み上げる | 日次の計上額は `料率 × 建玉評価額 ÷ 365`（決定 5 の机上確認の式）。**丸めない**（丸め規則は計画に無い） |

### 🔴 「計上できた日」と「取得できなかった日」を型で分ける

| 事実 | 型 | テーブル | イベント（監査） |
| --- | --- | --- | --- |
| その日の借株料を計上した | `BorrowFeeAccrual` | `borrow_fee_accruals` | `BorrowFeeAccrued` |
| **料率が取得できなかった** | `BorrowFeeUnavailableDay`（**金額の欄が無い**） | `borrow_fee_unavailable_days` | `BorrowFeeAccrualUnavailable` |

**未供給の側は金額を表現できない。** `decimal?`（null＝未供給）で畳むと `Sum()` が null を 0 として畳む経路が
自然に書け、決定 4 が禁じた「0 として積む」がスキーマ上表現可能なまま残る。
**0 を積むと「その日は費用が発生しなかった」と読め、Stage 1 の「借株料は 1 円も掛かっていない」という誤読が構造的に起こる。**

**累計の照会は必ず未供給の日数を伴う。** 集計の戻り値は `BorrowFeeAccrualSummary`（金額・計上日数・**未供給日数**）
の 1 種類であり、**金額だけを返す集計 API を公開面に置かない**（値の検査では、新しい集計 API が 1 つ生えれば破れる）。

### 🔴 供給は開始していない（借株料の計画 ADR の決定 6）

**`ShortFeeRate` の単位が確定しなければ累計の値そのものが定まらない —— 単位を取り違えると累計は 100 倍ずれる**
（`ShortFeeRate` の単位確定を PoC 項目 9 とした計画 ADR。期限 2026-08-31）。したがって次の 3 つを**意図的に**実装していない。

| 未実装 | 理由 | 担当 |
| --- | --- | --- |
| 日次の料率照会（`TrdGetMarginRatio`） | 実装すれば単位未確定の値が積まれる。計上サービスは料率を**引数**で受けるだけ | #331 / #342 |
| 日次で計上を回すスケジューラ | 登録＝供給の開始そのもの | #331 / #342 |
| 統制状態参照画面・日報・月報への表示 | `BorrowFeeAvailability` は `NotSupplied` のまま（供給可否をサーバが宣言する規約を維持） | PoC 項目 9 の成立後 |

**これは実装漏れではなく意図した遮断である。** 遮断は構造テスト 2 件（**T-10-275** 計上の公開面に `ShortFeeRate` が
生えたら赤／**T-10-276** 統制状態参照画面の依存に累計ストアが現れたら赤）で担保する
（一次ゲートの実装 ADR の決定 3 が全体前提条件の費用計算に対して行ったのと同型）。
...
| 業務ルール | 資金の 2% 到達で当日全停止・翌営業日までロックアウト（§5）。判定基準は**実現損益＋含み損益の合算**（A 案） |
...
### SIMULATE 限定のリスク上限プロファイル（#257 / #364。基準通貨反転の決定 4）

検証（ペーパー）環境では moomoo シミュレータ口座の残高（USD $1,000,000 / JPY ¥20,000,000）に見合う上限を用いる。

> **更新（#329。equity 比保持の決定 6）**: 金額系を equity 比で保持したことにより、**比率はスケール不変**と
> なった（基準資金を差し替えれば上限は自動的に比例する）。旧プロファイルの「金額系のみ 1,700 倍へスケールする」
> 機構（`SimulatorTradingDefaults.CreateRiskLimits` / `ScaleFactor`）は不要となり削除した。
> プロファイルが差し替えるのは**基準資金とペーパー段階の資金上限だけ**である。下表は更新後の姿。

> **更新（#364。基準通貨反転の決定 4）**: 基準通貨が USD になったため、プロファイルの基準資金も USD 建てで持つ。
...
リスク統制の要求本文は「kill switch・日次損失ロックアウト・一時停止は**いずれも手仕舞い（Close）と損切りは止めない**」と
...
| **ソフトウェア逆指値の成行決済**（moomoo SIMULATE の S1 のみ） | 発注執行が損切りライン到達を購読（下記「口座種別の軸」） | その記録の**残保護数量** | 通さない（機械的規則） |
...
| 逆指値が未受理（エントリーと同時に発注したが**確認できる形で拒否**された／接続できず**確実に送れなかった**） | エントリー未約定なら**取消**、約定済みなら**即座に成行で手仕舞い** | 発注執行の建玉解消（`ProtectiveStopCoverageLost`・Critical 通知） |
| 逆指値が滞留中に失効（取消・拒否・期限切れ） | **再発注**する。再発注が確実に送れなかった・拒否されたなら成行で手仕舞い | 発注執行の常駐ガード（定期巡回） |
| 🔴 逆指値（エントリー同時・再発注）を**送信したが結果を確認できない**（届いたか不明） | 🔴 **取消も成行もせず据え置く**（逆指値が生きていれば、建玉を落とすと逆指値だけが残り、発火で反対方向の建玉になる）。**同じ逆指値も送り直さない**（送る前に予約を取り、不明のあいだ予約を残す——以前は巡回ごとに同じ逆指値を送り直し、届いていれば同じ建玉に逆指値が並んだ）。保護記録は「送信結果待ち」で巡回に残り、注文照会の突合が発注済みと確定すればその注文を保護として引き取り、予約が解放されれば未発注として張り直す。**据え置き中に建玉が消えても記録を閉じない**。Critical 通知は「失敗」「未受理」と言わず、**証券会社の画面で逆指値の有無を確かめ、無ければ手で置くか手仕舞う**よう求め、解決まで約 1 時間ごと（と再起動のたび）に出し直す。逆指値の決済意図を運ぶので取引台帳は逆指値の武装と同じ由来で押さえる | `ProtectiveStopCoverageLost`（Remediation=StopDispatchIndeterminate）＋逆指値の予約 |
| 逆指値の予約を記録できない（記録の障害） | **逆指値を送らない**（取消も成行もしない）。エントリーと同時なら保護記録を送信結果待ちで残し、常駐ガードが次の巡回で建玉を確かめて張る（建玉がブローカーに現れてから）。ガードの再発注なら記録を変えずに巡回のたびに試み直す。Critical 通知は「予約できず未送信」と書き（「未受理」「解消にも失敗」とは言わない）、エントリーと同時なら手で逆指値を置かないよう、ガードの再発注なら逆指値なしの建玉が残っていることを伝え、約 1 時間ごとに繰り返す | `ProtectiveStopCoverageLost`（Remediation=StopReservationFailed） |
| 建玉が消滅した（owner 手仕舞い・自動縮小・強制買戻し等） | **残存逆指値を取り消す**（反対建玉の防止）。🔴 ただしブローカーの建玉 0 を「消滅」と読むのは、**エントリー注文が約定していて、約定を知った後に建玉を照会し直しても 0** のとき、または**エントリーが約定しないまま終わった**（取消・失効・拒否）ときだけである。エントリーの指値が**まだ約定していない**（受理済み・一部約定のまま生きている）あいだは、建玉 0 でも逆指値を取り消さず、失効した逆指値・送信結果待ちの保護記録も閉じない——閉じると約定した後の建玉が逆指値なしで残る。約定して建玉が現れた巡回で通常どおり守る（逆指値が失効していれば張り直す） | 同ガード（エントリー注文の状態は発注の記録が終端ならそれを、まだなら注文照会で確かめる） |
| 建玉が 0 に見えるが、エントリー注文の状態を確認できない（照会の失敗・答えが無い・発注の記録が無い） | **取消も記録の完了もせず据え置く**（まだ建っていないのか、建って消えたのか分からない）。何も送らない。Warning 通知で、証券会社の画面でエントリー注文・建玉・逆指値を確かめる（建玉が無く逆指値だけが残っていれば手で取り消す）よう求め、状態が分かるまで約 1 時間ごと（と再起動のたび）に出し直す。決済意図は運ばないので取引台帳は押さえない | `ProtectiveStopCoverageLost`（Remediation=EntryStateUnknown） |
| 建玉解消も失敗した（エントリーの取消に失敗し、照会でも約定を確かめられない／成行手仕舞いが**確実に未発注**＝接続確立の失敗。常駐ガードでは発注先に成行の能力が無い場合も） | Critical 通知で**人手対応**を求める（黙って残さない）。🔴 **その後は原因で分かれる。** エントリー直後の建玉解消で起きた場合は保護記録が無く巡回の対象に入らないため、**撃ち直しも再通知も無い**（通知は 1 回きり。同じ承認の再配送も出し直さない）。逆指値が滞留中に失効した側は記録が `Active` のまま残り、常駐ガードが巡回のたびに改めて評価して（成行を送れる発注先なら）撃ち直し、失敗が続くあいだは巡回のたびに通知が出る | `ProtectiveStopCoverageLost`（Remediation=None） |
| 成行手仕舞いを証券会社が**確認できる形で拒否した**（拒否・取消・期限切れが返った） | 🔴 **手仕舞い済みとは扱わない（建玉は残っている）。** 送った成行は生きていないので決済意図を運ばず、取引台帳は在庫を押さえない。Critical 通知は「拒否された・建玉が残っている」と伝え、証券会社の画面で確かめて手で手仕舞うよう求める。🔴 **その後は原因で分かれる。** エントリー直後の建玉解消で起きた場合は保護記録が無く、**巡回も撃ち直しも再通知も無い**（通知は 1 回きり。原因を取り除いてもシステムは再試行しない）。逆指値が滞留中に失効した側は記録を閉じず `Active` のまま試行番号を進め、次の巡回で撃ち直す。同じ理由の拒否が 3 回続くと成行は送らなくなるが、記録は閉じず巡回を続け、約 1 時間ごと（と再起動のたび）に通知を出し直す | `ProtectiveStopCoverageLost`（Remediation=CloseRejected） |
...
**エントリーそのものの送信結果が不明**だった注文を、後から注文照会の突合が「発注済み」と確定したときは、
**承認時の損切り手法で保護レグを張る**（ブローカー側逆指値・代替注文種別はその場で発注し、結果の扱いは上表と同じ。
ソフトウェア逆指値はエントリーの前に記録済みで配置の事実だけを通知する。逆指値を持たない手法〔S2〕と、承認時の手法を記録する前に
止まった注文には張れず、Critical で知らせる）。このため、ブローカー側逆指値・代替注文種別の新規建ては、エントリーを送る前に
承認時の手法・損切りライン・数量を巡回の対象外の形で記録しておく（建玉が生じない・保護を諦めたときは閉じる）。
...
- **画面と日報での表示・変更**: 手法は**リスク設定画面の専用フォーム**（発注先の変更の直後）から選ぶ（理由必須。BFF は
  ...
  発注先の行の直後に参照表示する。**日報**は §4 の子節「損切りの実行機構（当日）」に 2 行を並べる。1 行目「選ばれていた手法（承認時点）」は
  当日の新規建ての承認を**承認時点の手法**（承認が運ぶ値。日報を作る時点の設定値ではない）ごとに数える（監査台帳の承認を引く）。
  2 行目「実際に適用された手法（発注執行の解決結果）」は、同じ承認を発注執行がどう解決したか（下記「解決結果の記録」）を承認と
  判断 ID で突き合わせて数える。内訳は S0〜S3 の区分ごとで、2 行目は発注しなかった承認を「見送り（実際の発注先が SIMULATE でない）」の
  区分に数える。各行の内訳の合計はその行の「計」と一致する（件数 0 の区分は出さない）。2 行が食い違う承認があれば、選択 → 適用と理由
  （実際の発注先が SIMULATE でないための見送り・空売りの新規建ては S0・未知の値は S0）を件数つきで書く。解決結果の記録が見つからない承認は、一致とも食い違いとも
  書かず件数を別に書く。どちらかの記録を照会できなければ「なし」と書かず「照会できませんでした」と書く。件数は承認の件数であり、
  解決の後に発注を見送った場合（逆指値価格が無い等）・約定の有無は反映しない。**月報**は §6 の子節「損切りの実行機構（当月）」に
  「当月の損切りの実行機構: S0〜S3 の日数／選択と実際が食い違った日数: n 日」の 1 行を書く（個々の日は日報に任せる）。日数の内訳は
  S0〜S3 の 4 区分で、1 日に複数の手法があれば各手法に重複して数え、その日数を併記する。見送りは内訳に数えない（食い違った日数には数え、
  見送りがあった月はその旨を併記する）。
  日は日報と同じ JST の暦日（承認の時刻）で数える。週報には出さない。
- **解決結果の記録**: 発注執行は新規建ての承認の手法を解決するたびに `StopLossMethodResolved`（承認の手法・実際に適用した手法〔拒否なら
  無し〕・理由・実際の発注先）を発行し、監査台帳が記録する。解決の後に見送った回にも出る。手仕舞い・完了済み／見送り済みの承認の再配送では
  解決しないので出ない。同じ承認に 2 件あれば時刻の遅いほうを採る（通常は生じない）。送信後に届いたか不明で例外に終わった承認は、再試行も予約の競合で例外に終わるため解決結果が出ず、日報・月報では「解決結果の記録が見つからない」に数えられる。解決結果の発行の失敗は発注執行の他の発行を止めない。
...
- **発注執行の解決順**（新規建てにのみ適用。手仕舞いは手法に関わらず発注する）:
  ...
  5. S1 → 保護逆指値を発注せず、**エントリーを送る前に**ソフトウェア逆指値を記録する（下記「S1 の動作」）
  6. S3 → 保護レグを代替注文種別で発注し、試行（種別と拒否理由）を記録する。**結果の扱いは S0 と同じ**
  7. 未知の値 → S0 と同じ扱い（緩い側へ倒さない）
- **常駐ガードとの関係**: S2 の建玉は保護逆指値の記録を作らないため、失効検知の巡回対象に入らない
  （失効扱いで手仕舞われることが構造的に無い）。S1 の記録はブローカーへ照会せずに巡回する（下記）。
  S0 の建玉に対するガードの挙動は、同じ銘柄・方向に S1 または S2 の建玉が無い限り変わらない。S1 の建玉は差し引いて判定するが、
  🔴 **S2 の建玉は記録を持たないため差し引けない**（下の S1 の表の「同じ銘柄に S0 と S2 の建玉が併存」。残る制約）。
...
- **S3 の詳細**: 試す注文種別は発注執行の構成（`Broker:Moomoo:AlternativeStopOrderType`。既定はストップリミット。
  ...
  こちらはその下位の探索パラメータである。ストップリミットの**指値**は発火価格から不利側へ既定 1%
  （`Broker:Moomoo:StopLimitOffsetRatio`。0〜10%）ずらす。🔴 **発火価格と同値に置かない** ——
  急落・急騰で板が飛べば約定せず、「保護レグを置いたのに保護されない」状態になるためである。
  トレーリングストップの幅は「エントリーの判断価格 − 損切りライン」の絶対額とする。
  代替注文種別の能力を持たない発注先へ S3 が届いたら、S0 へ黙って読み替えず**発注せず見送る**。
...
| 場面 | 動作 |
| --- | --- |
| 新規買いの武装の前提条件 | 🔴 **同一銘柄・同方向に「帰属不明の建玉」（その方向の建玉 − 有効な保護記録が<u>その巡回で実際に動かせる株数</u>の合計 > 0）があるあいだは武装しない。** 🔴 **引くのは帳簿の主張ではなく実際に動かせる株数である**——帳簿の主張で引くと、未確定の観測を抱えた記録（実際には 1 株も動かせない）が他の建玉を「帰属済み」に見せ、帰属不明が 0 と読まれて武装してしまう。🔴 **判定の前に、終端したエントリーの約定数量を確定する**（常駐ガードの巡回を待つと、直前に約定したばかりの自分の建玉が帰属不明に見え、同一銘柄への 2 本目を取りこぼす）。 ブローカーの建玉は銘柄単位の純額でしかなく、他の実行機構の建玉・手動で建てた建玉と自分の建玉を区別できない——帰属不明の建玉がある銘柄で武装すると、その建玉を**この機構の損切りラインで売る**。**エントリーを送らず、建玉を持たずに見送る**（「保護レグを張れない新規建てでは建玉を持たない」と同じ倒し方）。**建玉を照会できない・照会の能力が無い場合も「ある」側へ倒す**（不明を「無い」と取り違えない）。判定は最初の武装のときだけ行う（再配送では触らない）。他の実行機構の経路は変わらない |
| 新規買いの発注 | ブローカーへ逆指値を出さない。**エントリーを送る前に**損切りライン（承認の損切り価格）・数量を記録する。エントリーが生きていれば `SoftwareStopArmed`（Warning）、建玉が生じなければ（拒否・失効・接続断の見送り）記録を完了にする |
| 損切りライン到達（市場監視の検知・既定 60 秒周期） | 同じ銘柄・方向の有効な記録のうち、**記録が到達の検知より前に作られ**、**検知時の価格が記録自身の損切りラインに達している**ものを対象にする。**先に到達を記録してから**決済に進む。一度到達したら、処理時点で価格が戻っていても決済する |
| 市場監視が比べる損切りライン（同じ銘柄に記録が複数あるとき） | 取引台帳は保有中の建玉を**エントリーごと**に持ち、**残っているエントリーのうち最も保護的なライン**（買い建て: 最も高い／売り建て: 最も低い）を市場監視へ渡す。市場監視はこのラインで到達を 1 件出し、上の行のとおり**記録自身のライン**に達した記録だけが決済される——後から低いラインで建て増しても、先に建てた記録の損切りは遅れない（以前は最新エントリーのラインと比べていたため、稼働環境で 713 株の損切りが 0.79 ドル遅れる配置があった）。減少（一部決済・取り込み）は**古いエントリーから**削る（外部要因による減少の割り当てと同じ向き）。🔴 **「古い」は発注（承認）の順であり、約定の順ではない**——先に出した指値が板に残り、後に出した注文が先に約定することがある（稼働環境で実際に起きた）。約定の順で削ると、外部要因の減少のあと記録が残っているエントリーのラインを台帳が先に捨て、その損切りが遅れる。在庫・平均取得単価の畳み込みは約定の順のままである。🔴 台帳はどの決済がどのエントリーを閉じたかを持たないため取り違えることがあり、そのときは「**ラインが実際より保護的**」側へ倒れる——到達は出るが決済する記録が無い（到達の通知は出る）。損切りラインの記録が無いエントリーは「ラインが無い」とは読まず、既定比率の近似で見積もって候補に入れる。日報のポジション一覧と取引判断が読む「記録上の損切りライン」も同じ値である。数量（建玉全体）と並べて 1 本だけ示すため、日報の凡例・取引判断の保有状況・到達の通知には「建て増しした建玉ではエントリーのうち最も保護的なライン（全量のラインではない）」と添える |
| エントリーがまだ受付中・一部約定 | 残りを取り消し、**終端になってから**決済する（終端になるまで次回へ据え置き）。約定 0 で取り消したら `SoftwareStopExecuted`（エントリー取消・Warning） |
| 持ち分（残保護数量） | 記録が**自分の守っている株数を状態として持つ**（建玉の純額から毎回計算し直さない）。エントリーの約定が終端で確定し、自分の決済で減り、外部要因（手動決済・強制決済・ブローカー側逆指値の約定）による減少は**観測した時点で一度だけ**割り当てて記録する。ブローカーの建玉照会は銘柄単位の純額でしかなく、毎回引き直すと持ち分が巡回ごとに揺れて「売り過ぎ」か「損切りが黙って出ない」のどちらかへ倒れるためである |
| 外部要因による減少の割り当て | 超過は**帳簿だけの記録（システム側）から先に**割り当て、**実在する逆指値を持つ記録（ブローカー側）は最後**に回す（それぞれの中では古い記録から）。ブローカー側の記録に注文照会が滞留を返すこと自体が「その建玉はまだ在る」証拠であり、順序を作成時刻だけで決めると**実在する逆指値が取り消される**（無音かつ不可逆）。🔴 **超過は「観測」として積むだけで、残保護数量（帳簿）は確定するまで書き換えない。同じ観測が 2 巡回連続したときだけ確定する。** 未確定のあいだは**記録を完了させず、ブローカー側の逆指値も取り消さない**。照会不能（不明）は観測とみなさない |
| その巡回で動かしてよい株数 | **残保護数量から、まだ確定していない観測分を引いた値**である。決済数量の上限と、ブローカー側の建玉残の判定にはこの値を使う。**一時的な計算であり、帳簿には書かない**（引かないと、同じ巡回の別の記録が古い建玉を主張して売り過ぎる） |
| 観測を数え続けてよいかの判定 | **「建玉が主張の合計より多い」ことは、建玉が戻った証拠ではない。** 純額の中には、帳簿に現れない株数——①**送信済みで建玉照会にまだ反映されていない決済**（受理済み・未約定。決済した記録はその巡回で完了するため、残っている記録からは見えない）②**確定前の新規エントリーの<u>約定済み</u>株数**（まだ主張になっていない。**承認数量ではなく約定数量で数える**——承認数量で数えると、1 株も約定していない新規エントリーが 1 件あるだけで判定が効かなくなる）——が含まれ得る。これらが超過の消失を説明するなら、超過は消えていないものとして観測を数え続ける |
| 超過が本当に消えたとき | **帳簿には何も書き戻さない**（帳簿を書き換えていないため、戻す＝復元する操作そのものが存在しない）。🔴 **未到達の記録では、超過が消えたことを確定と同じ回数（2 巡回）連続で観測したら、観測そのものを捨てる**（確定と対称にする）。捨てないと、建玉照会が **1 巡回だけ**過少に返っただけでその記録の動かせる株数が**恒久的に 0** になり、記録は有効・帳簿も無傷のまま**到達しても 1 株も決済しない**（無音で保護が失われる）。🔴 **到達済みの記録では捨てない**——捨てた瞬間に成行決済を出すため、取り違えたときの倒れ方が**不可逆（反対建玉）**になる。到達済みの記録は据え置いたまま下の 2 つの通知で人手へ回す |
| 動かせる株数が 0 のまま続いたとき | 🔴 **猶予（15 分）を過ぎたら `SoftwareStopExecuted`（保護の停止・**Critical**）を記録ごとに 1 回出す**（**決済は出していない**）。この状態の記録は有効・帳簿も無傷なので、状態や帳簿だけを見ても異常が分からない——違いは「到達しても 1 株も決済しない」ことだけである。**到達の有無に依らない**（据え置き継続の通知は到達済みの記録にしか効かない）。動かせる株数が戻ったら計時をやり直す |
| 守る不変条件 | **有効な記録の主張の合計 ＋ 送信済みで建玉照会に未反映の決済 ≦ その方向の建玉** |
| どの記録も主張していない建玉があるとき | **常駐ガード（と建玉観測の常駐）が検知し、`SoftwareStopExecuted`（帰属不明の建玉・Warning）を出す**（その方向の建玉 − 有効な記録の主張の合計 − 上の「帳簿に現れない株数」> 0）。**検知であって是正ではない**——建玉を売らず・記録も作らず・主張も動かさない。武装の前提条件は**武装の時点しか見ない**ため、これが無いと**武装より後に他の実行機構の建玉が現れる経路**が、どの通知も出ないまま無保護で残る。**同じ状態では毎巡回鳴らさない**（株数が変わったときか、一定間隔）。走査は**建玉の側から**行い、**システム側の記録の足跡がある銘柄・方向に限る**（有効な記録・完了した記録のいずれか）——**システム側の記録が 1 件も無い構成では何も出さない**。🔴 **検知は 2 か所から走る**——常駐ガード（有効な記録が 1 件以上ある巡回だけ）と、**建玉観測の常駐**（既定 600 秒・有効な記録の有無に依らない。建玉照会は観測の 1 回をそのまま使い、増やさない）。建玉を照会できなかった巡回では検知しない（「帰属不明なし」と読まない）。**建玉照会から銘柄が消えた（純額 0 の）巡回でも、通知済みの記録を消す**——解消したあと再発した同数の帰属不明は、一定間隔を待たずに改めて知らせる（下の「検知の射程」を必ず読むこと） |
| 外部要因で保護対象を減らしたとき | 確定した時点で `SoftwareStopExecuted`（保護対象の縮小・Warning）を**記録ごとに 1 回**出す（**決済は出していない**）。ブローカー側の記録が 0 になって逆指値を取り消す場合、この通知が唯一の痕跡になる |
| 決済 | **その巡回で動かしてよい株数**を成行で決済する（上の不変条件を保つ限り、累計の決済数量が保有数量を超えないため反対建玉を作らない）。0 なら決済せず、未確定の観測があれば据え置き、無ければ記録を完了にする |
| 二重決済の防止 | 決済の注文識別子はエントリーと試行番号から決定的に作る。①同じ識別子の発注記録があれば再送しない ②送る前に予約表で識別子を確保し、取れなければ送らない（到達の再発火・再配送・ガードとの並行） ③受理した数量だけ残保護数量を減らし、0 になった記録は以後の到達では対象にしない |
| 決済が受理された | 受理した数量だけ残保護数量を減らし、`SoftwareStopExecuted`（成行決済・Warning）を出す。**0 になったときだけ**記録を完了にする（部分的にしか決済できていない記録を閉じると、残りが無保護のまま黙って残る）。リスク管理が決済レグを取引台帳へ結線し、約定は約定追跡が届ける |
| 決済が拒否された | 試行番号を進め、**到達の記録は残したまま**撃ち直しを続ける（価格がラインの内側へ戻って到達が途絶えても、常駐ガードが撃ち直す——かつての「到達 1 回あたり 3 試行で到達の記録を消す」打ち切りは、価格が戻ると二度と撃たず出口を塞ぐため撤去した）。**続けて売れない記録には、記録ごとの待ち時間を置く**: 連続失敗 n 回目の後 min(30 秒 × 2^(n−1), 15 分) が過ぎるまで、到達でもガードでも新しい成行を送らない（待ち時間は記録に保存され、再起動しても続く）。受理された決済が **1 株も約定しないまま**終端して保護を戻した場合も連続失敗として数え、1 株でも約定していれば数えを 0 へ戻す。`SoftwareStopExecuted`（決済拒否・**Critical**・手動決済を促す）は**連続失敗 3 回目**に出し、以後 **4 回ごと**に出す（待ち時間が上限に達した後はおよそ 1 時間ごと。失敗のたびのエラーログは残す）。**前回の到達から 5 分以上空いた到達**（閉場を挟んだ・価格が一度戻った）では数えと待ち時間をやり直してすぐ撃つ |
| 接続断・建玉を照会できない・取消待ち | **据え置く**（イベントなし）。到達の記録が残るため、次の到達を待たずに常駐ガード（既定 30 秒）が再試行する。**据え置きが猶予（既定 15 分）を過ぎたら、記録ごとに 1 回だけ Critical**（据え置き継続）を出す——再試行は続けるが、無音のまま損切りが出ない状態を残さない |
...
| 同じ銘柄に S0 と S2 の建玉が併存（🔴 **残る制約**） | S2 の建玉は保護記録を持たないため、**差し引けない**。S0 の側から見た建玉残に S2 の数量が入るので、**S0 の建玉が消えても S2 の建玉が残っていれば S0 の逆指値を取り消さない**（残った逆指値が約定すると S2 の建玉を決済する）。S0 の逆指値が失効したときも、S0 の建玉が消えていて S2 の数量があれば、S0 の数量を上限に逆指値を張り直す（または手仕舞う）ため、S2 の建玉を決済し得る。**今は起きない**——SIMULATE ではブローカーが逆指値を受け付けないため S0 の建玉が残らない（手法を途中で S0 と S2 の間で切り替えた場合にだけ理論上あり得る）。回避は運用で行う: **同じ銘柄に S0 の建玉が残っている間は S2 へ切り替えない**（切り替えるなら、その銘柄の S0 の建玉を先に手仕舞う）。直し方（S2 にも記録を持たせる・併存する新規建てを見送る等）は未決で、実弾・S3 の常用より前に決める |

- 🔴 **受容した制約（原理的な限界）**: **ブローカーの建玉照会は銘柄単位の純額しか返さないため、次の 2 つを区別できない。**
  ①**建玉照会が 1 巡回だけ過少に返った**（＝主張を戻すべき。戻さないとその記録の損切りが二度と出ない）
  ②**建玉が本当に減った後で、保護記録を持たない建玉が同じ銘柄・同方向に現れた**（＝主張を戻してはいけない。戻すと他人の建玉を売る）。
  **どちらも建玉の系列が完全に同一である**（0 株 → 10 株 → 10 株）。どんな規則でも両方は満たせないため、
  **①として扱う（主張を戻す）ことを明示的に受容する**。
  - **起きること**: 未到達の記録が減少を観測してから次の観測までのあいだ（常駐ガードの巡回 1 回ぶん・既定 30 秒。
    観測を捨てるまでは約 2 巡回＝約 60 秒）に保護記録を持たない建玉が現れると、次の到達でその建玉を
    **この機構の損切りラインで成行決済する**。
  - **起きないこと**: 🔴 **売り過ぎ（反対建玉）にはならない**——決済数量は常に「その巡回で動かしてよい株数」
    （≦ その方向の建玉）で頭打ちである。
  - **射程**: ペーパー（SIMULATE）限定である（実弾ではこの機構を選べず、空売りは常にブローカー側逆指値）。
  - **恒久対策**: ブローカーの注文一覧から「自システムが出していない約定」を特定できるようにすること（別課題）。
    それまでこの挙動は仕様であり、テストで固定してある。
- 🔴 **検知の射程（有効な記録が無い口座でも走る）**: 常駐ガードは**有効な記録が 1 件も無い巡回では建玉を照会しない**
  （無駄な往復を避けるための既存の規律）。そのため、**受理された後に 0 約定で取り消された決済の残り**
  ——決済を送った記録はその時点で完了し、巡回の対象に 1 件も残らない——は、ガードからは検知されない。
  この配置は**建玉観測の常駐**（既定 600 秒・建玉照会を無条件に行う）が同じ観測で検知し、帰属不明の建玉として 1 回知らせる
  （**1 銘柄しか持たない口座でも無音にならない**。気づくまでの遅れは最大でその間隔）。
  - **走らない構成**: 建玉観測を無効にした構成（`Reconciliation:Positions:Enabled=false`）では常駐ごと止まるため、
    この配置の能動的な通知は無い（気づける経路はその銘柄へ次に新規建てを出したときの「見送り」だけになる）。
    内蔵 paper 構成は建玉照会を持たないため、どちらの経路も走らない。
  - **建玉を照会できなかった巡回では検知しない**（通知済みの記録も消さない）。照会不能が続くあいだは知らせが遅れる。
- 🔴 **この機構を選ぶときの運用上の前提条件**（監査がこの 3 つを満たすかぎり取引が成立することを実測した）:
  ①**切り替えの時点で、他の実行機構由来の建玉・手動で建てた建玉を手仕舞っておく**
  ②**終端していない一部約定が無い** ③**受理された後に取り消された決済の残りが無い**。
- **残る制約**: S2 の建玉は記録を持たないため按分できないが、**武装の前提条件**（上の表の 1 行目）により、帰属不明の建玉がある銘柄では S1 の新規建てがそもそも出ない。武装より**後**に他の実行機構の建玉が現れる経路では、**到達済みの記録**は建玉を外部で失ったことを一度観測すると以後その建玉を主張し直さないため、他の実行機構の建玉を売らない。🔴 **未到達の記録では成り立たない**——未到達の記録は観測を捨てて主張を取り戻すため、上の「受容した制約」のとおり売り得る。その代わり**到達済みの記録は動かせる株数が 0 のまま自動では戻らない**（確定するか人手が介入するまで決済が出ない）——**無音ではない**（据え置き継続と保護の停止のどちらも要人手対応で出る）が、**自動では回復しない**。エントリーの約定が**終端していない**あいだの記録は 1 株も主張しないため、**同一銘柄・同方向へ続けて新規建てを出すと 2 本目が「帰属不明の建玉がある」と判定されて見送られ得る**（安全側の見送り。**終端していれば武装の判定が確定を先に通すので見送られない**）。帰属不明の建玉の検知は、有効な記録が 1 件も無い口座では**建玉観測の常駐からだけ**走る（上の「検知の射程」。建玉観測を無効にした構成では走らない）。
  moomoo アダプタは送信後の例外を拒否として扱うため、実際には届いていた決済に次の試行が重なり得る（次の試行の前に建玉を照会し直すため、
  先の注文が約定済みなら重ならない）。市場監視は時間外にも到達を出すため、時間外の成行拒否は打ち切りで抑える。
...
- **評価基準**: 含み損益は日次終値（全体前提条件 §5 の「評価損益 = 日次終値」）で算出する。集計はリスク管理ホスト。
...
| 条件 | 振る舞い | 記録 |
| --- | --- | --- |
| 日次損失上限に到達（合算） | 新規建てを拒否。手仕舞いは許可 | `RejectionReason.DailyLossLimitReached`（監査ログと Discord 通知が購読） |
| ロックアウト（翌営業日まで） | ホストが当日ロックとして保持・翌営業日に解除 | ホスト側の状態管理（`RiskEvaluator` はステートレス） |
...
- [ ] ロックアウト（翌営業日まで）の状態管理をリスク管理ホストで実装する
...
- [x] 金額系上限の適用が手仕舞い（Close）・損切りを止めない
...
- 実装ADR: 維持率の適用閾値は口座単位（建玉ごとの閾値の最大値）とし、算式と条文選択を単一情報源へ寄せる／維持率割れの自動縮小の決定的規則（算式・発動条件・最小限・記録先）／
  空売り専用統制は「新規売り建て」を起点に判定し、外部照会が欠けたら通さない（拒否理由のクラス分類も持つ）／金額系の統制上限は equity 比で保持し解決点を 1 つに閉じる／日次損失上限は実現損益と含み損益の合算で判定する／
  エントリー・手仕舞いは建玉効果で判定し、売買方向から分離する／判定の基準通貨を USD へ反転する／統制の金額は基準通貨で判定し、換算は判断境界の 1 点で行う／
  SIMULATE 限定のリスク上限プロファイル／約定状態は短周期ポーリングで追跡し、台帳は `OrderId` 単位の単調 upsert で受ける／計画確定値の適合は「計画値テーブル＋既知逸脱レジストリ」で検査する
- 作業仕様書: 作業仕様書: 維持率の適用閾値を口座単位へ揃える（適用閾値の口座単位化）、仕様書: 判定の基準通貨を JPY から USD へ移行する（判定の基準通貨 USD 移行）、作業仕様書: 維持率割れによる建玉の自動縮小（自動縮小）、
  作業仕様書: リスク統制コアの再実装（#329・第 2 段階）（再実装・第 2 段階）、作業仕様書: リスク統制コアの再実装（#329・第 1 段階）（再実装・第 1 段階）、仕様書: リスク評価コアの是正
- テスト仕様書: [リスク統制（再実装）](../tests/FR-10_risk-controls-tests.md)、[リスクガードコア](../tests/FR-10_risk-guard-core-tests.md)
- データ仕様書: [リスク管理ドメインの集約](../data/risk-management-aggregates.md)
- 計画への環流（いずれも起草のみ・送付は未実施）: 空売りの拒否理由コードの不足、空売り比率 50% の構造的含意、
  維持率の算式・複数建玉時の適用閾値・信用買いの規制維持率
  （**送付済み・裁定済み**。空売り段階解禁の計画 ADR の決定 7 の 2026-08-07 追記）、追記の取り込みで残った 3 点

## 未決事項

- ロックアウトの具体的な状態保持・解除タイミングはリスク管理ホストで確定する。
- ~~**判定通貨の USD への移行**（計画 §3 の 2026-07-31 改定への追随）~~ →
  **[#364](https://github.com/endazon/ai-stock-trading/issues/364) の基準通貨反転で完了**
  （仕様書: 判定の基準通貨を JPY から USD へ移行する）。統制値の書き換えは生じなかった。
  残る未実装は**表示通貨 JPY への換算表示**である。
- 比率化に伴う設定の永続化（JSON）の互換性。旧プロパティ名の行は読めなくなるため、切替計画で扱う。
- **空売り比率 50% の分母**（建玉 0 件のとき空売りを開始できないこと）は計画側の裁定待ちである
  （環流記録に残す）。裁定に応じて判定式とテスト
  （T-10-156 / T-10-171）を追随させる。
- **保有建玉数を数える粒度**（建玉キーに商品種別が無い）は #332 の 3 値化と同時に見直す。
- リスク設定画面（リスク上限の保存）は API 契約の変更により現状 400 で拒否される（[#362](https://github.com/endazon/ai-stock-trading/issues/362)。
...
- ~~**維持率の算式・複数建玉時の適用閾値・信用買いの規制維持率**は計画に記述が無く、実装が安全側で仮置き
  ...
  意図の追認を環流記録に残している。
  moomoo が返す維持率の定義が本算式と異なる場合は新 IADR で改める。
- 自動縮小の**定期評価の周期**（何秒ごとに維持率を見るか）は計画に無く、供給元と同時に決める。
- 強制買戻し（buy-in）の**検知・通知と禁止リストの永続化**は未実装（作業仕様書: リスク統制コアの再実装〔#329・第 2 段階〕の未決事項 §2）。
