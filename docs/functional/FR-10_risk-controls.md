...
| 場面 | 動作 |
| --- | --- |
| 新規買いの武装の前提条件 | 🔴 **同一銘柄・同方向に「帰属不明の建玉」（その方向の建玉 − 有効な保護記録の主張の合計 > 0）があるあいだは武装しない。** ブローカーの建玉は銘柄単位の純額でしかなく、他の実行機構の建玉・手動で建てた建玉と自分の建玉を区別できない——帰属不明の建玉がある銘柄で武装すると、その建玉を**この機構の損切りラインで売る**。**エントリーを送らず、建玉を持たずに見送る**（「保護レグを張れない新規建てでは建玉を持たない」と同じ倒し方）。**建玉を照会できない・照会の能力が無い場合も「ある」側へ倒す**（不明を「無い」と取り違えない）。判定は最初の武装のときだけ行う（再配送では触らない）。他の実行機構の経路は変わらない |
...
- **残る制約**: S2 の建玉は記録を持たないため按分できないが、**武装の前提条件**（上の表の 1 行目）により、帰属不明の建玉がある銘柄では S1 の新規建てがそもそも出ない。武装より**後**に他の実行機構の建玉が現れる経路では、**建玉を外部で失ったことを一度観測した到達済みの記録**は以後その建玉を主張し直さないため、他の実行機構の建玉を売らない。その代わり**到達済みの記録は動かせる株数が 0 のまま自動では戻らない**（確定するか人手が介入するまで決済が出ない）——**無音ではない**（据え置き継続と保護の停止のどちらも要人手対応で出る）が、**自動では回復しない**。また、エントリーの約定が確定するまでの記録は 1 株も主張しないため、**同一銘柄・同方向へ続けて新規建てを出すと 2 本目が「帰属不明の建玉がある」と判定されて見送られ得る**（安全側の見送り）。
