...
- **画面と日報での表示・変更**: 手法は**リスク設定画面の専用フォーム**（発注先の変更の直後）から選ぶ（理由必須。BFF は
  ...
  判断 ID で突き合わせて数える（拒否は「発注せず（拒否）」）。2 行が食い違う承認があれば、選択 → 適用と理由（S0 以外は moomoo SIMULATE
  でしか適用しない・空売りの新規建ては S0・未知の手法の値は S0）を件数つきで書く。解決結果の記録が見つからない承認は、一致とも食い違いとも
  書かず件数を別に書く。どちらかの記録を照会できなければ「なし」と書かず「照会できませんでした」と書く。件数は承認の件数であり、
  解決の後の見送り・約定の有無は反映しない。**月報**は §6 の子節「損切りの実行機構（当月）」に、実際に適用された手法ごとの日数
  （1 日に複数の手法があれば各手法に重複して数え、その日数を併記する）と、選択と実際が食い違った日数だけを書く（個々の日は日報に任せる）。
