...
| **ソフトウェア逆指値の成行決済**（moomoo SIMULATE の S1 のみ） | 発注執行が損切りライン到達を購読（下記「口座種別の軸」） | 約定数量と建玉残の小さい方 | 通さない（機械的規則） |
...
| 決済 | **約定数量と建玉残の小さい方**を成行で決済する（手動決済・一部決済の後に全量を売って反対建玉を作らない）。建玉残が 0 なら決済せず記録を完了にする |
| 二重決済の防止 | 決済の注文識別子はエントリーと試行番号から決定的に作る。①同じ識別子の発注記録があれば再送しない ②送る前に予約表で識別子を確保し、取れなければ送らない（到達の再発火・再配送・ガードとの並行） ③受理で記録を完了にし、以後の到達では対象にしない |
| 決済が受理された | 記録を完了にし `SoftwareStopExecuted`（成行決済・Warning）。リスク管理が決済レグを取引台帳へ結線し、約定は約定追跡が届ける |
| 決済が拒否された | 試行番号を進めてガードの巡回で再試行する。**到達 1 回あたり 3 試行で打ち切り** `SoftwareStopExecuted`（決済拒否・**Critical**・手動決済を促す）。次の到達で再開する |
| 接続断・建玉を照会できない・取消待ち | **据え置く**（イベントなし）。到達の記録が残るため、次の到達を待たずに常駐ガード（既定 30 秒）が再試行する |
| 発注執行の再起動 | 記録は DB にあり、再起動後の到達・ガードの巡回がそのまま扱う。停止中に発行された到達は発注執行の購読キュー（永続）に残り、再開後に届く |
| 常駐ガード | ブローカーへ注文照会をしない。到達済みなら決済を再試行し、未到達なら「エントリーが約定 0 で終端」または「建玉残が 0 以下」で記録を完了にする |
| 同じ銘柄に S0 と S1 の建玉が併存 | 建玉残は**手法の異なる有効な記録の数量を差し引いて**判定する（S1 の建玉が S0 の逆指値を生かし続けない／S1 の決済が S0 の建玉を売らない） |
