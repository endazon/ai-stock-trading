...
| 持ち分（残保護数量） | 記録が**自分の守っている株数を状態として持つ**（建玉の純額から毎回計算し直さない）。エントリーの約定が終端で確定し、自分の決済で減り、外部要因（手動決済・強制決済・ブローカー側逆指値の約定）による減少は**観測した時点で一度だけ**古い記録から割り当てて記録する。ブローカーの建玉照会は銘柄単位の純額でしかなく、毎回引き直すと持ち分が巡回ごとに揺れて「売り過ぎ」か「損切りが黙って出ない」のどちらかへ倒れるためである |
...
| 常駐ガード | ブローカーへ注文照会をしない。**残保護数量が 0 になったときだけ**記録を完了にする（純額や他手法の主張では完了させない）。処理順は**ブローカー側の記録 → 到達済み → 未到達**である（役目を終えた逆指値の主張で持ち分を削らないため／未到達の記録に持ち分を先取りさせないため） |
| 同じ銘柄に S0 と S1 の建玉が併存 | ブローカー側の建玉残は**システム側の残保護数量を差し引いて**判定する（S1 の建玉が S0 の逆指値を生かし続けない／S1 の決済が S0 の建玉を売らない）。ブローカー側の主張は**実在する逆指値の数量**であり、帳簿だけを部分的に削っても注文は縮まないため、**全部か 0 か**でしか削らない（0 になった記録は逆指値そのものを取り消して完了する）。システム側の記録が 1 件も無い構成では差し引く量が 0 で、従来と挙動が変わらない |
