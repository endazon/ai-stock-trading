...
| 外部要因による減少の割り当て | 超過は**帳簿だけの記録（システム側）から先に**削り、**実在する逆指値を持つ記録（ブローカー側）は最後**に回す（それぞれの中では古い記録から）。ブローカー側の記録に注文照会が滞留を返すこと自体が「その建玉はまだ在る」証拠であり、順序を作成時刻だけで決めると**実在する逆指値が取り消される**（無音かつ不可逆）。**数量の減算はその場で行う**（遅らせると同じ巡回の別の記録が古い建玉を主張して売り過ぎる）が、削りは**未確定**として持ち、**2 巡回連続で観測してから確定**する。未確定のあいだは**記録を完了させず、ブローカー側の逆指値も取り消さない**。建玉が戻れば**未確定の削りを復元する**（確定済みは戻さない）。照会不能（不明）は観測とみなさない |
| 復元してよいかの判定 | **「建玉が主張の合計より多い」ことは、建玉が戻った証拠ではない。** 保護対象として**既に手当て済みの株数**——①**送信済みで建玉照会にまだ反映されていない決済**（受理済み・未約定。決済した記録はその巡回で完了するため、残っている記録からは見えない）②**確定前の新規エントリーの建玉**（まだ主張になっていない。承認数量で多めに見積もる）——を差し引き、それでもなお多いぶんだけ復元する。差し引かずに復元すると、**同じ建玉を二度売って反対建玉（空売り）を作る**。迷ったら復元しない側へ倒す（復元しなければ通知つきの縮退で済むが、復元し過ぎると不可逆な事故になる） |
...
| 決済 | **残保護数量**を成行で決済する（上の不変条件を保つ限り、累計の決済数量が保有数量を超えないため反対建玉を作らない）。残保護数量が 0 なら決済せず記録を完了にする |
...
| 常駐ガード | ブローカーへ注文照会をしない。**残保護数量が 0 になり、かつ外部要因の削りが確定したときだけ**記録を完了にする（純額や他手法の主張では完了させない）。処理順は**ブローカー側の記録 → 到達済み → 未到達**である（役目を終えた逆指値の主張で持ち分を削らないため／未到達の記録に持ち分を先取りさせないため） |
...
- **残る制約**: S2 の建玉は記録を持たないため按分できず、同じ銘柄の S1 の決済が S2 の建玉を売り得る。
