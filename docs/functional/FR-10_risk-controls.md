...
- 実装ADR: 維持率の適用閾値は口座単位（建玉ごとの閾値の最大値）とし、算式と条文選択を単一情報源へ寄せる／維持率割れの自動縮小の決定的規則（算式・発動条件・最小限・記録先）／空売り専用統制は「新規売り建て」を起点に判定し、外部照会が欠けたら通さない（拒否理由のクラス分類も持つ）／金額系の統制上限は equity 比で保持し解決点を 1 つに閉じる／日次損失上限は実現損益と含み損益の合算で判定する／エントリー・手仕舞いは建玉効果で判定し、売買方向から分離する／判定の基準通貨を USD へ反転する／統制の金額は基準通貨で判定し、換算は判断境界の 1 点で行う／SIMULATE 限定のリスク上限プロファイル／約定状態は短周期ポーリングで追跡し、台帳は `OrderId` 単位の単調 upsert で受ける／計画確定値の適合は「計画値テーブル＋既知逸脱レジストリ」で検査する
- 作業仕様書: 作業仕様書: 維持率の適用閾値を口座単位へ揃える（適用閾値の口座単位化）、仕様書: 判定の基準通貨を JPY から USD へ移行する（判定の基準通貨 USD 移行）、作業仕様書: 維持率割れによる建玉の自動縮小（自動縮小）、作業仕様書: リスク統制コアの再実装（#329・第 2 段階）（再実装・第 2 段階）、作業仕様書: リスク統制コアの再実装（#329・第 1 段階）（再実装・第 1 段階）、仕様書: リスク評価コアの是正
