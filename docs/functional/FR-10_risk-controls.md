...
adrs: [ADR-0003, ADR-0008, ADR-0009, ADR-0016, ADR-0018, ADR-0019, ADR-0021]
...
issues: [#12, #257, #270, #292, #329, #330, #331, #332, #338, #340, #342, #346, #364, #417, #419, #420]
-->
...
> 既存の節は変更していない（IADR-0133: 維持率割れの自動縮小の決定的規則（算式・発動条件・最小限・記録先）・
...
> 「規則 3 の一次ゲート — 借株可否」を追加した（IADR-0158: 空売りの一次ゲートは借株可否（`IsShortPermit`）とし、20% の借株料閾値は「発火しない既知の統制」として残置する・
...
（利用者委任に基づく決定 2026-08-02・planning#61）。資金の増減に応じて各上限が比例調整され、
...
- **推定台帳は `OrderScreeningService` の必須依存である**（#428・
  IADR-0163: allow-list の適用範囲と、必須にすべき依存の見分け方 決定2）。省略可能引数で受けていると、
...
**照会 API は実装済みである**（#463・IADR-0181: 強制買戻しの推定の照会 API と、観測の到達の記録）。
...
**［2026-08-08 改定］粒度は取引日の集合である**（計画 FR-21・裁定 [planning#292](https://github.com/endazon/project-planning/issues/292)）。
...
### 拒否理由のクラス分類（ADR-0016 決定10 / planning#58）
...
**現金口座対応（#375・ADR-0021 決定4-5・IADR-0153: 口座種別を観測として供給し、不明・食い違い・供給欠如をすべて新規建ての停止へ倒す 決定5）で 3 種を追加した**（序数は末尾 25/26/27）。

| 拒否理由 | クラス | 解除条件 | 根拠 |
| --- | --- | --- | --- |
| `CashAccountSettlementHold` | **A** | T+1 の決済 | ADR-0021 決定4-5 が明示。制度・決済由来の事象であり `BannedSymbol`（クラス C）へ混ぜない |
| `GoodFaithViolationLimitReached` | **A** | **利用者の明示的な解除操作**（Discord `/gfv clear`。ADR-0028 決定2/決定3・IADR-0182: GFV 違反による停止の解除（記録は消さず解除を追記する）） | 統制が設計どおり作動した記録。**「違反記録の失効」ではない**——ADR-0028 決定1 が「記録は失効させない」と定め、解除は記録を消さず**追記**で表す |
| `BrokerAccountTypeUnverified` | **B** | 照会の成功（と設定値の一致） | 「取引を止めている状態そのものの記録」。kill switch / pause / 段階制約と同じ区分 |

**3 種を 1 つに畳まないのは ADR-0016 決定10 の規律による**——解除条件が互いに異なり、畳むと監査ログと
日報・月報が実態と食い違う（「決済待ちで止まっている」「口座が使えなくなりかけている」「OpenD が落ちている」は
運用者が採るべき行動がまったく違う）。**計画も同じ理由で 3 種を分けている**——起案時に計画が明示したのは
1 種のみだったが、環流（feedback/20260806_adr0021-rejection-reasons-and-settled-cash.md（環流記録））を
...
## 借株料の累計（#465 / ADR-0027 / IADR-0183: 借株料は「建玉 × 取引日」で積み、未供給の日を別テーブル・別イベントで持つ）

ADR-0016 決定15 は SC-03 と報告書へ「借株料の累計」を求めていたが、**その値をどう作るかを定めていなかった**。
ADR-0027（2026-08-07 Accepted）が 5 点を確定させたため、**記録側**を実装した。

| ADR-0027 の決定 | 実装 |
| --- | --- |
| 決定1 建玉を保有している**日ごと**に積む | 計上は「**建玉 × 取引日**」で 1 件（`BorrowFeeAccrualService.Accrue`）。主キーにより**冪等**（二度計上しても金額が狂わず、2 度目はイベントも出さない） |
| 決定2 **建玉ごと**に積み、銘柄・口座へは**合算で導出**する | 一次識別子は (銘柄, 市場)。銘柄別・口座全体・月次は**すべて日次行の合算**（`SummarizeAccount` / `SummarizePosition` / `SummarizeByPosition`）。**別々に積むストア・列を持たない** |
| 決定3 計上日の属する日・月へ帰属させる（**按分しない**） | 集計は期間で行を絞り込むだけ。**生涯累計と月次合計は別の値であり同じ語で呼ばない**（口座全体の生涯累計という入口は作らない） |
| 決定4 **計上日の料率**を用いる。取得できなかった日は **0 として積まず未供給として記録する** | 料率は計上ごとに引数で受ける。未供給は**金額の欄を持たない別の型・別テーブル**（下記） |
| 決定5 自前で日次に積み上げる | 日次の計上額は `料率 × 建玉評価額 ÷ 365`（決定5 の机上確認の式）。**丸めない**（丸め規則は計画に無い） |
...
自然に書け、決定4 が禁じた「0 として積む」がスキーマ上表現可能なまま残る。
...
### 🔴 供給は開始していない（ADR-0027 決定6）

**`ShortFeeRate` の単位が確定しなければ累計の値そのものが定まらない —— 単位を取り違えると累計は 100 倍ずれる**
（ADR-0026 PoC 項目 9・期限 2026-08-31）。したがって次の 3 つを**意図的に**実装していない。

| 未実装 | 理由 | 担当 |
| --- | --- | --- |
| 日次の料率照会（`TrdGetMarginRatio`） | 実装すれば単位未確定の値が積まれる。計上サービスは料率を**引数**で受けるだけ | #331 / #342 |
| 日次で計上を回すスケジューラ | 登録＝供給の開始そのもの | #331 / #342 |
| SC-03・日報・月報への表示 | `BorrowFeeAvailability` は `NotSupplied` のまま（IADR-0154 の挙動を維持） | PoC 項目 9 の成立後 |

**これは実装漏れではなく意図した遮断である。** 遮断は構造テスト 2 件（**T-10-275** 計上の公開面に `ShortFeeRate` が
生えたら赤／**T-10-276** SC-03 の依存に累計ストアが現れたら赤）で担保する
（IADR-0158 決定3 が FR-17 の費用計算に対して行ったのと同型）。
...
- 実装ADR: IADR-0160: 維持率の適用閾値は口座単位（建玉ごとの閾値の最大値）とし、算式と条文選択を単一情報源へ寄せる（維持率の適用閾値は口座単位・算式と条文選択の単一情報源）、IADR-0133: 維持率割れの自動縮小の決定的規則（算式・発動条件・最小限・記録先）（維持率割れの自動縮小の決定的規則）、IADR-0131: 空売り専用統制は「新規売り建て」を起点に判定し、外部照会が欠けたら通さない。拒否理由はクラス分類を実装の単一情報源として持つ（空売り統制のフェイルクローズと拒否理由の分類）、IADR-0130: 金額系の統制上限は equity 比で保持し解決点を 1 つに閉じる。equity の権威値は USD で持ち、判定通貨の移行とは切り離す（equity 比の保持と解決点の集約）、IADR-0008: 日次損失上限は実現損益と含み損益の合算で判定する（日次損失の判定基準）、IADR-0004: エントリー/手仕舞いは建玉効果（PositionEffect）で判定し、売買方向から分離する（エントリー判定）、IADR-0152: 判定の基準通貨を USD へ反転し、換算の向き・equity の 1 点換算・表示単位をまとめて正す（判定の基準通貨 USD への移行）、IADR-0107: 統制の金額は基準通貨（JPY）で判定し、換算は判断境界の 1 点で行う（基準通貨換算）、IADR-0108: SIMULATE 限定のリスク上限プロファイル（SIMULATE プロファイル）、IADR-0113: 約定状態は短周期ポーリングで追跡し、台帳は OrderId 単位の単調 upsert で受ける（約定の台帳到達＝統制の前提）、IADR-0127: 計画確定値の適合は「計画値テーブル＋既知逸脱レジストリ」で検査し、逸脱の解消を機械的に強制する（計画適合の既知逸脱レジストリ）
...
- 計画への環流（いずれも起草のみ・送付は未実施）: 空売りの拒否理由コードの不足（環流記録）、
  空売り比率 50% の構造的含意（環流記録）、
  維持率の算式・複数建玉時の適用閾値・信用買いの規制維持率（環流記録）
  （**送付済み・裁定済み**。ADR-0016 決定7 の 2026-08-07 追記）、
  追記の取り込みで残った 3 点（環流記録）

## 未決事項

- ロックアウトの具体的な状態保持・解除タイミングはリスク管理ホストで確定する。
- ~~**判定通貨の USD への移行**（計画 §3 の 2026-07-31 改定への追随）~~ →
  **[#364](https://github.com/endazon/ai-stock-trading/issues/364) / IADR-0152: 判定の基準通貨を USD へ反転し、換算の向き・equity の 1 点換算・表示単位をまとめて正す で完了**
...
- SC-02 / SC-03 は「比率」と「現在 equity での実額」を併記する必要がある。SC-03 は空売り比率・
...
- ~~**維持率の算式・複数建玉時の適用閾値・信用買いの規制維持率**は計画に記述が無く、実装が安全側で仮置き
  している~~ → **計画側の裁定（ADR-0016 決定7 の 2026-08-07 追記・環流 planning#214）で確定し、
  [#420](https://github.com/endazon/ai-stock-trading/issues/420) /
  IADR-0160: 維持率の適用閾値は口座単位（建玉ごとの閾値の最大値）とし、算式と条文選択を単一情報源へ寄せる で取り込んだ**
...
- 強制買戻し（buy-in）の**検知・通知と禁止リストの永続化**は未実装（作業仕様書: リスク統制コアの再実装（#329・第 2 段階） 未決事項 §2）。
