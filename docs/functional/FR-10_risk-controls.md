...
- 🔴 **検知が走らない配置（そのままでは無音になる）**: 常駐ガードは**有効な記録が 1 件も無い巡回では建玉を照会しない**
  （無駄な往復を避けるための既存の規律）。そのため、**受理された後に 0 約定で取り消された決済の残り**は
  ——決済を送った記録はその時点で完了し、巡回の対象に 1 件も残らないため——
  **その口座に他の有効な記録が 1 件も無いあいだ、帰属不明の検知が一度も走らない**。
  **1 銘柄しか持たない口座では、この配置は無音のままである。**
  - **無保護の建玉が残っていることに気づける唯一の経路は、その銘柄へ次に新規建てを出したときの「見送り」**である
    （武装の前提条件が「帰属不明の建玉がある」と判定して発注せず、見送りの通知を出す）。
  - **建玉照会を無条件に行う常駐（既定 600 秒の建玉観測）が同じサービスに既にある**ため、
    そこへ相乗りすれば往復を増やさずに塞げる。**本書の対象外（別課題）**。
