...
- **画面と日報での表示・変更**: 手法は**リスク設定画面の専用フォーム**（発注先の変更の直後）から選ぶ（理由必須。BFF は
  ...
  （実際の発注先が SIMULATE でないための見送り・空売りの新規建ては S0・未知の手法の値は S0）を件数つきで書く。解決結果の記録が見つからない承認は、一致とも食い違いとも
