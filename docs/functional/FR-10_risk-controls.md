...
| どの記録も主張していない建玉があるとき | 🔴 **常駐ガードが毎巡回検知し、`SoftwareStopExecuted`（帰属不明の建玉・Warning）を出す**（その方向の建玉 − 有効な記録の主張の合計 − 上の「帳簿に現れない株数」> 0）。**検知であって是正ではない**——建玉を売らず・記録も作らず・主張も動かさない。武装の前提条件は**武装の時点しか見ない**ため、これが無いと ①武装より後に他の実行機構の建玉が現れる経路 ②**受理された後に 0 約定で取り消された決済の残り**（決済を送った記録はその時点で完了するため、巡回の対象に 1 件も残らない）が、どの通知も出ないまま無保護で残る。**同じ状態では毎巡回鳴らさない**（株数が変わったときか、一定間隔）。走査は**建玉の側から**行い、**システム側の記録の足跡がある銘柄・方向に限る**（有効な記録・完了した記録のいずれか）——**システム側の記録が 1 件も無い構成では何も出さない** |
