...
| 入力 | 供給 | 分からないとき |
| --- | --- | --- |
| 借株可否（一次ゲート） | 発注執行の照会（(銘柄, 市場) ごとに答えは 60 秒・失敗は 30 秒キャッシュ。照会は失敗も含めて 30 秒あたり 9 回まで。米国株以外は照会しない） | 文脈を組まない → `BorrowUnavailable` |
