...
| 超過が本当に消えたとき | **観測の連続回数を 0 に戻すだけで、何も書き戻さない。** 帳簿を書き換えていないため、戻す（復元する）操作そのものが存在しない。**一度観測した超過は、確定するか記録が完了するまで消えない**（迷ったら「その記録はしばらく動けない」側へ倒す——建玉を誤って売る側へは倒さない） |
...
- **残る制約**: S2 の建玉は記録を持たないため按分できず、同じ銘柄の S1 の決済が S2 の建玉を売り得る（ただし**建玉を外部で失ったことを一度観測した記録**は、以後その建玉を主張し直さないため、他の実行機構の建玉を売らない）。**超過が確定しないまま消え続ける配置**（ブローカー側の逆指値が約定して主張が消え、システム側の観測だけが残る等）では、その記録は有効・帳簿も無傷のまま**動かせる株数が 0** の状態が続く——建玉を誤って売ることはないが、その記録の損切りは実質出ない（到達済みなら猶予 15 分で要人手対応が 1 回出るため無音では終わらない）。
