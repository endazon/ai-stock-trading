...
| 市場監視が比べる損切りライン（同じ銘柄に記録が複数あるとき） | 取引台帳は保有中の建玉を**エントリーごと**に持ち、**残っているエントリーのうち最も保護的なライン**（買い建て: 最も高い／売り建て: 最も低い）を市場監視へ渡す。市場監視はこのラインで到達を 1 件出し、上の行のとおり**記録自身のライン**に達した記録だけが決済される——後から低いラインで建て増しても、先に建てた記録の損切りは遅れない（以前は最新エントリーのラインと比べていたため、稼働環境で 713 株の損切りが 0.79 ドル遅れる配置があった）。減少（一部決済・取り込み）は**古いエントリーから**削る（外部要因による減少の割り当てと同じ向き）。🔴 台帳はどの決済がどのエントリーを閉じたかを持たないため取り違えることがあり、そのときは「**ラインが実際より保護的**」側へ倒れる——到達は出るが決済する記録が無い（到達の通知は出る）。損切りラインの記録が無いエントリーは「ラインが無い」とは読まず、既定比率の近似で見積もって候補に入れる。日報のポジション一覧と取引判断が読む「記録上の損切りライン」も同じ値である |
...
| 決済が拒否された | 試行番号を進めてガードの巡回で再試行する。**到達 1 回あたり 3 試行で打ち切り** `SoftwareStopExecuted`（決済拒否・**Critical**・手動決済を促す）。次の到達で再開する |
