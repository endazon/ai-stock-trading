...
>
> **用語**（05_screens 共通規約）: `SIMULATE` を「ペーパー」と呼ばない。内蔵 `paper` を「SIMULATE」
...
### 2. 内蔵 `paper` 稼働中の警告表示（FR-12・05_screens 共通規約）

**SC-01 / SC-02 / SC-03 のすべてで画面上部に常時表示する。** 文言には次の 2 点を必ず含める
...
[FR-20 機能仕様書](FR-20_staged-gates.md) §4-2。
...
- [x] 内蔵 `paper` 稼働中、SC-01・SC-02・SC-03 のすべてで必須 2 文言のバナーが表示される
