...
adrs: []
iadrs: [IADR-0127, IADR-0140, IADR-0142]
...
- 実装 ADR: IADR-0127: 計画確定値の適合は「計画値テーブル＋既知逸脱レジストリ」で検査し、逸脱の解消を機械的に強制する（計画確定値の適合検査）
...
| T-02 | 発注先＝内蔵 `paper` | 逆指値付きの新規建てを発注する | 逆指値も擬似的に受理され、建玉と同時に成立する（FR-10 の「逆指値が未受理なら建玉を持たない」と整合） | —| 自動 |
