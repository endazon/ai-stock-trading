...
- **新規採番は「その時点の最大値 ＋ 1」。** 最大値は検査器が出す。
  ...
  $ node scripts/check-test-traceability.js
    テスト ID: 採番 674 件 / 参照行 3 件 / 重複 24 件（すべて baseline 記載済み）。
    採番の最大値: T-10-633 / T-12-4 / T-15-103 / T-17-4 / T-19-323 / T-20-3
  ```

  🔴 **上の数値は書いた時点（2026-09-23）のものであり、読むときには必ず自分で走らせること。**
  この転記自体が古くなった実例がある（PR #912 監査。転記の最大値に従うと、並行レーンが既に
  確保していた番号を採ってしまう）。**採る根拠は自分が走らせた出力であって、本書の転記ではない。**
