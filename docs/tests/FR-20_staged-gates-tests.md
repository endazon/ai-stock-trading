...
| ID | 前提条件 | 手順 | 期待結果 | 対応受け入れ基準 | 区分 |
| --- | --- | --- | --- | --- | --- |
| T-81 | 任意 | 最小取引件数に 0 / 1 / 1000 / 1001 を設定する | **0・1001 は 400**（設定も履歴も変わらない）。**1・1000 は受理**（境界は含む） | FR-20, FR-13 | 自動（境界値・否定形） |
| T-82 | 任意 | 件数を省略する／理由を空（空白のみ）で送る | いずれも **400**（省略は既定値 0 への暗黙束縛を防ぐ。理由は監査のため必須） | FR-20, FR-11 | 自動（否定形） |
| T-83 | 既定 100 件 | 40 件へ変更する | **受理され**、変更履歴に `Stage1MinimumTradeCountChanged`・理由・**前後値（100 → 40）**が残る（「下げた事実が記録に残る」） | FR-20, FR-11, FR-13 | 自動 |
| T-84 | 任意 | 1 / 30 / 99 件（100 件未満）を設定する | **すべて受理される**（**警告は設定を妨げない**）。ここが 400 になる実装は警告を拒否へすり替えている | FR-20 | 自動（否定形・裁定の核心） |
| T-85 | 最小取引件数 10 件を保存済み | 段階ゲートの現況を取得する | `stage1Criteria.minimumTradeCount = 10` かつ **`belowStatisticalBasis = true`**（**サーバが警告を宣言する**）。目標営業日数 60・打ち切り 120 は**変わらない** | FR-20, SC-02, SC-03 | 自動 |
| T-86 | 最小取引件数 100 / 101 / 1000 件 | 同上 | **`belowStatisticalBasis = false`**（満たしているのに警告を出すと、警告が常時出て誰も読まなくなる） | FR-20 | 自動（否定形） |
| T-87 | 最小取引件数 20 件・営業日 60・統制違反 0 件・約定 20 件 | 昇格を承認する | **受理される**（設定した件数で判定される。「保存はできたが判定には効かない」状態を作らない）。19 件なら `Stage1TradeCountInsufficient` | FR-20 | 自動（境界値） |
| T-88 | 最小取引件数 1 件（最小）・営業日 59 日／統制違反の供給なし | 昇格を承認する | **いずれも拒否される**（`Stage1TradingDaysInsufficient` / `ControlViolationCountUnavailable`）。**件数を下げても条件 1・条件 2 は緩まない**（裁定が「及ばない」と明示） | FR-20 | 自動（否定形・適用範囲） |
| T-89 | 設定ストアに本項目を持たない旧行／値域外の値（0・-1・1001・`int` 上限）がある | 設定を読み出す | **すべて既定 100 へ落ちる**（**少ない件数＝合格に近い側へ倒れてはならない**。発注先の allow-list と同型だが倒す先は「厳しい側」） | FR-20 | 自動（否定形） |
| T-90 | 最小取引件数を 1〜1000 のどこに置く | 分割約定 3 回・手仕舞い・内蔵 `paper`・実弾を混ぜて集計する | **計上単位は変わらない**（分割約定は 1 件・手仕舞いと `paper` と実弾は 0 件）。**設定化は「合格に必要な件数」だけを変え、「1 件」の定義には触れない** | FR-20, #386 | 自動（プロパティ・否定形） |
| T-91 | 34 注文がそれぞれ 3 回に分割約定（イベント 102 件） | 取引件数を集計する | **34 件**（既定 100 件を満たさない）。**約定イベントを数える実装は設定を約 34 件へ下げたのと同じ効果を持つ**——しかも画面にも履歴にも現れない | FR-20, #386 | 自動（否定形） |
| T-92 | 最小取引件数 30 件（100 件未満）を保存済み | SC-02 / SC-03 / Discord `/stage status` を見る | **3 か所すべてで警告が出る**。判定は**サーバの `belowStatisticalBasis` に従う**（画面が `< 100` を自前判定していれば、宣言を `false` にしたテストで赤くなる）。100 件以上では**出さない** | FR-20, SC-02, SC-03 | 自動（否定形・画面/E2E/Discord） |
...
| ID | 前提条件 | 手順 | 期待結果 | 対応受け入れ基準 | 区分 |
| --- | --- | --- | --- | --- | --- |
| T-93 | MarketMonitorService / InformationCollectionService の実配線 | `EndpointDataSource` のルート表を走査する | **`interval` / `poll` / `cadence` / `frequency` / `schedule` を含むルートが 1 本も無い**。対照として `monitor/settings`・`movement-threshold`・`cooldown`・`health` は実在する（検査器の空振り防止） | FR-13, SC-01 | 自動（構造・否定形） |
| T-94 | 監視設定のドメイン型 | プロパティ名を走査する | **間隔を表すプロパティが無い**（ルート名だけの検査は別の語で回避され得るため、型側からも塞ぐ） | FR-13, SC-01 | 自動（構造・否定形） |
| T-95 | 任意 | `/monitor/settings/poll-interval` 等へ PUT する／全置換 PUT に間隔を混ぜる | **404**／**保存された設定に間隔が現れない**（未知プロパティが設定に化けない） | FR-13, SC-01 | 自動（否定形） |
| T-96 | SC-01 / SC-02 を開く | 収集間隔の入力欄・保存操作を探す | **どちらの画面にも存在しない**。SC-01 は `/monitor/*` を**1 度も呼ばない**（節を隠しただけで実体が残っていないこと） | FR-13, SC-01, SC-02 | 自動（否定形・画面/E2E） |

### 市場監視パラメータの値域と監査（SC-01 §2 → SC-02 の移管・#423・[IADR-0165] 決定2/決定3）

| ID | 前提条件 | 手順 | 期待結果 | 対応受け入れ基準 | 区分 |
| --- | --- | --- | --- | --- | --- |
| T-97 | 任意 | 変動閾値に 0 / 0 超 / 0.50 / 0.50 超を設定する | **0 以下は不可・0 超は可・0.50 は可・0.50 超は不可**（0 では**あらゆる変動が閾値超過**となり LLM 費用が暴走する） | FR-03, FR-13 | 自動（境界値・否定形） |
| T-98 | 任意 | クールダウンに 0 / 24h / 24h 超 / 負値を設定する | **0 は可・24h は可・24h 超と負値は不可**（0 は「抑制なし」という正当な設定であり、変動閾値の 0 とは非対称） | FR-03, FR-13 | 自動（境界値・否定形） |
| T-99 | 任意 | **全置換 `PUT /monitor/settings`** で値域外を送る／理由なしで送る | **いずれも 400**。受理された変更は**履歴に残る**。**画面から使わない経路だからこそ塞ぐ**（画面だけの統制は API 直叩きで消える） | FR-03, FR-11, FR-13 | 自動（否定形） |
| T-100 | SC-02 | 変動閾値を 5% にして保存する／クールダウンを 2 時間にして保存する | ワイヤはそれぞれ **比率 `0.05`** と **`TimeSpan` 文字列 `"02:00:00"`**（単位の取り違えは統制設定で最も危険な誤り） | FR-03, SC-02 | 自動（画面/E2E） |
