...
| T-10-12 | 金額上限は基準通貨（円）換算後の金額で判定する（#257 / IADR-0107） | `外貨建て注文の金額上限は換算後の金額で判定する` / `外貨建て注文の日次発注累計と段階資金上限も換算後の金額で判定する` / `換算レート既定1の注文は従来どおり判定される` | 自動 |
...
| T-10-14 | 非基準通貨で換算レートが解決できなければ新規建てを見送る（fail-safe）。基準通貨は実 FX 源が無くても従来どおり判断する | `TradeDecisionServiceTests.外貨建て銘柄で換算レートが無ければ見送る` / `FX未結線の既定では外貨建て銘柄を見送る` / `換算レート取得が例外なら見送る` / `基準通貨の市場は実FX源が無くても従来どおり判断する` / `FredFxRateSourceTests.基準通貨は外部へ問い合わせずレート1を返す` | 自動 |
