...
| `ReportKnowledgeReingested` | 報告書 | RunId, Actor, Scope, RefreshExisting, Status（Completed/Aborted/Cancelled）, AbortReason, Targeted, Created, BodyAttached, BodyRefreshed, AlreadyPresent, SkippedEmptyBody, SkippedBodyTooLarge, Failed, Unknown, DuplicatesInKb, Breakdown[{PeriodKey, Outcome, Reason, DocumentId}], BreakdownOmitted, OccurredAt | 所有者が確定済みの報告書を KB へ入れ直した実行（1 回につき 1 件・中止も含む）。監査が購読する。`Actor` はトークンの主体。不明（結果が分からない書き込み）は送ったにも失敗にも数えない。内訳は送らなかった／失敗／不明の報告書だけで 200 件まで |
