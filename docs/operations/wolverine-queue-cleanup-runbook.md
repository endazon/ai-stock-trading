...
> 起点: [ADR-0013](../../planning/projects/ai-stock-trading/07_adr/ADR-0013_messaging-follow-wolverine-kafka.md)（Wolverine 移行）/
> [IADR-0129](../adr/IADR-0129_wolverine-messaging-topology.md)（トポロジ設計・「結果」の悪い影響に本手順の必要性を記載）/
> [IADR-0106](../adr/IADR-0106_consumer-endpoint-name-uniqueness.md)（旧キュー名の導出規則。Superseded）/
> 作業仕様書 [20260803_354_wolverine-migration.md](../specs/20260803_354_wolverine-migration.md)。
>
...
Wolverine 移行でキュー名の導出規則が変わった（[IADR-0129](../adr/IADR-0129_wolverine-messaging-topology.md) 決定 1）。
...
- `*_error`（デッドレター）に残っているもの: 処理に失敗した業務イベントである。**捨てる前に監査台帳
  ...
  **Wolverine 版へそのまま再投入しても消費できない**（[IADR-0129](../adr/IADR-0129_wolverine-messaging-topology.md) 決定 7）。
...
| 18 | `OrderApprovedLedger` | RiskManagement | 同上（1 キューを 2 ハンドラが共有・[IADR-0129](../adr/IADR-0129_wolverine-messaging-topology.md) 決定 10） |
...
> **`TradeDecisionMadeBaseline`（#45）の由来**: `MarketMonitorService` の consumer は
> [IADR-0106](../adr/IADR-0106_consumer-endpoint-name-uniqueness.md)（#258）で `TradeDecisionMade` と
...
- 実装ADR: [IADR-0129](../adr/IADR-0129_wolverine-messaging-topology.md)（新トポロジ）/
  [IADR-0106](../adr/IADR-0106_consumer-endpoint-name-uniqueness.md)（旧キュー名の規則・Superseded）
