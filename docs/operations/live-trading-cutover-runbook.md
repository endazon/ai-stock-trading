...
| 3 | **発注予約 `Reserved` 滞留の監視＋自動リコンサイル** | [#141](https://github.com/endazon/ai-stock-trading/issues/141) / 自動リコンサイルの実装 ADR | `Reconciliation:Enabled=true` かつ**実照会プローブが配線済み**であること（既定 no-op では自動解消しない）。滞留＝「発注済みか不明な建玉」で実弾では実損リスク |
