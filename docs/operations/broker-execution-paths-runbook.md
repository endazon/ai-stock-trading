...
| 訂正・取消 | 訂正・取消の口（`IOrderAmendmentBroker`）を**paper だけが実装**する（ただし既定の即時約定では常に終端のため成立しない） | **その口を実装しない**＝訂正・取消の配管（`OrderAmendmentService`）を構成上そもそも登録しない（fail-safe・IADR-0067: 注文履歴テレメトリは「イベント追加＋Risk 専有 DB への射影」で供給し、訂正・取消の口はペーパー専用ポートに閉じる） |
...
Helm では**単一スイッチ `broker.tier`**（IADR-0111: ブローカー選択は provider × environment の直交 2 軸で表現する）で選ぶ。
...
- **注文の備考（remark）に `DecisionId` が入る**。ハイフン無しの 32 桁 hex（`Guid` の `"N"` 書式）で、
  AST 側の `executed_orders."DecisionId"` と 1 対 1 に対応する（IADR-0092: Reserved 滞留の実照会プローブ（moomoo remark 突合・SIMULATE））。
...
> 解禁には別の実装 ADR と IADR-0056: moomoo SIMULATE PoC 完了に基づき実アダプタを実装（実弾は引き続きゲート） §3 の前提充足が要る。
...
- 約定状態を後から取りに行く定期経路が無い（`GetOrderAsync` の呼び出し元は訂正・取消と、既定無効の
  リコンサイル IADR-0074: Reserved 滞留の自動リコンサイルはプローブ・ポート＋fail-safe 既定 no-op で行い、実照会は後続へ分離する / IADR-0092: Reserved 滞留の実照会プローブ（moomoo remark 突合・SIMULATE） のみ）。
...
- IADR-0016: 発注執行は安全既定（ペーパー）とし、moomoo 実発注は PoC までゲートする（安全既定のブローカ執行）/
  IADR-0056: moomoo SIMULATE PoC 完了に基づき実アダプタを実装（実弾は引き続きゲート）（SIMULATE PoC 完了・実弾はゲート）/
  IADR-0111: ブローカー選択は provider × environment の直交 2 軸で表現する（ブローカ階層）
