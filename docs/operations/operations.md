...
issues: [#13, #24, #121, #131, #132, #137, #141, #243, #262, #263, #267, #268, #303, #364, #380, #407, #627, #686, #705, #817, #891, #858, #942, #937, #853, #346, #1028, #1022, #856, #1051, MSP#266, MSP#635, planning#54, planning#676]
-->
...
- **判定**: 滞留閾値より古い `Reserved` を走査し、次の 6 分岐へ振り分ける。右列は巡回サマリの内訳名である。
  ...
  | ③ | 照会が未発注（`NotPlaced`）**かつその予約の取引環境の解放の門が開いている** | 予約を解放（＝再発注を許可） | `解放` |
  | ④ | 照会が未発注（`NotPlaced`）**だがその予約の取引環境の門が閉じている**（取引環境が不明・照会先と食い違うときも） | **据え置き**（解放しない） | `据え置き〔未発注だが門が閉〕` |
  | ⑤ | 照会不達・判定不能（`Indeterminate`） | **据え置き**（人手/`_error`・解放しない） | `不確定` |
  | ⑥ | 当該 1 件の処理で例外 | **据え置き**（次回巡回で再試行。他の件は巻き添えにしない） | `失敗` |

  🔴 **配備では解放の門がどちらも閉じている**（`Reconciliation__ReleaseOnNotPlaced__Simulate` / `__Real` とも `false`）ため、
  **③は起こらない**——未発注と出ても④になる。したがって `解放` は**常に 0** であり、0 は故障ではない。
- 🔴 **門は取引環境ごとに分かれ、予約ごとに選ばれる。** 予約は、取った時点で送る先だった発注先（moomoo `SIMULATE` / `REAL` /
  内蔵 `paper`）を覚えている。解放してよいのは「予約が `SIMULATE` で照会先も `SIMULATE`、`SIMULATE` の門が開」か
  「予約が `REAL` で照会先も `REAL`、実弾の門が開」のどちらかだけである。**`SIMULATE` の門を開けても実弾の予約は解放されない。**
  取引環境を覚える前に取られた予約（取引環境が不明）・内蔵 `paper` の予約・照会先と取引環境が食い違う予約は、
  **どちらの門を開けても据え置く**（不明を `SIMULATE` と読まない）。発注先を切り替えても、切り替える前の予約は元の取引環境の門で判定される。
...
Reconciliation:
  ...
  ReleaseOnNotPlaced: # 🔴 解放の門（取引環境ごと）。アプリ既定・配備ともにどちらも false
    Simulate: false # moomoo SIMULATE へ送った予約の門（env: Reconciliation__ReleaseOnNotPlaced__Simulate）
    Real: false # moomoo REAL（実弾）へ送った予約の門（env: Reconciliation__ReleaseOnNotPlaced__Real）
...
- 🔴 **`ReleaseOnNotPlaced`（解放の門）は、どちらも閉じたままにする。** 解放は予約行を消して**再発注を許可する**操作であり、
  誤判定は二重発注に直結する。「未発注」の根拠は備考の突合であって証券会社が「無い」と答えた事実ではなく、
  備考が往復するかは**実機未検証**である（往復していなければ発注済みの注文も「一致ゼロ」に見える）。
  **`true` にしてよいのは、その取引環境の実機の記録で下記「解放の門を開けるときの記録」の (a)(b) を示した後だけである。**
  **`SIMULATE` の記録で `Real` を開けない**（`SIMULATE` で備考が往復しても、実弾で往復する保証は無い）。
- **旧キー `Reconciliation__ReleaseOnNotPlaced`（取引環境を区別しない真偽値）は廃止した。** 構成に残っていれば
  **`SIMULATE` の門にだけ写し、実弾の門には写さない**（新キー `__Simulate` が在れば新キーが勝つ）。起動時に
  `旧キー Reconciliation:ReleaseOnNotPlaced=… を SIMULATE の門にだけ写しました…` の Warning が出る。`true` / `false` 以外の値は起動時に止まる。
  起動時停止にしていないのは、発注執行が再起動を繰り返すと発注と保護逆指値ガードがまとめて止まるためである。
...
- **監視すべきログ**:
  ...
  - **Warning** `…照会は「未発注」と答えましたが、この予約の取引環境の解放の門が閉じているため据え置きます（DecisionId=… 取引環境=…）…`。
    滞留は解消していない。Runbook の人手手順で解決する。`取引環境=Unknown` は取引環境を覚える前に取られた予約であり、どちらの門でも解放されない。
  - **業務メトリクス** `ast_order_reservation_reconciliations_total{outcome,provider}`（#856 / #1051）。上の 6 分岐を件数で数える
    （①＝`self-healed`・②＝`probe-placed`・③＝`released`・④＝`held-not-placed`・⑤＝`indeterminate`・⑥＝`failed`）。
    `provider` は**予約の取引環境**（`MoomooSimulate` / `MoomooReal` / `InternalPaper`、不明は `Unknown`）であり、照会先ではない。
    ログの grep と違い保持期間に縛られないので、**解放の門を開けてよいかの観測**（#856）に使う——証券会社の画面で存在を確かめた注文の
    `DecisionId` について `held-not-placed` が出ていたら、その取引環境の門を開けてはならない（備考が往復していない）。
    **門の判断は同じ `provider` の系列だけで行う**（`MoomooSimulate` の系列で実弾の門を開けない）。
    ④⑤⑥の予約は Reserved のまま次の巡回にも載るため、**同じ予約が巡回ごとに数え直される**（件数は判定の回数であり、予約の数ではない）。
    リコンサイルが有効な構成でだけ、起動完了時に 24 系列（判定 6 × 取引環境 4）が 0 で現れる。アラートは置いていない。
- **停止**: `Reconciliation__Enabled=false` に戻して再デプロイすれば次回巡回から走査しない。

### 解放の門を開けるときの記録

門を開ける（`__Simulate` または `__Real` を `true` にする）のは**利用者の判断**であり、次の記録を添えて判断する。
記録は**取引環境ごとに**集める（`SIMULATE` の記録は `SIMULATE` の門の判断にだけ使う）。

- **(a) 発注済みと分かっている予約について、照会が発注済みと判定した記録が 1 件以上あること。** 備考が往復することの直接の証拠である。
  計器では、その取引環境の `ast_order_reservation_reconciliations_total{outcome="probe-placed",provider="MoomooSimulate"}`
  （実弾なら `provider="MoomooReal"`）が増えたこと、ログでは Critical `…突合で「発注済み」と確定しました…` の行で分かる。
- **(b) 観測期間中の `held-not-placed`（未発注だが門が閉のため据え置き）の判定について、1 件ずつ `DecisionId` を証券会社の画面で確かめ、
  発注済みだったものが 0 件であること。** 計器は判定の回数しか数えないため、1 件ずつ画面で確かめる。
  (a) で発注済みと分かっている予約が `held-not-placed` と判定されたら、それは誤判定であり (b) を満たさない。
- **1 件ごとに残す項目**:

  | 項目 | 内容 | どこから取るか |
  | --- | --- | --- |
  | `DecisionId` | 予約の DecisionId | ログの `DecisionId=`（証券会社の画面では備考にハイフン無しの 32 桁で入っている） |
  | 判定 | `probe-placed` / `held-not-placed` | ログ・計器の `outcome` |
  | 画面での確認結果 | 注文が有った（発注済み）／無かった（未発注） | 証券会社の画面 |
  | 日時 | 判定の日時と画面で確かめた日時 | ログの時刻・確認した時刻 |
  | 取引環境 | `SIMULATE` / `REAL` | 据え置きの Warning の `取引環境=`・計器の `provider` |
  | 一致を得た市場 | 米国 / 日本（`probe-placed` のとき） | 確定の Critical の銘柄と、証券会社の画面 |
  | 列挙の経路 | 当日の注文（現在）／注文履歴（履歴） | 証券会社の画面（どちらの一覧に出たか） |

  ⚠️ **一致を得た市場と列挙の経路は、ログに出ていない**（照会は全市場の現在と履歴を列挙するが、どこで一致したかを記録しない）。
  画面で確かめて書き留める。(a) の 1 件が示すのは、その市場・経路で備考が往復することだけであり、ほかの市場・経路は (b) の個別の確認で押さえる。
- **置き場所**: 記録は #856（実機の記録の収集）へ表で残し、門を開ける変更（`deploy/helm/ai-stock-trading/values.yaml` の
  `__Simulate` / `__Real` を `true` にする PR）の本文から、その記録への参照を張る。`.github/workflows/helm.yml` の描画アサーションも同じ PR で改める。
- 🔴 **基準を満たすまで、未発注・判定不能の予約を解放する手段は利用者の判断だけである**（発注経路の Runbook の DB 直接操作）。
  判定不能（`Indeterminate`）は、門を開けても解放しない。
- `SIMULATE` に限り、記録を集めるために送信後に結果を確認できない発注を意図的に作ってよい（模擬口座なので実損が出ない）。
  **実弾では作らない**——実弾の記録は自然に起きた分だけで集め、集まらないあいだ実弾の門は閉じたままである。
