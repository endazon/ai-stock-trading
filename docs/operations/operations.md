...
| 11 | **発注予約 `Reserved` 滞留の監視・自動リコンサイル** | 🔴 **未充足** | 現状は人手（下記 Runbook）。自動化は **#141**。実弾では「発注済みか不明な注文」＝未確定の建玉を意味する（実アダプタ実装の実装 ADR §3） |
...
3. 応答を読む。**200 以外（503＝KB 未構成・502＝KB の文書一覧を引けなかった）は 1 件も書いていない**ので、原因を直してそのまま再実行する。
   ...
   | `Failed` | 拒否された・届かなかった（理由つき） | 理由を直して再実行する。所有者でない本文なしの写しは基盤の管理者が削除してから再実行する |
   | `Unknown` | タイムアウト等で結果が分からない | **そのまま再実行する**（入っていれば一覧で見つかるので作り直さない） |

4. **何度実行しても KB の件数は増えない**（既存の写しを一覧から属性で探してから書く）。索引だけが消えて文書が残った場合は
   `refreshExisting: true` で本文を入れ直す（文書は増えない）。同じ報告書の写しが複数あると `duplicatesInKb` に出る（本システムの資格では消せない）。
5. 実行は監査台帳に `ReportKnowledgeReingested`（操作者・範囲・件数・送らなかった／失敗／不明の内訳）として残る。同時に 2 本は走らない（409）。
...
- **判定**: 滞留閾値より古い `Reserved` を走査し、次の 6 分岐へ振り分ける。右列は巡回サマリの内訳名である。
  ...
  | ② | 照会が発注済み（`Placed`） | 記録して確定＋`OrderExecuted` 発行 | `終端化` |
...
| **発注予約が `Reserved` のまま滞留**（#131。発注の冪等化は 3 相で行い、不明な窓は再発注せず拒否する。自動化は #141） | `order-approved_error` キューの滞留。および `order_dispatch_reservations` に `State=Reserved`（＝0）の行が残る（`SELECT * FROM order_dispatch_reservations WHERE "State" = 0 ORDER BY "ReservedAt";`） | **自動再開はしない**（意図的な at-most-once）。配備では自動リコンサイルが有効で実照会プローブも配線済みなので、**「発注済み」と確定できたものは自動で解消される**（🔴 ただしその注文に保護逆指値は張られない。Critical ログを見て確認する）。**「未発注」「判定不能」は解放の門が閉じているため据え置かれる**ので手動で: ブローカ側の注文状態を確認し、①発注済み→当該注文を台帳へ手動計上して予約を確定／②未発注→予約行を削除して再配送を許可 | **不明なら「発注済み」として扱う**（二重発注を避ける側に倒す）。実弾運用中は建玉と突き合わせ、判断が付かなければ取引を停止して人間が判断する |
