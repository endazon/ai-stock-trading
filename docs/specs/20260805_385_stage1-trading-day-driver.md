...
- [ ] 日次の稼働分数が記録され、`Stage1QualifiedTradingDays` が**実測から**更新される
- [ ] ET 基準で判定される（**DST 切替をまたいでも同じ現地時刻が同じ分に写る**）
- [ ] 半日取引日で誤って算入されない（**両仮説**方式。朝方停止 + 午後稼働で算入されないこと）
- [ ] OpenD 停止・ブローカー障害が除外される（probe が失敗した区間は credit されない）
- [ ] 市場休場のうち**週末**が除外される（祝日は塞げない。未決事項へ明記）
- [ ] **否定形**: 供給が途絶えたときに日数が水増しされないこと（観測が無い＝0 分＝算入されない）
- [ ] **否定形**: probe を 1 回落とした区間が稼働として計上されないこと
- [ ] **否定形**: 同じ観測を二重に credit しないこと（再送・逆行）
- [ ] **否定形**: 内蔵 `paper` で稼働した日は算入されないこと（**除外日として別掲**されること）
- [ ] **否定形**: 1 件の観測が上限（30 分）を超えて credit されないこと
- [ ] **カレンダーを内蔵していないことを構造で確認できる**（下記テスト方針の「構造テスト」）
