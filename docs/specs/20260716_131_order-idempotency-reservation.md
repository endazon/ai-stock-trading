...
- [ ] 同一 `OrderApproved` の再処理で、ブローカへの発注が高々1回に限定される（**発注成功後の永続化失敗を跨いでも**）
- [ ] 予約は必ずブローカ発注の**前**にコミットされる
- [ ] Save 成功・Complete 失敗を跨いだ再処理は、既存結果を再発行する（拒否しない）
- [ ] 予約済み未確定の再処理は例外で拒否し、ブローカへ発注しない
- [ ] 既存挙動（ペーパー約定・Rejected・Close・スリッページ・完了済み再処理の再発行）が回帰しない
- [ ] EF 実装で `DecisionId` の一意制約により並行予約が高々1つに限定される
- [ ] `TrdEnv_Real` 解禁は本件＋別 IADR＋明示 config を前提とする旨が IADR に記載される
- [ ] 既定は安全側（本 PR はブローカ既定 paper・moomoo ゲート・SIMULATE 固定を一切変更しない）
