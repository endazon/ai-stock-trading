...
- [ ] 用途／モデルが混在しても、計上額が**実際に使われたモデルの単価**で算出される
- [ ] `trade-decision` が `claude-sonnet-5` である現状で、計上額が実勢と一致する（現行の 2.5 倍過大が解消される）
- [ ] `report-*` 3 種別が別モデルへ解決されても、それぞれ正しい単価が引ける（#282 の対応と揃う配置になっている）
- [ ] 未知モデル・単価未設定は安全側へ倒れ、例外にしない（未知＝最大単価／表が空＝既定ペア／全部未設定＝0）
- [ ] `claude-sonnet-5` の 2026-08-31 の導入価格終了に対する更新運用が文書に明記されている
- [ ] 本番（`values.yaml`）の描画・挙動は不変。SIMULATE / 実弾の閂に変更がない
