...
- 計画書: [05_trading-assumptions](../../planning/projects/ai-stock-trading/06_technical/05_trading-assumptions.md) §3
...
### 3. 取得窓 ≥ 受容窓（`ObservationLimit` 10 → **20**）

受容窓 14 日 ≒ 営業日 10 件。欠測が `"."` レコードとして返る場合、10 件では受容窓の端まで届かず、
**広げた窓が部分的に無効**になる（実測時のように欠測日のレコード自体が存在しない場合は届くが、
どちらの返り方でも成立させる必要がある）。20 件（約 4 週ぶんの営業日）にして関係を構造で保つ。
リクエスト数は変わらない（1 回）ため費用増は無い。
...
| 10 | 取得窓 ≥ 受容窓 | `FredFxRateSourceTests`: 要求 URL の `limit` が既定受容窓の営業日数以上／`"."` が 10 件続いても直近の数値観測を採る |
