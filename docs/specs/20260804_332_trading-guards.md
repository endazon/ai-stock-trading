...
いずれも**計画の値・規則を変更するものではない**ため計画への環流（`/plan-feedback`）は要さないと判断した。
差異 1 は FR-19 の適用範囲を明示化する余地があるため、監査で環流の要否を判断されたい。

## 未決事項

1. **市場ガード・禁止銘柄ガードの手仕舞い適用**: `MarketDisabled` / `BannedSymbol` は現在も**全注文**へ
   適用する（既存挙動）。禁止銘柄へ登録した瞬間に既存建玉を手仕舞えなくなる（利用者が禁止登録した銘柄の
   建玉が閉じられない）という同種の懸念があるが、**商品種別と違って「登録は利用者の意思」であり
   計画（ADR-0007）が「登録されたものを確実に強制する」と明記している**ため、本 issue では変更しない。
   計画側の裁定が要る論点として記録する。
...
4. **東芝（旧 6502）の再上場時の銘柄コード**: 計画は「旧 6502。再上場時に適用」と記す。再上場時の
