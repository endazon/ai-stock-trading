---
title: moomoo OpenAPI PoC（ADR-0019 決定 1 の 7 項目）の実施計画と、SDK 契約から静的に確定した到達可能性
type: spec
status: draft
...
- 関連 ADR: **ADR-0019**（PoC 7 項目・本仕様書の直接の起点）・ADR-0002（moomoo 採用）・ADR-0016（空売り段階解禁。決定 3・4・7・8 が PoC 結果に条件付き）
...
ADR-0019 は moomoo 連携に 7 項目の PoC を課し、**各項目が不成立だった場合の帰結**を併記した。結果次第で ADR-0016 の決定 3・4・7・8（空売り統制）の見直しが要るため、空売りの再実装（#329・#331）に先行して着手する必要がある。期限は **2026-08-31**。
...
- **対象**: ADR-0019 決定 1 の 7 項目の確認、および結果の記録（本仕様書・IADR・#342 への報告・`/plan-feedback`）。退行防止のための結合テストの追加。
- **対象外**:
...
実機確認が要る作業を洗い出した `blockedtasks.md` は A-2 を **7 項目**として記載しているが、**ADR-0019 決定 1 の 7 項目と内容が一致しない**。計画書が正であるため（planning `.claude/rules/adr.md`「計画書は絶対的な正である」）、**ADR-0019 を権威とする**。

> ### ⚠️ 訂正 0（2026-08-05・AI レビュー指摘による）
>
> **本仕様書は当初「ADR-0019 決定 1 は 6 項目であり、米国株日足 OHLC は ADR-0019 に無い補助項目」と記録したが、これは誤りである。**
>
> **ADR-0019 決定 1 は 7 項目**である。項目 7（米国株の日足 OHLC 履歴を API から取得できるか、およびその品質）は **2026-08-04 の追補で正式に追加**されており（[ADR-0023](../../planning/projects/ai-stock-trading/07_adr/ADR-0023_us-daily-ohlc-history-source.md) 決定 3 が「ADR-0019 決定 1 に項目 7 を追加した」と明記している）、補助項目ではない。
>
> **原因**: 起案時、submodule が**古い pin（`df8bce5`）**で checkout されており、その版の ADR-0019 は 6 項目だった。作業中に develop の pin が `d980a01` へ上がって項目 7 が入ったが、本仕様書は起案時の読みのまま据え置かれた。
>
> **教訓**: submodule の pin が作業中に動き得る構成では、**計画書の引用は「いつの pin で読んだか」とともに記録しないと陳腐化する**。本仕様書の他の計画書引用も同じリスクを持つ（`d980a01` 時点で読み直し済み）。
>
> **項目 7 の不成立時の帰結も、当初の記録より重い**。ADR-0019 は「**Stage 0 の合格判定を実施できない。ADR-0005 の有料枠の検討へ移る**（月次データ費用上限〔現在 0 円配分〕の見直しを伴う）」と定めている。本 PoC で moomoo から取得できたことは、**有料枠の検討そのものを回避する**という意味を持つ。
...
| 7 | **米国株の日足 OHLC 履歴を API から取得できるか、およびその品質**（2026-08-04 追補） | 7. 米国株の日足 OHLC 履歴の取得可否と品質 | 一致（当初は「ADR-0019 に無い補助項目」と誤記録していた。訂正 0 を参照） |
...
| 7 日足 OHLC 履歴 | 履歴 K 線 | `QotRequestHistoryKL` / **`QotRequestHistoryKLQuota`**（取得枠の照会） | ✅ **API は存在する。ただし取得枠（quota）の制約がある** |
...
8. `QotRequestHistoryKL` で米国株日足がどこまで遡れるか、`QotRequestHistoryKLQuota` の枠はいくつか（項目 7）
...
| 7 | 米国株日足 OHLC が実用的な期間・品質で取得できる | **FR-15 のバックテストに流せるデータが無い状態が続き、Stage 0 の合格判定が成立しない**（#382） |

## 受け入れ基準

- [ ] ADR-0019 決定 1 の 7 項目それぞれについて、成否と**不成立時の帰結の適用有無**を記録した
- [ ] 項目 7（米国株日足 OHLC）の可否と品質（遡れる期間・quota）を記録した
...
| **借株可否** | （該当する API 無し） | ❌ **取得できない**。`TrdGetMaxTrdQtys.MaxSellShort` は借株情報を含まない（下記の訂正を参照） |
...
> ### 訂正（2026-08-05・第 3 回の実測による）
>
> **本仕様書は当初「借株可否は `TrdGetMaxTrdQtys.MaxSellShort` で取得できる」と記録したが、これは誤りである。`MaxSellShort` は借株の在庫情報を一切含まない。**
>
> 2 回の実行で得た値を突き合わせたところ、**純粋な証拠金計算**であることが判明した。
>
> | 銘柄 | `MaxSellShort` | `equity ÷ (数量 × 0.6)` が含意する株価 |
> | --- | --- | --- |
> | AAPL（1 回目） | 5,228 | **308.85** |
> | AAPL（2 回目） | 5,203 | **310.33** |
> | TSLA | 4,999 | 322.99 |
> | GME | 83,789 | 19.27 |
>
> AAPL の実測現在値（`QotGetSecuritySnapshot`）は **309.38** であり、2 回の実行値の間に挟まる。つまり
> **`MaxSellShort` = 自己資金 ÷ (株価 × 0.6)** という買付余力の割り算にすぎず、銘柄ごとに値が違うのは
> **株価が違うから**であって借株の需給を反映したものではない。
>
> **副産物**: この関係式が 3 銘柄すべてで成立することから、**SIMULATE の空売り初期証拠金が 60% である**ことが
> 実測で裏付けられた。[ADR-0016](../../planning/projects/ai-stock-trading/07_adr/ADR-0016_short-selling-staged-release.md) 決定 14 の
> 「SIMULATE はショート 初期 60% / 維持 50%」という記述と一致する（出所が計画書に無かった値の裏取りになる）。
>
> **したがって項目 3 は借株料・借株可否の両方が取得できない。** 決定 3 が課す「年率 20% を超える銘柄を弾く」
> コスト判定はもちろん、「そもそも借りられるか」すら API から分からない。**残る確認手段は、実際に
> `TrdSide_SellShort` の注文を出して受理されるかを見ること（項目 2）だけである。**

### 項目 2・5・7 — 未実施

- **項目 2（空売り発注）**: 第 5 回で実施し、**`TrdSide_SellShort` は拒否された**（下記）。
- **項目 5（buy-in 検知）**: 現在の口座に約定履歴が無く、判定材料が無い。ADR-0016 決定 14 が「SIMULATE では発生しない」と明記しているため、確認できるのは受信経路の疎通までである。
- **項目 7（米国株日足 OHLC）**: ✅ **成立**（第 2 回で実測。下記）。

### 項目 7: 米国株日足 OHLC 履歴 — ✅ **成立。#382 を解く**
...
| 2 空売り発注 | ✅ **成立**。`Sell`（建玉なし）で受理・取消まで一巡 | 空売りと通常売却が API 上で区別されない（IADR-0118 / 0119 の重要度が上がる） |
| 3 借株料 | 🟡 **照会は成立（実弾口座のみ）だが一律 1.5 で銘柄別でない** | **決定 3 の 20% 閾値は恒久的に発火しない。要環流** |
| 3 借株可否 | ✅ **成立（実弾口座のみ）**`IsShortPermit` | 同上 |
| 3 維持率 | ✅ **照会は成立（実弾口座のみ）**`ImShortRatio=50` / `MmShortRatio=30`。SIMULATE では `Funds` の維持率が埋まらない | 決定 7 は実装可能。**Stage 1 では検証できない**（決定 14 の想定どおり） |
| 4 実弾/ペーパー切替 | ✅ 成立 | 安全要件を満たす |
| 5 buy-in 検知 | ⏸ 未実施 | — |
| 6 常駐安定性 | 🟡 7 日 20 時間・5 再起動で無人継続を確認 | 単一ノードでの観測に限る |
| 7 日足 OHLC | ✅ **成立**（AAPL で 2006-07 まで・1,000 件/req・ページング可） | **#382 を解く。ADR-0023 へ環流する** |

### 訂正 2（2026-08-05・第 4 回）— 項目 3 は**成立する**。ただし実弾口座でしか取れない

**利用者が実弾の信用口座（accId=284852705357372276）に対して読み取り照会を実行した結果、`TrdGetMarginRatio` は成功した。** 本仕様書がここまで記録していた「項目 3 は不成立の公算」は**誤りである**。

```
REAL-MARGIN accId=284852705357372276 AAPL retType=0 件数=1
REAL-MARGIN AAPL isShortPermit=True shortFeeRate=1.5 shortPoolRemain=26452338 imShort=50 mmShort=30
REAL-MARGIN GME  isShortPermit=True shortFeeRate=1.5 shortPoolRemain=1898200  imShort=50 mmShort=30
```

| ADR-0016 決定 3・7 が要求する値 | フィールド | 実弾口座 | SIMULATE 口座 |
| --- | --- | --- | --- |
| 借株可否 | `IsShortPermit` | ✅ `True` | ❌ 照会自体が失敗 |
| **借株料** | `ShortFeeRate` | ✅ **1.5** | ❌ 同上 |
| 借株在庫 | `ShortPoolRemain` | ✅ AAPL 26,452,338 / GME 1,898,200 | ❌ 同上 |
| 空売り初期証拠金率 | `ImShortRatio` | ✅ **50** | ❌ 同上 |
| 空売り維持証拠金率 | `MmShortRatio` | ✅ **30** | ❌ 同上 |

**エラー文言が誤解を招く。** SIMULATE 口座が返した `Get Margin Trading Data does not support Stocks in US Market` は市場を理由にしているが、**実際の制約は口座環境（SIMULATE）にある**。同じ市場・同じ銘柄が実弾口座では通る。**この文言を額面どおり読むと誤った恒久的結論に至る**ため、切り分けは必須だった。

**`ImShortRatio=50` / `MmShortRatio=30` は [ADR-0016](../../planning/projects/ai-stock-trading/07_adr/ADR-0016_short-selling-staged-release.md) 決定 14 が記載する実弾の規制値（ショート 初期 50% / 維持 30%）と完全に一致する。** 出所が計画書に無かった値の裏取りになる。あわせて、SIMULATE 側で実測した初期証拠金 60%（訂正 1）も決定 14 の記述と一致しており、**決定 14 の「SIMULATE だけが保守側の外れ値になる」という指摘が実測で裏付けられた**。

#### 銘柄を 8 つに増やした結果（第 5 回・実弾口座・読み取りのみ）

| 銘柄 | `IsShortPermit` | `ShortFeeRate` | `ShortPoolRemain` | `ImShortRatio` | `MmShortRatio` |
| --- | --- | --- | --- | --- | --- |
| AAPL | True | **1.5** | 26,452,338 | 50 | 30 |
| MSFT | True | **1.5** | 16,655,203 | 50 | 30 |
| GME | True | **1.5** | 1,898,200 | 50 | 30 |
| MSTR | True | **1.5** | 3,443,217 | 50 | 30 |
| RIOT | True | **1.5** | 1,201,180 | 50 | 30 |
| **AMC** | **False** | **0** | **0** | **100** | 30 |
| **SPCE** | **False** | **0** | **0** | **100** | 30 |
| TSLA | — | — | — | — | — （レート制限で取得できず） |

**API は銘柄を明確に区別する。** 空売り不可の銘柄は `IsShortPermit=False` ＋ `ShortPoolRemain=0` ＋ `ImShortRatio=100` を返し、**可否のゲートとしては機能する**。

**しかし `ShortFeeRate` は空売り可能な 5 銘柄すべてで 1.5 だった。** 借株在庫は 120 万〜2,600 万と 20 倍以上開いているのに料率は動かない。**借株料は銘柄別ではなく一律である公算が高い。**

> **[ADR-0016](../../planning/projects/ai-stock-trading/07_adr/ADR-0016_short-selling-staged-release.md) 決定 3 の「借株料が年率 20% を超える銘柄への空売りは拒否する」は、この情報源では恒久的に発火しない統制になる。**
> 借株料でコストと危険度を同時に弾くという設計意図は達成できない。**代わりに機能するのは `IsShortPermit` による可否のゲートである。**
> 決定 3 を「照会できること」と「閾値で弾けること」に分けて評価し、後者が成立しないことを計画へ環流する必要がある。
> なお `1.5` の単位（年率 1.5% か否か）は未確定である。

#### ⚠️ レート制限を実測した

```
TSLA retType=-1 Get Margin Trading Data request failed due to high frequency.
                Maximum 10 times per 30 seconds.
```

**`TrdGetMarginRatio` は 30 秒あたり 10 回**である。決定 3 は「発注前に借株料を照会する」ことを求めているため、**照会をキャッシュせずに毎回叩く実装は上限に触れる**。監視銘柄数と判断頻度によっては設計上の制約になる。

#### 旧: `ShortFeeRate` が両銘柄とも 1.5 である点は要確認（第 4 回時点の記述）

**AAPL と GME で `ShortFeeRate` が同一の 1.5 だった。** `ShortPoolRemain` は銘柄ごとに大きく異なる（26,452,338 対 1,898,200）ため API は銘柄を認識している。それでも借株料が同値ということは、次のいずれかである。

1. moomoo が**一律の借株料**を課しており、銘柄別の需給を反映しない
2. 調査した 2 銘柄がたまたま同じ料率の帯にある
3. このフィールドが借株料ではなく別の意味（信用金利等）を持つ

**1 だとすると ADR-0016 決定 3（年率 20% 超を拒否）は恒久的に発火しない統制になる。** 「借株料を照会して閾値で弾く」という設計が、実質的に何も弾かないことになる。**銘柄数を増やして確認する必要がある**（借りにくい銘柄で値が上がるか）。単位（`1.5` が年率 1.5% なのか他の単位か）も未確定である。

#### 帰結

- **決定 3・決定 7 は実装可能である。** 借株料・借株可否・維持率のいずれも実弾口座から取得できる。
- **ただし Stage 1（SIMULATE）ではこれらの統制を一切検証できない。** 照会 API が SIMULATE で使えないため、ADR-0016 決定 14 が挙げた「Stage 1 で検証できない統制」は**当初の想定より深刻**である。決定 14 は「SIMULATE は年率 6% 固定のため 20% を超えない」＝値は取れるが閾値に届かない、と書いているが、**実際には値そのものが取れない**。
- **実装上、借株料の照会は実弾口座のヘッダで行う必要がある。** SIMULATE で発注しながら実弾口座へ照会するという**環境をまたぐ構成**になり、[IADR-0111](../adr/IADR-0111_broker-tier-selection.md)（provider × environment の直交 2 軸）の前提に影響する。**これは設計判断であり IADR に残す必要がある。**

### 訂正 3（2026-08-05・第 5 回）— `TrdSide_SellShort` は使えない。空売りは `Sell` で表現する

SIMULATE 口座へ `TrdSide_SellShort` の指値注文を出したところ、**ブローカに拒否された**。

```
SHORT PlaceOrder retType=-1
msg=Order side must be BUY or SELL. A SELL order without holdings will open a short position.
    A BUY order when short will close the short position.
```

**本仕様書が §設計「SDK 契約からの静的確認」に記録した「`TrdSide` に `SellShort` / `BuyBack` があるので空売り専用区分が存在する」という判定は誤りである。** SDK の列挙に値が存在することと、ブローカがその値を受理することは別である。**型定義だけを見て「到達可能」と判定した推論が外れた例として記録する。**

moomoo の実際の約束事は次のとおりである。

| 意図 | 発注する `TrdSide` | 空売りになる条件 |
| --- | --- | --- |
| 新規ショート | **`Sell`** | **建玉を持っていないこと** |
| ショートの手仕舞い | **`Buy`** | ショート建玉を持っていること |

**つまり空売りと通常の売却は API 上で区別されない。** 建玉の状態から暗黙に決まる。

#### この訂正が実装に与える影響

1. **現行の `MoomooSide { Buy, Sell }` の 2 値は、moomoo の契約に対して正しい。** 本仕様書が当初「`SellShort` / `BuyBack` を表現できないので項目 2 が成立しても空売りを発注できない」と指摘し #331 へ引き継ぐとした点は**取り下げる**。SDK の列挙に合わせて 4 値へ広げると、かえってブローカが受理しない値を扱うことになる。
2. **「建玉を持たない `Sell` は新規ショートになる」という moomoo の挙動は、[IADR-0119](../adr/IADR-0119_decision-derived-close.md)（#298）が扱った問題そのものである。** AI の `Sell` 判断が意図せず新規ショートとして発注される経路が、**ブローカ側の仕様として実在する**ことが実測で確認された。IADR-0119 の是正（手仕舞いか新規かを判断側で決める）は、推測ではなく**実際の仕様に対する必要な防御**だったことになる。
3. **統制は「発注前に建玉状態を確定させる」ことに依存する。** 建玉を取り違えると、手仕舞いのつもりが新規ショートになる。[IADR-0118](../adr/IADR-0118_broker-position-reconciliation.md)（ブローカ実ポジションとの突合）の重要度が上がる。

#### 第 6 回の再実行 — `Sell`（建玉なし）で ✅ **成立**

probe を `TrdSide_Sell` へ変更して再実行した結果、**空売り注文は受理された**。

```
SHORT PlaceOrder retType=0 price=400
SHORT orderId=9078735564654079011
SHORT ORDER id=9078735564654079011 side=TrdSide_Sell status=OrderStatus_Submitting qty=1 fill=0 price=400
SHORT Cancel  retType=0
```

**項目 2 は成立する。** 発注 → 状態確認 → 取消の一巡が SIMULATE の信用口座で通った。約定はしていない（`fill=0`・板から離れた指値）。

**ただし維持率フィールドは埋まらなかった。**

```
SHORT FUNDS(after) riskLevel=- initialMargin=- maintenanceMargin=0 shortMv=0 maxPowerShort=-
```

注文が `Submitting` のままで**約定していない**ため建玉が生じていない。**項目 3 の維持率は SIMULATE では依然として未確定**である。埋めるには実際に約定させて建玉を持つ必要があるが、
そもそも `TrdGetMarginRatio` が SIMULATE で使えない以上、**決定 7（維持率 40%）の検証は Stage 1 では成立しない**。これは [ADR-0016](../../planning/projects/ai-stock-trading/07_adr/ADR-0016_short-selling-staged-release.md) 決定 14 が
「Stage 1 で検証できない統制」として既に想定している範囲である。

#### レート制限の実装上の含意（追加の実測）

第 6 回では銘柄間に 3.5 秒の待ちを入れたにもかかわらず、**8 銘柄目の TSLA が再びレート制限で落ちた**。
原因は、**同じ 30 秒窓で SIMULATE 口座への `TrdGetMarginRatio` を 3 回（AAPL / TSLA / GME）叩いていた**ためである。
合計 11 回となり上限 10 回を超えた。

> **失敗した照会もレート制限にカウントされる。** SIMULATE 側の 3 回はすべて
> `does not support Stocks in US Market` で失敗しているが、それでも枠を消費している。
> **エラー時のリトライを素朴に実装すると、リトライ自体が枠を食って連鎖的に失敗する。**

### 実弾口座への読み取り照会について（実施済み・私からは実行できない）
