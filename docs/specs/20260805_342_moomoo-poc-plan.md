...
| 2 空売り発注 | 🟡 **未確定**。`SellShort` は拒否された。`Sell`（建玉なし）で再試行が要る | 空売りと通常売却が API 上で区別されない（IADR-0118 / 0119 の重要度が上がる） |
| 3 借株料 | 🟡 **照会は成立（実弾口座のみ）だが一律 1.5 で銘柄別でない** | **決定 3 の 20% 閾値は恒久的に発火しない。要環流** |
| 3 借株可否 | ✅ **成立（実弾口座のみ）**`IsShortPermit` | 同上 |
| 3 維持率 | ✅ **成立（実弾口座のみ）**`ImShortRatio=50` / `MmShortRatio=30` | 決定 7 は実装可能。SIMULATE では検証できない |
...
**項目 2 の判定は未確定である。** `SellShort` が拒否されただけで、**`Sell`（建玉なし）で空売りが成立するかは未検証**である。probe を `TrdSide_Sell` に変更して再実行する必要がある。
