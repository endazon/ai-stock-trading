...
- **項目 7（米国株日足 OHLC・補助）**: 見通しは良い。**`Historical Candlestick Quota: 300`** が付与されており、`QotRequestHistoryKL` / `QotRequestHistoryKLQuota` も SDK にある。ただし quote 側の probe は `MMSPI_Qot`（133 メソッド）の実装が要るため未着手。**#382 を解く可能性がある。**
...
| 7 日足 OHLC（補助） | ⏸ 未実施（quota 300 は付与済み） | #382 を解く可能性 |
