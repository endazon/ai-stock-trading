...
| G2 | **空振りパターンの通知** | 1 ファイルも一致しないパターンは CI notice で報告する（誤記・対象消滅の検知）。**失敗はさせない**（部分実行では正当に空振りするため） |
| G3 | `reason` 必須 | 理由の無いエントリは失敗 |
| **G4** | **除外集合 ⊆ 自動生成ファイル** | 実ツリーを走査し、`// <auto-generated />` を持たない**手書き**ファイルが除外に一致したら**失敗**（IADR-0143 決定2 を機械的に担保する。決定6） |
| **G5** | **パターンの隠蔽（shadowing）** | 空振りパターンを先行パターンが飲み込んでいたら**失敗**（設定が広すぎることの決定的証拠） |

G1 の 0.35 は現状の除外率 23.9% に対する余裕であり、「マイグレーションが今の 1.5 倍に増えても通るが、
`**/*.cs` のような事故は止まる」水準に置く。

**G4 / G5 は当初なかった**（監査で穴が実測された）。`**/Migrations/*.Designer.cs` → `**/Migrations/**` の
変異を当てると、手書き本体 24 ファイル・611 行が黙って分母から外れ（81.55% → **85.50%**）ながら
**`check-coverage.js` は exit 0・テストは 152 件全件緑**であった。当時の否定形テストが `applyExcludes` に
**ハードコードしたパターン**を渡しており、**実設定（`coverage-floor.json`）を検査していなかった**ためである。
G4 は名前ではなく**中身**（`// <auto-generated />` マーカー）で判定するため、パターンをどう広げても欺けない。
詳細と棄却案は IADR-0143 決定6。
...
- [ ] **実設定（`coverage-floor.json`）を対象に**、手書きファイルの除外が失敗として検出される（G4）
- [ ] **`**/Migrations/**` への変異で `check-coverage.js` とテストの両方が落ちる**（実測すること）
...
**実設定 × 実ツリーを対象にする検査**（監査で見つかった穴の是正。合成入力の検査だけでは変異が素通りした）:

| 確認 | 種別 | 内容 |
| --- | --- | --- |
| マーカーが判別する | 正 | 実ファイルで `*.Designer.cs` = 自動生成 / 本体 = 手書き と判定される |
| **実設定が手書き本体を除外しない** | **否定形・実物** | 実ツリーの手書き本体を列挙し、**実設定**のパターンに 1 件も一致しない |
| **実設定が除外するのは自動生成だけ** | **否定形・実物** | `findWrongfullyExcluded(backend, 実設定)` の違反が空 |
| **検査に歯があること（変異検査の常設化）** | **メタ** | 実ツリーへ `**/Migrations/**` を当てると手書き本体が違反として挙がり、自動生成物は挙がらない |
| 実設定に隠蔽が無い | 正 | `findShadowedPatterns` が空 |
| **隠蔽を検出する** | **否定形** | 先行パターンが後続を飲み込む設定で検出される |
| 空振り ≠ 隠蔽 | 否定形 | 対象が単に存在しないだけでは隠蔽と判定しない（誤検出しない） |

**メタ検査を置く理由**: 上の 2 本は「違反が**無い**こと」を主張するため、検査器が壊れて常に空を返しても
緑になる。変異を常設テストとして固定して初めて、検査に歯があることが保証される。
