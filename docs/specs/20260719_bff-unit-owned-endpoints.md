...
- [ ] `AiStockTrading.Bff.Endpoints` が AST 単独で `dotnet build` 成功（`FrameworkReference` のみ）。
- [ ] 3 モジュールのルート定義・`ProxyAsync`・DELETE 本文転送・502/4xx/409 透過・匿名 401・`Authorization` 伝播が
      MSP interim とバイト等価（`namespace` 以外の差分なし）。
- [ ] `backend.slnx` に登録され、AST 既存 CI（build/format）が緑。
...
- 振る舞いのテストは MSP 側の合成後アプリで担保される（`Platform.Bff.Tests` の Assumptions/RiskControls/Monitor）。
