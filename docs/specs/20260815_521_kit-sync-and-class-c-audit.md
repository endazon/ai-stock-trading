...
> **測ったのは IADR-0203 を書く前、公開したのは書いた後である** ——
> `docs/adr/IADR-0203_*.md` が走査対象に入って +1 になった。
> **これは母集合の規則 8 そのものである**（「走査対象に自分の記録が入るときは、記録を書く行為が
> 母集合を動かす。走査がそのまま返す数を先に出し、除外と時点を明示する」）。
> 🔴 **規則 8 をキットから取り込む当の PR で、規則 8 を破った。**
> 是正: **294 → 295（本 PR が追加した `.md` は 3 件だが、走査対象に新規で入るのは IADR-0203 の 1 件）**。
> `.claude/rules/traceability.md` と `docs/adr/README.md` は既存ファイルの更新であり母数を動かさない。
