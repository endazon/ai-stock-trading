...
---
<!-- trace:
ids: [FR-02, FR-08, FR-10, FR-11, FR-14, FR-19, FR-20, UC-06, UC-07]
...
- 関連 ADR: **ADR-0012（計画リポ）**（取引データの MCP 非公開）／ADR-0003（AI 判断のガードレール）／ADR-0004（収集ソースの受理）

## 本書の読み方 — **「無いこと」を 4 語で書き分ける**（IADR-0175: セキュリティ仕様書における「無いこと」の書き分け）

**本書に空欄は無い。** 記述が無い＝未記入、ではない。以下の 4 語で区別する。

| 表記 | 意味 |
| --- | --- |
| （記述あり） | **対策がある**。コードの所在または IADR を添えてある |
...
### 方式 — Keycloak OIDC / JWT（IADR-0011: 基盤ランタイム Foundation は最小移植しコピー＋AiStockTrading 命名で持つ・platform ADR-0004 の最小移植）
...
| `OwnerOrService` | `trading-owner` **または** `trading-service` | **読み取り系の同期照会のみ**（sizing-context / open-positions / daily-policy・IADR-0051: サービス間同期照会の service-to-service 認証） |
...
### Discord からの操作 — **6 層。すべて既定拒否**（FR-14 / UC-06 / IADR-0062: Discord Bot は Gateway 常駐＋多層認証とし、既定 no-op・owner トークンで kill switch を呼ぶ）
...
| **個人情報** | — | **本システムは単独利用者運用であり、第三者の個人情報を扱わない**（IADR-0011: 基盤ランタイム Foundation は最小移植しコピー＋AiStockTrading 命名で持つ：認可は単層） |
...
| 実弾解禁との関係 | IADR-0056: moomoo SIMULATE PoC 完了に基づき実アダプタを実装（実弾は引き続きゲート） §3 が実弾解禁の前提に挙げる「**秘匿情報の Vault 化**」は**未充足のまま**である |
...
| 対象イベント | 記録項目 | 保管期間 |
| --- | --- | --- |
| **全ドメインイベント**（FR-11 / UC-07 / IADR-0019: 監査ログは専用サービスが全ドメインイベントを購読し追記専用台帳へ記録する）<br>`AuditService.Application/State/AuditEntry.cs` | `Id`（冪等キー＝Wolverine `Envelope.Id`）／`EventType`／`CorrelationId`（注文系は `DecisionId`・市場系は `EventId`）／`Symbol`／`Summary`（人間可読 1 行）／`Detail`（**イベント全量 JSON**）／`OccurredAt`（イベント時刻）／`RecordedAt`（記録時刻）。**追記専用** | 🔴 **未実装**（下記） |
...
パージ（IADR-0059: 重複排除ストアは「終端行のみ・保持期間 90 日・下限クランプ付き」でパージし、未確定の行には触れない）の対象は**重複排除ストア 2 つ**（`processed_messages` / `order_dispatch_reservations`）**だけ**であり、`audit_events`・`cost_entries`・`executed_orders` は**明示的に対象外**である。
...
| ID | 脅威 | 影響 | 対策 |
| --- | --- | --- | --- |
| T-1 | **取引データが外部 AI エージェントへ流出する** | 建玉・発注履歴・判断根拠の第三者流出 | ✅ **MCP へ公開しない**。`McpExposureNotDeclaredTests` が `backend/` と `deploy/` を走査し、**宣言が入り込むとテストが落ちる**（IADR-0171: MCP 非公開の維持）。上記「MCP への公開」節を参照 |
| T-2 | **無認証の内部エンドポイントを踏まれ、収集サイクルを任意に起動される** | LLM 費用の消費・レート制限の枯渇（**発注はしない**。判断は下流の統制を通る） | ✅ **対策済み**（[#456](https://github.com/endazon/ai-stock-trading/issues/456) / IADR-0176: run-once の認可と CronJob のトークン取得）。`POST /internal/collection/run-once` は **`OwnerOrService`** を要求する。**渡しているのは「サイクルを起こす権限」であって「発注する権限」ではない**（決定1）。CronJob は **client_credentials でトークンを取ってから**叩き、**資格情報が無い・token 取得に失敗した場合は Job が赤くなる**（fail-closed・決定2）。**退行は構造テストが止める** —— `UnauthenticatedEndpointsNotAllowedTests` が**認可メタデータを持たないエンドポイントを許可リスト以外に許さない**（決定4） |
| T-3 | **JWT の検証が緩く、別クライアント向けトークンが通る** | 認可の迂回 | 🔴 **未対策（判断済み・値は据え置き）**。`RequireHttpsMetadata = false`（メタデータ取得が平文 HTTP を許す）・`ValidateAudience = false`（**aud を検証しない**）。**基盤（microservices-platform）の Keycloak クライアント構成と揃っている必要があり、片側だけ厳しくすると全サービスが 401 になる**ため据え置いた（IADR-0176: run-once の認可と CronJob のトークン取得 決定3）。**再判断する条件**: ①Ingress が入り外部から到達可能になる ②マルチテナント化する ③基盤側の構成が確認できるようになる。担当: [#24](https://github.com/endazon/ai-stock-trading/issues/24) |
| T-4 | **収集した外部テキストが LLM への指示として作用する**（プロンプトインジェクション） | AI が意図しない判断・発注を行う | ✅ **RAG 取得文脈を「データ」として構造分離し、出典で限定する**（IADR-0169: RAG 取得文脈のプロンプトインジェクション対策）。文脈は**参考情報として本判断プロンプトのみ**に注入し、**既定 no-op・取得失敗は文脈なしへ縮退**する（IADR-0072: RAG 文脈は Application 抽象ポートで受け、本判断プロンプトのみに参考情報として注入し、既定 no-op・取得失敗は文脈なしへ縮退する） |
| T-5 | **AI の暴走・誤判断が実弾の発注になる** | 実損 | ✅ **多重。** 実弾 triple-latch（IADR-0060: OpenD 本番化は「既定 no-op の整備」として先行し、切替はゲート＋チェックリストで人手に残す）／ブローカー階層の閂 0〜4（IADR-0111: ブローカー選択は provider × environment の直交 2 軸で表現する）／**アダプタは `TrdEnv_Simulate` 固定**（`Mode=Live` でも SIMULATE を用いる）／`BrokerFactory` の安全既定はペーパー／kill switch は `OwnerOnly` |
| T-6 | **設定で統制を実質無効化される** | ガードの空洞化 | ✅ **構造クランプ。** 保持期間は**下限** 7 日（IADR-0059: 重複排除ストアは「終端行のみ・保持期間 90 日・下限クランプ付き」でパージし、未確定の行には触れない）、為替の鮮度上限は**上限** 30 日（IADR-0174: 為替レートの鮮度 — 警告と停止の分離）。**設定値ではなく構造で担保する** |
...
| 5 | **JWT の `ValidateAudience=false` / `RequireHttpsMetadata=false`**（**判断済み・値は据え置き**。再判断の条件は T-3 と IADR-0176: run-once の認可と CronJob のトークン取得 決定3）。**`/internal/collection/run-once` の無認証は #456 で解消した** | [#24](https://github.com/endazon/ai-stock-trading/issues/24) |
...
- 実装 ADR: IADR-0176: run-once の認可と CronJob のトークン取得（**run-once の認可・CronJob のトークン・無認証経路の構造固定**）／IADR-0175: セキュリティ仕様書における「無いこと」の書き分け（**本書の書き分けの規約**）／IADR-0171: MCP 非公開の維持（MCP 非公開）／IADR-0011: 基盤ランタイム Foundation は最小移植しコピー＋AiStockTrading 命名で持つ・IADR-0051: サービス間同期照会の service-to-service 認証（認証・認可）／IADR-0062: Discord Bot は Gateway 常駐＋多層認証とし、既定 no-op・owner トークンで kill switch を呼ぶ（Discord 多層認可）／IADR-0169: RAG 取得文脈のプロンプトインジェクション対策（プロンプトインジェクション）
