...
- **ADR / IADR の実在性**（issue #319）: 書式に加え、スコープの `IADR-xxxx` は本リポ `docs/adr/` に、
...
4. **PR タイトル**（= スカッシュ後にコミット件名として develop へ載る。#314 で追随漏れが実際に混入した。
   ブランチ名は既 push なら変更不要だが、タイトルは必ず直す）
...
（ブランチ名由来の規約外件名がそのまま develop に載る事象）。これを防ぐため `pr-title.yml` が PR タイトルを規約
...
- **誤記補正（`action: "remap"`）**: 誤った起点 ID・種別・要約を、CHANGELOG 上でのみ差し替える。
  ...
  - 例: 件名の起点 ID が誤って `feat(FR-10)` となっているが実体は基盤スケルトン（P0）である場合、
    `scope` を `FR-10` → `P0` へ補正する。実体が大規模実装なら `type` は `feat` のまま保持し、`docs` へは
    remap しない（実装をドキュメントとして過小計上する新たな誤帰属を避ける）。
  - 現時点で本リポジトリに補正対象のコミットは無く、`overrides` は空である。
