...
「一意・昇順。**欠番は許す**（再利用は禁止）」であり（[IADR-0272](../../.ai-context/adr/IADR-0272_iadr-gap-acceptance-over-renumbering.md)）、
...
- **理由**: 2026-08-28 の並行実装（12 本のブランチ）で生じた 16 件の欠番を「次の空き番号を
  ...
  参照追随。実測は IADR-0272 参照）。**欠番を埋めない**運用に統一することで、この種の大規模な
  繰り下げ作業自体を今後発生させない。
