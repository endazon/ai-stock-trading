...
`../project-planning` で `node tools/doc-checks/gen-plan-ranges.js --check` が ADR **[1, 41]・41 件・欠番なし**）。
...
- **11**: **窓（時間差）を扱う是正は、形を決める前に「増える側」と「減る側」の両方のプローブを書き、3 通りの形で
  実測する。表を出せるまで形を確定しない。** 片側だけの形は**必ず逆側が空く**。
- **機械検査は無い**（1〜8 と同じ）。**着手前に自分で引き、結果と除外理由を作業仕様書へ書く**（規則 6）。
  実例と 3 通りの表は[別紙](../../docs/how-to/correction-population-and-window-procedure.md)。
